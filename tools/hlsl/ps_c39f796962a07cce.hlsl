// ps_c39f796962a07cce.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 228 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000390 10041000 0000080A 00000000 00007908 003F00FF 00000001 0000F050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c12); // float4
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c14); // float2
float4 SpotDirection : register(c13); // float3
float4 UniformScalar_0 : register(c5); // float
float4 UniformScalar_1 : register(c6); // float
float4 UniformScalar_2 : register(c7); // float
float4 UniformScalar_3 : register(c8); // float
float4 UniformScalar_4 : register(c9); // float
float4 UniformScalar_5 : register(c10); // float
float4 UniformScalar_6 : register(c11); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D ModShadowAccumTexture : register(s8);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r6.xy = r0.xy * UniformScalar_2.xx;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.zw = r0.xy * UniformScalar_0.xx;
    r9.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.x = UniformScalar_1.x * 4.0;
    r9.w = ps;
    ps = 1.0 / r5.w;
    r9.xy = r5.xy * ScreenPositionScaleBias.xy;
    r6.z = ps;
    r9.xy = r9.xy * r6.zz + ScreenPositionScaleBias.wz;
    ps = r7.x;
    r6.zw = r0.wz * UniformScalar_4.xx;
    ps = r1.x * ps;
    r8.xy = r9.zw * abs(r8.xy);
    r7.x = ps;
    ps = UniformScalar_1.x * r1.y;
    r11.x = saturate(r5.w * 0.0001);
    r7.y = ps;
    r0.xyz = tex2D(LightAttenuationTexture, r9.xy).xyz;
    r9.xy = tex2D(ModShadowAccumTexture, r8.xy).xy;
    r12.xyz = tex2D(Texture2D_6, r6.zw).xyz;
    r6.z = tex2D(Texture2D_2, r1.xy).x;
    r15.xyz = tex2D(Texture2D_4, r7.zw).xyz;
    r16.xyz = tex2D(Texture2D_5, r7.xy).xyz;
    r10.xyw = tex2D(Texture2D_0, r7.zw).xyz;
    r13.xyz = tex2D(Texture2D_1, r7.xy).xyz;
    r7.xyz = tex2D(Texture2D_3, r6.xy).wxy;
    ps = 0.1 - -r2.z;
    r6.x = ps;
    ps = UniformVector_1.x * UniformVector_1.w;
    r1.xyz = -ModShadowColor.xyz + 1.0;
    r11.y = ps;
    ps = UniformVector_1.y * UniformVector_1.w;
    r0.w = dot(r4.zxy, r4.zxy);
    r11.z = ps;
    ps = UniformVector_1.z * UniformVector_1.w;
    r6.y = dot(r3.zxy, r3.zxy);
    r11.w = ps;
    r8.xy = r7.yz * 2.0 - 1.0;
    ps = 1.0 - r6.y;
    r1.w = dot(r2.zxy, r2.zxy);
    r6.w = saturate(ps);
    r14.xyz = r13.xyz * 2.0 - 1.0;
    r13.xyz = r10.xyw * 2.0 - 1.0;
    ps = rsqrt(abs(r1.w));
    r7.yzw = -r15.xyz + r16.xyz;
    r1.w = ps;
    ps = rsqrt(abs(r0.w));
    r5.xyz = r1.www * r2.xyz;
    r0.w = ps;
    ps = rsqrt(abs(r6.y));
    r10.xyz = r0.www * r4.xyz;
    r6.y = ps;
    ps = 5.0 * r6.x;
    r2.xyz = r6.yyy * -SpotDirection.xyz;
    r6.y = saturate(ps);
    r7.yzw = r7.yzw * r6.zzz + r15.xyz;
    ps = log2(r6.w);
    r4.xyz = r14.xyz - r13.xyz;
    r6.x = ps;
    r4.xy = r4.xy * r6.zz + r13.xy;
    r6.w = r4.z * r6.z - 2.0;
    ps = 1.0 - r6.y;
    r7.yzw = r11.yzw * r7.yzw;
    r6.z = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r6.y = dot(r2.zxy, r3.zxy);
    r11.y = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r2.xzw = r7.yzw * r12.xyz;
    r11.z = ps;
    r6.w = r10.w * 2.0 + r6.w;
    ps = (-1.0) + r7.x;
    r7.yzw = -r11.xyz + 1.0;
    r6.z = ps;
    r3.zw = r6.wz * r7.yy + 1.0;
    ps = UniformScalar_3.x * r7.y;
    r7.x = ps;
    r6.zw = r7.xx * r8.xy + r4.xy;
    ps = -UniformVector_0.x;
    r3.xy = r6.zw * r7.yy;
    r3 = (r7.xyyy > 0.0) ? r3.wxyz : float4(1.0, 0.0, 0.0, 1.0);
    r8 = (r7.xyyy >= 0.0) ? r3 : float4(1.0, 0.0, 0.0, 1.0);
    ps = 1.0 + ps;
    r6.z = dot(r8.wyz, r8.wyz);
    r2.y = ps;
    ps = -SpotAngles.x - -r6.y;
    r3.xyz = r2.xzw * r8.xxx;
    r6.y = ps;
    r4.xyz = r3.xyz * UniformScalar_5.xxx + UniformScalar_6.xxx;
    ps = rsqrt(abs(r6.z));
    r2.x = saturate(r6.y * SpotAngles.y);
    r6.y = ps;
    ps = -UniformVector_0.y;
    r8.xyz = r8.yzw * r6.yyy;
    ps = 1.0 + ps;
    r6.y = dot(r8.zxy, r10.zxy);
    r2.z = ps;
    ps = -UniformVector_0.z;
    r6.yzw = r8.xyz * r6.yyy;
    r6.yzw = r6.yzw * 2.0 - r10.xyz;
    ps = 1.0 + ps;
    r6.y = saturate(dot(r5.zxy, r6.wyz));
    r2.w = ps;
    ps = log2(r6.y);
    r3.xyz = r3.xyz * r2.yzw;
    r9.z = ps;
    r6.yzw = r9.zxy * float3(15.0, 0.875, 0.875);
    r2.yz = r6.zw * r7.zw + 0.125;
    ps = pow(2.0, r6.y);
    r6.z = saturate(dot(r8.zyx, r5.zyx));
    r6.y = ps;
    ps = LightColorAndFalloffExponent.w * r6.x;
    r7.xyz = r4.zxy * r6.yyy;
    r6.y = ps;
    r7.yz = r3.xy * r6.zz + r7.yz;
    r7.w = r3.z * r6.z + r7.x;
    ps = pow(2.0, r6.y);
    r6.xw = r2.yx * r2.zx;
    r7.x = ps;
    r6.xyz = r6.xxx * r1.xyz + ModShadowColor.xyz;
    r7.xyz = r7.yzw * r7.xxx;
    r7.xyz = r7.xyz * r0.xyz;
    r7.xyz = r7.xyz * LightColorAndFalloffExponent.xyz;
    r7.xyz = r7.xyz * r6.www;
    r6.xyz = r7.xyz * r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
