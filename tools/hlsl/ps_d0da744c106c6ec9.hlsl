// ps_d0da744c106c6ec9.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 384 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000600 10041700 0000080A 00000000 00006908 003F00FF 00000001 00003050 00003151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c15); // float4
float4 ModShadowAccumResolution : register(c20); // float2
float4 ModShadowColor : register(c18); // float3
float4 ModShadowGroupColor : register(c19); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c17); // float2
float4 SpotDirection : register(c16); // float3
float4 UniformScalar_0 : register(c10); // float
float4 UniformScalar_1 : register(c11); // float
float4 UniformScalar_5 : register(c12); // float
float4 UniformScalar_6 : register(c13); // float
float4 UniformScalar_7 : register(c14); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UniformVector_7 : register(c9); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
sampler2D Texture2D_8 : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 r21 = 0.0;
    float4 r22 = 0.0;
    float4 r23 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r21.w = saturate(r5.w * 0.0001);
    r6.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.y = -r1.y + 1.0;
    r6.y = ps;
    r6.xy = r6.xy * abs(r8.xy);
    r10.yw = tex2D(ModShadowAccumTexture, r6.xy).xy;
    r10.z = r1.x * UniformVector_2.x;
    ps = r5.w;
    r6.zw = r0.xy * UniformScalar_0.xx;
    ps = (-4e+02) + ps;
    r0.zw = r1.xy * 6.0;
    r7.z = ps;
    ps = r1.x;
    r9.zw = r5.xy * ScreenPositionScaleBias.xy;
    ps = UniformVector_7.x * ps;
    r9.xy = r1.xy * UniformVector_1.xy;
    r1.z = ps;
    ps = 1.0 / UniformVector_1.x;
    r6.x = r2.z + 0.1;
    r6.y = ps;
    ps = 5.0 * r6.x;
    r7.x = dot(r4.zxy, r4.zxy);
    r4.w = saturate(ps);
    ps = rsqrt(abs(r7.x));
    r10.x = saturate(r7.z * 0.00022222222);
    r6.x = ps;
    ps = 1.0 / r5.w;
    r8.xyz = r6.xxx * r4.xyz;
    r7.z = ps;
    ps = 1.0 / UniformVector_1.y;
    r20.yzw = r10.ywx * float3(0.875, 0.875, -0.025);
    r7.x = ps;
    r9.xy = r8.xy * r20.ww + r9.xy;
    ps = UniformVector_2.x * r6.y;
    r4.xyz = r9.wyz * r7.zxz;
    r6.x = ps;
    ps = r6.x;
    r7.xz = -r4.yw + 1.0;
    ps = r9.x * ps;
    r10.yw = r7.xy * UniformVector_2.yy;
    r10.x = ps;
    ps = r1.y;
    r5 = r10 - 0.5;
    ps = UniformVector_7.y * ps;
    r17 = float4((r10.xywz >= 0.5));
    r1.w = ps;
    ps = ScreenPositionScaleBias.w + r4.z;
    r5 = r5.xywz - r10.xywz;
    r6.x = ps;
    r5 = r5.xywz * r17.xywz + r10;
    ps = ScreenPositionScaleBias.z + r4.x;
    r5 = r5 + r5;
    r6.y = ps;
    r4.xyz = tex2D(LightAttenuationTexture, r6.xy).xyz;
    r13.xyz = tex2D(Texture2D_7, r9.xy).xyz;
    r22 = tex2D(Texture2D_1, r5.xy);
    r23 = tex2D(Texture2D_1, r5.zw);
    r5.xyz = tex2D(Texture2D_8, r1.zw).xyz;
    r18.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r9.xyz = tex2D(Texture2D_0, r9.xy).xyz;
    r11.xyz = tex2D(Texture2D_2, r0.zw).xyw;
    r6.yzw = tex2D(Texture2D_3, r6.zw).wxy;
    r1.xyz = tex2D(Texture2D_2, r1.xy).xyw;
    r10 = tex2D(Texture2D_6, r0.xy).wxyz;
    ps = -UniformVector_0.x;
    r16.xyz = UniformVector_6.xyz * UniformVector_6.www;
    ps = 1.0 + ps;
    r14.xyz = -ModShadowColor.xyz + 1.0;
    r12.x = ps;
    ps = -UniformVector_0.y;
    r7.w = float((UniformScalar_5.x >= 1.0));
    ps = 1.0 + ps;
    r0.x = dot(r2.zxy, r2.zxy);
    r12.y = ps;
    ps = -UniformVector_0.z;
    r0.w = float((UniformScalar_5.x > 1.0));
    ps = 1.0 + ps;
    r9.w = r10.x * UniformScalar_6.x;
    r12.z = ps;
    ps = r1.x + r1.x;
    r6.x = dot(r3.zxy, r3.zxy);
    r19.x = ps;
    ps = r1.y + r1.y;
    r19.zw = r6.zw + r6.zw;
    r19.y = ps;
    r0.yz = r11.xy * 2.0 - 1.0;
    r15.xyz = r9.xyz * 2.0 - 1.0;
    ps = (-1.0) + r6.y;
    r18.xyz = r18.xyz + r18.xyz;
    r20.x = ps;
    ps = 1.0 - r6.x;
    r6.z = r11.z * r1.z;
    r6.y = saturate(ps);
    r1.xyz = (-abs(r0.www) >= 0.0) ? r5.xyz : 1.0;
    ps = rsqrt(abs(r0.x));
    r6.w = -r10.x + 1.0;
    r0.x = ps;
    ps = rsqrt(abs(r6.x));
    r2.xyz = r0.xxx * r2.xyz;
    r6.x = ps;
    r5.xzw = (-abs(r7.www) >= 0.0) ? 1.0 : r1.xzy;
    ps = log2(r6.y);
    r1.xyz = r6.xxx * -SpotDirection.xyz;
    r6.x = ps;
    ps = UniformVector_5.x * r6.w;
    r6.y = dot(r1.zxy, r3.zxy);
    r11.y = ps;
    ps = UniformVector_5.y * r6.w;
    r1.x = dot(r17.xyy, float3(1.0, 1.0, 1.0));
    r11.z = ps;
    ps = (-1.0) - -r6.z;
    r1.y = dot(r17.wzz, float3(1.0, 1.0, 1.0));
    r17.x = ps;
    r0.x = (r1.y == 0.0) ? r23.x : r23.y;
    r7.w = (r1.x == 0.0) ? r22.x : r22.y;
    ps = ModShadowGroupColor.x * r7.z;
    r1 = r1.xxyy + float4(-3.0, -2.0, -2.0, -3.0);
    r21.y = ps;
    r7.w = (r1.y == 0.0) ? r22.z : r7.w;
    r0.x = (r1.z == 0.0) ? r23.z : r0.x;
    r21.x = (r1.w == 0.0) ? r23.w : r0.x;
    r7.w = (r1.x == 0.0) ? r22.w : r7.w;
    r18.w = r7.w * 2.0 - r6.z;
    ps = ModShadowGroupColor.y * r7.z;
    r7.w = max(r21.x, 0.0);
    r21.z = ps;
    ps = -SpotAngles.x - -r6.y;
    r1 = -r21 + 1.0;
    r7.x = ps;
    r3.yzw = r20.xyz * r1.wyz + float3(1.0, 0.125, 0.125);
    ps = r1.w;
    r0.x = min(r7.w, 0.3);
    r6.y = ps;
    ps = UniformScalar_1.x * r6.y;
    r18 = r18.zxyw + float4(-1.0, -1.0, -1.0, 1.25);
    r7.w = ps;
    r14.w = r18.x * UniformVector_4.z - r15.z;
    ps = 3.3333333 * r0.x;
    r19 = r19 - 1.0;
    r18.x = ps;
    r7.yz = r7.ww * r19.zw + r18.yz;
    r6.y = (r7.w > 0.0) ? r3.y : 1.0;
    ps = 0.5 * r0.y;
    r6.z = saturate(r18.w - r6.z);
    r18.y = ps;
    ps = 0.5 * r0.z;
    r13.xyz = r6.zzz * r13.xyz;
    r18.z = ps;
    r16.w = (r7.w >= 0.0) ? r6.y : 1.0;
    r6.yz = r7.yz * UniformVector_4.xy - r15.xy;
    ps = SpotAngles.y * r7.x;
    r17.yz = r19.xy + r18.yz;
    r5.y = saturate(ps);
    r0.xy = r18.xx * r17.yz + r6.yz;
    ps = r1.x;
    r7 = r16 * r10.yzwx;
    r6.y = ps;
    r6.z = r18.x * r17.x + 1.0;
    ps = 2.5 * r6.y;
    r1.xyz = r7.xzy * r5.xzw;
    r10.w = ps;
    ps = abs(r6.z) * abs(r6.z);
    r6.y = r1.x * r16.w;
    r3.x = ps;
    ps = r6.y;
    r10.yz = r1.zy * r7.ww;
    ps = r10.x * ps;
    r5.xz = r3.xz * r3.xw;
    r10.x = ps;
    r7.xyz = r5.zzz * r14.xyz + ModShadowColor.xyz;
    r14.xyz = r10.xyz * r6.zzz - r13.xyz;
    r7.w = saturate(r10.w * r6.z - 0.5);
    r3.xy = r0.xy * r7.ww + r15.xy;
    r11.x = r14.w * r7.w - 2.0;
    r0.xyz = r14.xyz * r7.www + r13.xyz;
    ps = UniformVector_5.z * r6.w;
    r1.xyz = r0.xyz * r12.xyz;
    r11.w = ps;
    r0 = r11.yzwx + r9.wwwz;
    r0.xyz = r10.xyz * UniformScalar_7.xxx + r0.xyz;
    r3.z = r0.w + r9.z;
    r3.xyw = r3.xyz * r1.www;
    ps = r5.x * r5.x;
    r3.z = r3.w + 1.0;
    r6.w = ps;
    r3.xyz = (r1.www > 0.0) ? r3.xyz : float3(0.0, 0.0, 1.0);
    r3.xyz = (r1.www >= 0.0) ? r3.xyz : float3(0.0, 0.0, 1.0);
    ps = r5.y * r5.y;
    r6.z = dot(r3.zxy, r3.zxy);
    r6.y = ps;
    ps = rsqrt(abs(r6.z));
    r0.xyz = r6.www * r0.xyz;
    r6.z = ps;
    r3.xyz = r3.xyz * r6.zzz;
    r6.z = dot(r3.zxy, r8.zxy);
    r5.xyz = r3.xyz * r6.zzz;
    r5.xyz = r5.xyz * 2.0 - r8.xyz;
    r6.z = saturate(dot(r2.zxy, r5.zxy));
    ps = log2(r6.z);
    r0.xyz = r0.xyz * r7.www;
    r6.z = ps;
    ps = LightColorAndFalloffExponent.w * r6.x;
    r6.w = r6.z * 15.0;
    r6.x = ps;
    ps = pow(2.0, r6.w);
    r6.z = saturate(dot(r3.zyx, r2.zyx));
    r6.w = ps;
    ps = pow(2.0, r6.x);
    r0.xyz = r0.xyz * r6.www;
    r6.x = ps;
    r0.xy = r1.xy * r6.zz + r0.xy;
    r0.z = r1.z * r6.z + r0.z;
    r6.xzw = r0.xyz * r6.xxx;
    r6.xzw = r6.xzw * r4.xyz;
    r6.xzw = r6.xzw * LightColorAndFalloffExponent.xyz;
    r6.xyz = r6.xwz * r6.yyy;
    r6.xyz = r6.xzy * r7.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
