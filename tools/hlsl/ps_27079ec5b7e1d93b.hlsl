// ps_27079ec5b7e1d93b.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 393 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000624 10041300 0000080A 00000000 00006908 003F00FF 00000001 00003050 00003151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c15); // float4
float4 ModShadowAccumResolution : register(c20); // float2
float4 ModShadowColor : register(c18); // float3
float4 ModShadowGroupColor : register(c19); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c17); // float2
float4 SpotDirection : register(c16); // float3
float4 UniformScalar_0 : register(c10); // float
float4 UniformScalar_1 : register(c11); // float
float4 UniformScalar_5 : register(c12); // float
float4 UniformScalar_6 : register(c13); // float
float4 UniformScalar_7 : register(c14); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UniformVector_7 : register(c9); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_8 : register(s8);
sampler2D Texture2D_9 : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r10 = r1.xyxy * float4(6.0, 6.0, 0.5, 0.5);
    r9.xyw = tex2D(Texture2D_2, r10.xy).xyw;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.y = -r1.y + 1.0;
    r7.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.x = dot(r4.zxy, r4.zxy);
    r7.z = ps;
    ps = 1.0 / UniformVector_1.x;
    r6.y = r5.w - 4e+02;
    r6.z = ps;
    r11.yw = r9.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r6.x));
    r13.zw = r7.xz * abs(r8.xy);
    r6.w = ps;
    ps = r2.z;
    r13.xy = r0.xy * UniformScalar_0.xx;
    ps = 0.1 + ps;
    r0.zw = r1.xy * UniformVector_7.xy;
    r6.x = ps;
    ps = 5.0 * r6.x;
    r12.zw = r5.xy * ScreenPositionScaleBias.xy;
    r4.w = saturate(ps);
    ps = 0.00022222222 * r6.y;
    r7.xz = r1.xy * UniformVector_1.xy;
    r11.x = saturate(ps);
    ps = 1.0 / r5.w;
    r9.xyz = r6.www * r4.xyz;
    r6.y = ps;
    ps = 1.0 / UniformVector_1.y;
    r15.xyw = r11.ywx * float3(0.5, 0.5, -0.025);
    r6.x = ps;
    r12.xy = r9.xy * r15.ww + r7.xz;
    ps = UniformVector_2.x * r6.z;
    r4.xyz = r12.wyz * r6.yxy;
    r6.x = ps;
    ps = r6.x;
    r7.xz = -r4.yw + 1.0;
    ps = r12.x * ps;
    r11.yw = r7.xy * UniformVector_2.yy;
    r11.x = ps;
    ps = 0.0001 * r5.w;
    r11.z = r1.x * UniformVector_2.x;
    r15.w = saturate(ps);
    r6 = r11 - 0.5;
    r18 = float4((r11.xywz >= 0.5));
    ps = ScreenPositionScaleBias.w + r4.z;
    r6 = r6.xywz - r11.xywz;
    r7.x = ps;
    r6 = r6.xywz * r18.xywz + r11;
    ps = ScreenPositionScaleBias.z + r4.x;
    r6 = r6 + r6;
    r7.y = ps;
    r8.xyz = tex2D(LightAttenuationTexture, r7.xy).xyz;
    r11.xyz = tex2D(Texture2D_8, r12.xy).xyz;
    r4.zw = tex2D(Texture2D_3, r10.zw).xy;
    r16 = tex2D(Texture2D_1, r6.xy);
    r17 = tex2D(Texture2D_1, r6.zw);
    r10.xyz = tex2D(Texture2D_9, r0.zw).xyz;
    r19.xyz = tex2D(Texture2D_5, r0.xy).xyz;
    r0 = tex2D(Texture2D_6, r0.xy).xwyz;
    r1.xyz = tex2D(Texture2D_2, r1.xy).xwy;
    r12.xyz = tex2D(Texture2D_0, r12.xy).xyz;
    r7.xy = tex2D(ModShadowAccumTexture, r13.zw).xy;
    r6.yzw = tex2D(Texture2D_4, r13.xy).xyw;
    ps = UniformVector_6.x * UniformVector_6.w;
    r7.w = dot(r2.zxy, r2.zxy);
    r13.x = ps;
    ps = UniformVector_6.y * UniformVector_6.w;
    r1.w = float((UniformScalar_5.x >= 1.0));
    r13.y = ps;
    ps = UniformVector_6.z * UniformVector_6.w;
    r6.x = dot(r3.zxy, r3.zxy);
    r13.z = ps;
    r5.xyz = r12.xyz * 2.0 - 1.0;
    ps = 1.0 - r6.x;
    r14.zw = r6.yz + r6.yz;
    r6.y = saturate(ps);
    ps = r1.x + r1.x;
    r6.z = float((UniformScalar_5.x > 1.0));
    r14.x = ps;
    ps = r1.z + r1.z;
    r13.xyz = r13.xyz * r0.xzw;
    r14.y = ps;
    r0.xzw = r19.zxy * 2.0 - 1.0;
    r10.xyz = (-abs(r6.zzz) >= 0.0) ? r10.xyz : 1.0;
    ps = rsqrt(abs(r6.x));
    r0.y = float((r0.y >= 0.5));
    r6.x = ps;
    r10.yzw = (-abs(r1.www) >= 0.0) ? 1.0 : r10.xyz;
    ps = log2(r6.y);
    r19.xyz = r6.xxx * -SpotDirection.xyz;
    r6.y = ps;
    r14 = r14 - 1.0;
    ps = (-1.0) + r6.w;
    r6.z = dot(r19.zxy, r3.zxy);
    r10.x = ps;
    ps = r9.w;
    r1.xz = r14.xy + r15.xy;
    ps = r1.y * ps;
    r3.x = dot(r18.xyy, float3(1.0, 1.0, 1.0));
    r6.x = ps;
    ps = UniformVector_4.z * r0.x;
    r3.y = dot(r18.wzz, float3(1.0, 1.0, 1.0));
    r1.y = ps;
    r1.w = (r3.y == 0.0) ? r17.x : r17.y;
    r6.w = (r3.x == 0.0) ? r16.x : r16.y;
    ps = ModShadowGroupColor.x * r7.z;
    r3 = r3.xxyy + float4(-3.0, -2.0, -2.0, -3.0);
    r15.x = ps;
    r6.w = (r3.y == 0.0) ? r16.z : r6.w;
    r1.w = (r3.z == 0.0) ? r17.z : r1.w;
    r15.z = (r3.w == 0.0) ? r17.w : r1.w;
    r4.x = (r3.x == 0.0) ? r16.w : r6.w;
    ps = ModShadowGroupColor.y * r7.z;
    r6.w = max(r15.z, 0.0);
    r15.y = ps;
    ps = 0.25 + r4.x;
    r3 = -r15.wxyz + 1.0;
    r4.y = ps;
    ps = r3.x;
    r15.x = min(r6.w, 0.3);
    r6.w = ps;
    ps = UniformScalar_1.x * r6.w;
    r4.xyw = r4.zwy + r4.zwx;
    r2.w = ps;
    r14.xy = r2.ww * r14.zw + r0.zw;
    ps = (-1.0) - -r6.x;
    r15.yz = r4.xy - 1.0;
    r1.w = ps;
    ps = r3.w;
    r0.xzw = r15.xyz * float3(3.3333333, 0.012, 0.012);
    r7.z = ps;
    ps = 2.5 * r7.z;
    r1.xzw = r0.xxx * r1.wxz;
    r7.z = ps;
    r14.xy = r14.xy * UniformVector_4.xy - r0.zw;
    r0.xz = r14.xy * r0.yy + r0.zw;
    ps = r1.y;
    r4.xyz = r13.xyz * r10.yzw;
    r6.w = ps;
    ps = (-1.0) + r6.w;
    r0.zw = r0.xz + r1.zw;
    r0.x = ps;
    ps = 1.0 + r1.x;
    r10.yz = -r5.xy + r0.zw;
    r0.w = ps;
    r6.w = saturate(r7.z * r0.w - 0.5);
    r10.yz = r10.yz * r6.ww + r5.xy;
    r10.xyw = r10.yzx * r3.xxx;
    ps = r4.w;
    r7.z = r10.w + 1.0;
    r7.z = (r2.w > 0.0) ? r7.z : 1.0;
    r0.z = (r2.w >= 0.0) ? r7.z : 1.0;
    ps = -r6.x + ps;
    r1.xy = r0.yx * r0.zy;
    r6.x = saturate(ps);
    ps = -r5.z;
    r14.xyz = r6.xxx * r11.xyz;
    ps = r1.y + ps;
    r4.xyw = r4.xyz * r1.xxx;
    r6.x = ps;
    r1.yzw = r4.xyw * r0.www - r14.xyz;
    ps = 1.0 + r6.x;
    r4.z = -UniformVector_0.z + 1.0;
    r1.x = ps;
    ps = 0.875 * r7.x;
    r13 = r1 * r6.wwww;
    r11.y = ps;
    ps = rsqrt(abs(r7.w));
    r5.z = r14.z + r13.w;
    r6.x = ps;
    ps = 0.875 * r7.y;
    r11.xw = -UniformVector_0.xy + 1.0;
    r11.z = ps;
    ps = UniformScalar_6.x * r0.y;
    r5.xyw = -ModShadowColor.xyz + 1.0;
    r12.w = ps;
    ps = 1.0 - r0.y;
    r1.xyz = r6.xxx * r2.xyz;
    r6.x = ps;
    ps = -SpotAngles.x - -r6.z;
    r7.yzw = r6.xxx * UniformVector_5.xyz;
    r6.z = ps;
    ps = r13.x;
    r2.xy = r14.xy + r13.yz;
    r6.x = ps;
    ps = (-2.0) + r6.x;
    r2.xy = r2.xy * r11.xw;
    r7.x = ps;
    ps = SpotAngles.y * r6.z;
    r13 = r7 + r12.zwww;
    r7.x = saturate(ps);
    r4.xyw = r4.xyw * UniformScalar_7.xxx + r13.yzw;
    ps = abs(r0.w) * abs(r0.w);
    r11.x = r13.x + r12.z;
    r7.y = ps;
    r7.zw = r11.yz * r3.yz + 0.125;
    r10.z = r11.x * r3.x + 1.0;
    r0.xyz = (r3.xxx > 0.0) ? r10.xyz : float3(0.0, 0.0, 1.0);
    r7.xyz = r7.zxy * r7.wxy;
    r6.xzw = r7.xxx * r5.xyw + ModShadowColor.xyz;
    r0.xyz = (r3.xxx >= 0.0) ? r0.xyz : float3(0.0, 0.0, 1.0);
    ps = r7.z * r7.z;
    r7.w = dot(r0.zxy, r0.zxy);
    r5.w = ps;
    ps = rsqrt(abs(r7.w));
    r5.xy = r5.ww * r4.xy;
    r7.w = ps;
    r3.xyz = r0.xyz * r7.www;
    r0 = r5.wxyz * r4.wxyz;
    r7.w = dot(r3.zxy, r9.zxy);
    r5.xyz = r3.xyz * r7.www;
    r5.xyz = r5.xyz * 2.0 - r9.xyz;
    r7.w = saturate(dot(r1.zxy, r5.zxy));
    ps = log2(r7.w);
    r2.z = r0.x * r4.w;
    r7.w = ps;
    ps = LightColorAndFalloffExponent.w * r6.y;
    r7.w = r7.w * 15.0;
    r6.y = ps;
    ps = pow(2.0, r7.w);
    r1.y = saturate(dot(r3.zyx, r1.zyx));
    r1.x = ps;
    ps = pow(2.0, r6.y);
    r0.xyz = r0.yzw * r1.xxy;
    r6.y = ps;
    r0.xyz = r2.xyz * r1.yyx + r0.xyz;
    r0.xyz = r0.xyz * r6.yyy;
    r0.xyz = r0.xyz * r8.xyz;
    r0.xyz = r0.xyz * LightColorAndFalloffExponent.xyz;
    r7.xyz = r0.xzy * r7.yyy;
    r6.xyz = r7.xzy * r6.xzw;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
