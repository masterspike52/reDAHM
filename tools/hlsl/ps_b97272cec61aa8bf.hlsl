// ps_b97272cec61aa8bf.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 360 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000005A0 10041200 0000080A 00000000 00007908 001F00FF 00000001 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A1 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR1 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c15); // float3
float4 ModShadowAccumResolution : register(c18); // float2
float4 ModShadowColor : register(c16); // float3
float4 ModShadowGroupColor : register(c17); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c10); // float
float4 UniformScalar_1 : register(c11); // float
float4 UniformScalar_5 : register(c12); // float
float4 UniformScalar_6 : register(c13); // float
float4 UniformScalar_7 : register(c14); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UniformVector_7 : register(c9); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
sampler2D Texture2D_8 : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color1 : COLOR1; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color1;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r2.y = -r0.z + 1.0;
    r5.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r2.z = r0.w * UniformVector_2.x;
    r5.y = ps;
    r5.xy = r5.xy * abs(r8.xy);
    r8.yz = tex2D(ModShadowAccumTexture, r5.xy).xy;
    ps = r4.w;
    r5.xz = r0.xy * UniformScalar_0.xx;
    ps = (-4e+02) + ps;
    r6.xy = r0.wz * 6.0;
    r6.w = ps;
    ps = 1.0 / UniformVector_1.x;
    r7.xy = r0.wz * UniformVector_1.xy;
    r5.y = ps;
    ps = UniformVector_2.x * r5.y;
    r5.w = dot(r3.zxy, r3.zxy);
    r6.z = ps;
    ps = rsqrt(abs(r5.w));
    r8.x = saturate(r6.w * 0.00022222222);
    r5.y = ps;
    ps = 1.0 / UniformVector_1.y;
    r3.xyz = r5.yyy * r3.xyz;
    r5.y = ps;
    ps = 1.0 / r4.w;
    r18.xyw = r8.yzx * float3(0.875, 0.875, -0.025);
    r5.w = ps;
    r7.xy = r3.xy * r18.ww + r7.xy;
    ps = r6.z;
    r7.zw = r4.xy * ScreenPositionScaleBias.xy;
    r6.zw = r7.zw * r5.ww + ScreenPositionScaleBias.wz;
    r2.x = -r7.y * r5.y + 1.0;
    ps = r7.x * ps;
    r2.yw = r2.xy * UniformVector_2.yy;
    r2.x = ps;
    ps = r0.w;
    r8 = r2 - 0.5;
    ps = UniformVector_7.x * ps;
    r12 = float4((r2.xywz >= 0.5));
    r5.y = ps;
    ps = r0.z;
    r8 = r8.xywz - r2.xywz;
    r2 = r8.xywz * r12.xywz + r2;
    ps = UniformVector_7.y * ps;
    r2 = r2 + r2;
    r5.w = ps;
    r11.xyz = tex2D(Texture2D_7, r7.xy).xyz;
    r9.xyz = tex2D(LightAttenuationTexture, r6.zw).xyz;
    r10 = tex2D(Texture2D_1, r2.xy);
    r14 = tex2D(Texture2D_1, r2.zw);
    r16.xyz = tex2D(Texture2D_8, r5.yw).xyz;
    r8.yzw = tex2D(Texture2D_0, r7.xy).xyz;
    r17.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r7.xyw = tex2D(Texture2D_2, r0.wz).xwy;
    r2.xyz = tex2D(Texture2D_2, r6.xy).xyw;
    r13 = tex2D(Texture2D_6, r0.xy);
    r6.xyz = tex2D(Texture2D_3, r5.xz).xwy;
    ps = r1.z;
    r5.y = float((UniformScalar_5.x >= 1.0));
    ps = 0.1 + ps;
    r6.w = float((UniformScalar_5.x > 1.0));
    r5.x = ps;
    r15.yz = r2.xy * 2.0 - 1.0;
    r2.xyw = r17.xyz * 2.0 - 1.0;
    ps = 5.0 * r5.x;
    r5.w = r2.z * r7.y;
    r5.z = saturate(ps);
    r0.xyz = (-abs(r6.www) >= 0.0) ? r16.xyz : 1.0;
    r16.xyz = (-abs(r5.yyy) >= 0.0) ? 1.0 : r0.xzy;
    ps = r8.y + r8.y;
    r0.z = dot(r12.xyy, float3(1.0, 1.0, 1.0));
    r0.x = ps;
    ps = r8.z + r8.z;
    r0.w = dot(r12.wzz, float3(1.0, 1.0, 1.0));
    r0.y = ps;
    r5.x = (r0.w == 0.0) ? r14.x : r14.y;
    r5.y = (r0.z == 0.0) ? r10.x : r10.y;
    ps = r8.w + r8.w;
    r12 = r0.zzww + float4(-3.0, -2.0, -2.0, -3.0);
    r0.z = ps;
    r5.y = (r12.y == 0.0) ? r10.z : r5.y;
    r5.x = (r12.z == 0.0) ? r14.z : r5.x;
    r5.x = (r12.w == 0.0) ? r14.w : r5.x;
    r5.y = (r12.x == 0.0) ? r10.w : r5.y;
    r0.w = r5.y * 2.0 - r5.w;
    ps = r4.w;
    r12 = r0 + float4(-1.0, -1.0, -1.0, 1.25);
    r11.w = r2.w * UniformVector_4.z - r12.z;
    ps = 0.0001 * ps;
    r2.w = saturate(r12.w - r5.w);
    r0.z = saturate(ps);
    ps = r6.x + r6.x;
    r14.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r17.x = ps;
    ps = r6.z + r6.z;
    r10.xyz = -UniformVector_0.xyz + 1.0;
    r17.y = ps;
    ps = r7.x + r7.x;
    r4.xyz = -ModShadowColor.xyz + 1.0;
    r17.z = ps;
    ps = r13.w;
    r5.y = dot(r1.zxy, r1.zxy);
    r6.x = ps;
    ps = 1.0 - r6.x;
    r7.xyz = r9.xyz * LightColor.xyz;
    r6.w = ps;
    ps = r7.w + r7.w;
    r9.xyz = r6.www * UniformVector_5.xyz;
    r17.w = ps;
    ps = 1.0 - r5.z;
    r17 = r17 - 1.0;
    r5.z = ps;
    ps = 1.0 - r5.x;
    r6.w = max(r5.x, 0.0);
    r5.x = ps;
    ps = ModShadowGroupColor.x * r5.z;
    r15.x = min(r6.w, 0.3);
    r0.x = ps;
    ps = ModShadowGroupColor.y * r5.z;
    r15.xyz = r15.xyz * float3(3.3333333, 0.5, 0.5);
    r0.y = ps;
    ps = (-1.0) + r6.y;
    r0.xyw = -r0.xyz + 1.0;
    r18.z = ps;
    ps = (-1.0) - -r5.w;
    r6.yz = r17.zw + r15.yz;
    r6.x = ps;
    r15.yzw = r18.xyz * r0.xyw + float3(0.125, 0.125, 1.0);
    ps = rsqrt(abs(r5.y));
    r0.z = r0.w * UniformScalar_1.x;
    r5.y = ps;
    r5.zw = r0.zz * r17.xy + r2.xy;
    r6.w = r15.x * r6.x + 1.0;
    r7.w = (r0.z > 0.0) ? r15.w : 1.0;
    r14.w = (r0.z >= 0.0) ? r7.w : 1.0;
    ps = abs(r6.w) * abs(r6.w);
    r2.xyz = r5.yyy * r1.xyz;
    r5.y = ps;
    r5.zw = r5.zw * UniformVector_4.xy - r12.xy;
    r5.zw = r15.xx * r6.yz + r5.zw;
    ps = r13.w;
    r15.x = r5.y * r5.y;
    r5.y = ps;
    ps = r15.x * r15.x;
    r1 = r14 * r13;
    r7.w = ps;
    ps = r15.y * r15.z;
    r0.xyz = r1.xzy * r16.xyz;
    r6.x = ps;
    r6.xyz = r6.xxx * r4.xyz + ModShadowColor.xyz;
    ps = r0.x;
    r1.xyz = r2.www * r11.xyz;
    ps = r14.w * ps;
    r4.yz = r0.zy * r1.ww;
    r0.z = ps;
    ps = 2.5 * r5.x;
    r4.x = r0.z * r13.w;
    r4.w = ps;
    r11.xyz = r4.xyz * r6.www - r1.xyz;
    r6.w = saturate(r4.w * r6.w - 0.5);
    r5.xz = r5.zw * r6.ww + r12.xy;
    r9.w = r11.w * r6.w - 2.0;
    r1.xyz = r11.xyz * r6.www + r1.xyz;
    ps = UniformScalar_6.x * r5.y;
    r0.xyz = r1.xyz * r10.xyz;
    r8.x = ps;
    r1 = r9 + r8.xxxw;
    r1.xyz = r4.xyz * UniformScalar_7.xxx + r1.xyz;
    r5.w = r1.w + r8.w;
    r5.xyz = r5.wxz * r0.www;
    ps = 1.0 + r5.x;
    r5.w = ps;
    r5.xyz = (r0.www > 0.0) ? r5.yzw : float3(0.0, 0.0, 1.0);
    r5.yzw = (r0.www >= 0.0) ? r5.xyz : float3(0.0, 0.0, 1.0);
    r5.x = dot(r5.wyz, r5.wyz);
    ps = rsqrt(abs(r5.x));
    r1.xyz = r7.www * r1.xyz;
    r5.x = ps;
    r5.yzw = r5.yzw * r5.xxx;
    r5.x = dot(r5.wyz, r3.zxy);
    r4.xyz = r5.yzw * r5.xxx;
    r3.xyz = r4.xyz * 2.0 - r3.xyz;
    r5.x = saturate(dot(r2.zxy, r3.zxy));
    ps = log2(r5.x);
    r1.xyz = r1.xyz * r6.www;
    r5.x = ps;
    ps = 15.0 * r5.x;
    r5.x = ps;
    ps = pow(2.0, r5.x);
    r5.w = saturate(dot(r5.wzy, r2.zyx));
    r5.x = ps;
    r5.xyz = r1.xzy * r5.xxx;
    r5.y = r0.z * r5.w + r5.y;
    r5.xz = r0.xy * r5.ww + r5.xz;
    r5.xyz = r7.xzy * r5.xyz;
    r5.xyz = r5.xyz * r6.xzy;
    oC0.xyz = r5.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
