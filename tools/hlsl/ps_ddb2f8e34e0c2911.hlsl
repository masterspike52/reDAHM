// ps_ddb2f8e34e0c2911.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 189 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002F4 10040B00 0000070A 00000000 000068E7 001F007F 00000001 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c13); // float3
float4 ModShadowAccumResolution : register(c16); // float2
float4 ModShadowColor : register(c14); // float3
float4 ModShadowGroupColor : register(c15); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c8); // float
float4 UniformScalar_1 : register(c9); // float
float4 UniformScalar_5 : register(c10); // float
float4 UniformScalar_6 : register(c11); // float
float4 UniformScalar_7 : register(c12); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_2 : register(c4); // float4
float4 UniformVector_3 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r5.xy = r0.xy * UniformScalar_0.xx;
    r5.xyw = tex2D(Texture2D_0, r5.xy).xyw;
    r6.xy = r0.wz * UniformVector_3.xy;
    r8.yz = r4.xy * ScreenPositionScaleBias.xy;
    ps = r4.w;
    r5.z = r1.z + 0.1;
    r10.xy = r5.xy * 2.0 - 1.0;
    ps = 0.0001 * ps;
    r5.x = saturate(r5.z * 5.0);
    r5.z = saturate(ps);
    ps = 1.0 / r4.w;
    r5.x = -r5.x + 1.0;
    r10.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r5.xy = r5.xx * ModShadowGroupColor.xy;
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r5.xyz = -r5.xzy + 1.0;
    r6.w = ps;
    ps = UniformScalar_1.x * r5.y;
    r6.zw = r6.zw * abs(r7.xy);
    r8.x = ps;
    r4.xy = r8.yz * r10.ww + ScreenPositionScaleBias.wz;
    r2.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r9.yz = tex2D(ModShadowAccumTexture, r6.zw).xy;
    r6.xyz = tex2D(Texture2D_2, r6.xy).xyz;
    r11.xyz = tex2D(LightAttenuationTexture, r4.xy).xyz;
    r0.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r7.xyz = -ModShadowColor.xyz + 1.0;
    r0.w = dot(r3.zxy, r3.zxy);
    r4.xyz = UniformVector_4.xyz * UniformVector_4.www;
    r6.w = dot(r1.zxy, r1.zxy);
    r10.z = r0.z * 2.0 - 1.0;
    r9.xw = r0.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r6.w));
    r0.xyz = r11.xzy * LightColor.xzy;
    r6.w = ps;
    r11.xyz = r4.xzy * UniformVector_5.xzy - r4.xzy;
    r4.yzw = r11.xyz * r6.xxx + r4.xzy;
    r1.yzw = r6.www * r1.xyz;
    r10.xy = r8.xx * r10.xy + r9.xw;
    r11.xyz = r10.xyz * UniformVector_2.xyz;
    ps = rsqrt(abs(r0.w));
    r6.w = dot(r11.zxy, r11.zxy);
    r0.w = ps;
    ps = rsqrt(abs(r6.w));
    r10.xyz = r0.www * r3.xyz;
    r6.w = ps;
    r3.xyz = r11.xyz * r6.www;
    r6.w = dot(r3.zxy, r10.zxy);
    ps = -UniformVector_0.x;
    r11.xyz = r3.xzy * r6.www;
    r10.xyz = r11.xyz * 2.0 - r10.xzy;
    ps = 1.0 + ps;
    r0.w = saturate(dot(r1.wyz, r10.yxz));
    r1.x = ps;
    ps = log2(r0.w);
    r6.w = saturate(dot(r3.zxy, r1.wyz));
    r9.x = ps;
    ps = (-1.0) + r5.w;
    r3.yzw = r9.xyz * float3(15.0, 0.875, 0.875);
    r3.x = ps;
    r5.xyw = r3.zwx * r5.xzy + float3(0.125, 0.125, 1.0);
    r5.w = (r8.x > 0.0) ? r5.w : 1.0;
    ps = -UniformVector_0.y;
    r5.x = r5.x * r5.y;
    r5.xyz = r5.xxx * r7.xyz + ModShadowColor.xyz;
    r4.x = (r8.x >= 0.0) ? r5.w : 1.0;
    ps = 1.0 + ps;
    r6.xyz = r4.xwz * r6.xyz;
    r1.y = ps;
    ps = -UniformVector_0.z;
    r6.xyz = r6.xyz * r4.yxx;
    ps = 1.0 + ps;
    r6.xyz = r6.xyz * UniformScalar_5.xxx;
    r1.z = ps;
    r1.xyz = r6.xyz * r1.xyz;
    ps = pow(2.0, r3.y);
    r2.xyz = -r6.xyz + r2.xyz;
    r5.w = ps;
    r6.xyz = r2.xyz * UniformScalar_5.xxx + r6.xyz;
    r6.xyz = r6.xyz * UniformScalar_6.xxx + UniformScalar_7.xxx;
    r6.xyz = r6.xyz * r5.www;
    r6.xy = r1.xy * r6.ww + r6.xy;
    r6.z = r1.z * r6.w + r6.z;
    r6.xyz = r0.xzy * r6.xyz;
    r5.xyz = r6.xzy * r5.xzy;
    oC0.xyz = r5.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
