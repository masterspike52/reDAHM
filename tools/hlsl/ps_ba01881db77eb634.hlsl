// ps_ba01881db77eb634.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 126 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000001F8 10040900 00000406 00000000 00004084 000F000F 00000001 0000F054 0000F156 0000F257 0000F358
//   interpolator: r0 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD8 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c7); // float4
float4 ConstantLighting : register(c8); // float3
float4 LowerSkyColor : register(c6); // float3
float4 ModShadowAccumResolution : register(c11); // float2
float4 ModShadowColor : register(c9); // float3
float4 ModShadowGroupColor : register(c10); // float3
float4 OpacityOverride : register(c4); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UpperSkyColor : register(c5); // float3
sampler2D ModShadowAccumTexture : register(s0);

struct PS_INPUT
{
    float4 texcoord4 : TEXCOORD4; // r0
    float4 texcoord6 : TEXCOORD6; // r1
    float4 texcoord7 : TEXCOORD7; // r2
    float4 texcoord8 : TEXCOORD8; // r3
    float2 vPos : VPOS;   // r4 (pixel parameters)
    float vFace : VFACE;  // r4
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord4;
    float4 r1 = In.texcoord6;
    float4 r2 = In.texcoord7;
    float4 r3 = In.texcoord8;
    float4 r4 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r5 = 0.0;
    float4 r6 = 0.0;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r5.z = dot(r0.zxy, r0.zxy);
    r5.y = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r5.x = dot(r3.zxy, r3.zxy);
    r5.w = ps;
    r4.xy = r5.yw * abs(r4.xy);
    r9.yz = tex2D(ModShadowAccumTexture, r4.xy).xy;
    r4.w = ConstantLighting.z * 0.21952;
    r4.xyz = UniformVector_0.xyz + float3(0.2, 0.5, 1e+01);
    r6.xyz = AmbientColorAndSkyFactor.xyz * 0.21952;
    r7.xyz = -ModShadowColor.xyz + 1.0;
    ps = rsqrt(abs(r5.z));
    r5.y = dot(r2.zxy, r2.zxy);
    r5.z = ps;
    ps = rsqrt(abs(r5.y));
    r0.yzw = r5.zzz * r0.zxy;
    r0.x = ps;
    ps = OpacityOverride.x;
    r2.xyz = r0.xxx * r2.xyz;
    r8.z = saturate(ps);
    ps = rsqrt(abs(r5.x));
    r8.yw = -r2.xy;
    r9.x = ps;
    r8.x = r2.z * 2.0 - r2.z;
    ps = 0.1 - -r0.y;
    r5.yzw = -UniformVector_0.xyz + 1.0;
    r0.x = ps;
    ps = 5.0 * r0.x;
    r2 = r9.xxyz * float4(-0.5, 0.5, 0.875, 0.875);
    r0.x = saturate(ps);
    ps = 1.0 - r0.x;
    r0.y = saturate(dot(r0.zwy, r8.ywx));
    r0.x = ps;
    ps = log2(r0.y);
    r8.xy = r0.xx * ModShadowGroupColor.xy;
    r0.y = ps;
    ps = 15.0 * r0.y;
    r3.xy = r2.xy * r3.zz;
    r0.w = ps;
    ps = pow(2.0, r0.w);
    r0.xyz = -r8.xyz + 1.0;
    r5.x = ps;
    r3.zw = r2.zw * r0.xy;
    ps = r4.w;
    r0.w = float((r0.z >= 0.004));
    ps = r5.x * ps;
    r2 = r5 * 0.21952;
    r9.z = ps;
    ps = r2.x;
    r3 = r3 + float4(0.5, 0.5, 0.125, 0.125);
    r0.y = ps;
    ps = r3.z * r3.w;
    r0.xz = abs(r3.xy) * abs(r3.xy);
    r3.x = ps;
    r3.xyz = r3.xxx * r7.xyz + ModShadowColor.xyz;
    ps = ConstantLighting.x * r0.y;
    r7.xyz = r2.ywz * r0.zzz;
    r9.x = ps;
    ps = ConstantLighting.y * r2.x;
    r8.xyz = r2.ywz * r0.xxx;
    r9.y = ps;
    r0.xyz = r2.yzw * ConstantLighting.xyz + r9.xyz;
    r0.xyz = r8.xzy * LowerSkyColor.xyz + r0.xyz;
    r0.xyz = r7.xzy * UpperSkyColor.xyz + r0.xyz;
    r0.xyz = r6.xyz * r5.yzw + r0.xyz;
    r0.xyz = r4.xyz + r0.xyz;
    ps = -r1.w;
    r0.xyz = r0.xyz * r3.xyz;
    ps = OpacityOverride.x + ps;
    r2.xyz = r0.xyz - r0.xyz;
    r2.w = ps;
    oC0.w = r2.w * r0.w + r1.w;
    r0.xyz = r2.xyz * r0.www + r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
