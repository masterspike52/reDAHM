// ps_ffc8f44f3db55694.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 120 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000001E0 10040700 00000606 00000000 000058C6 003F003F 00000021 00003050 0000F151 0000F254 0000F356 0000F457 0000F558
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 ModShadowAccumResolution : register(c10); // float2
float4 ModShadowColor : register(c8); // float3
float4 ModShadowGroupColor : register(c9); // float3
float4 OpacityOverride : register(c7); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_3 : register(c6); // float4
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D ModShadowAccumTexture : register(s2);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float2 vPos : VPOS;   // r6 (pixel parameters)
    float vFace : VFACE;  // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r7 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r0.zw = r0.xy * 0.2 + UniformVector_1.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r5.xyz = -ModShadowColor.xyz + 1.0;
    r7.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.xy = r0.yx - 0.5;
    r7.w = ps;
    r6.xy = r7.zw * abs(r6.xy);
    r6.zw = tex2D(ModShadowAccumTexture, r6.xy).xy;
    r5.w = tex2D(Texture2D_0, r0.zw).x;
    r0.w = dot(r4.zxy, r4.zxy);
    r0.z = dot(r2.zxy, r2.zxy);
    r6.y = dot(r7.xy, UniformVector_3.yx) + 0.5;
    r6.x = dot(r7.xy, UniformVector_2.yx) + 0.5;
    ps = rsqrt(abs(r0.z));
    r7.z = r5.w * 0.25;
    r0.z = ps;
    ps = rsqrt(abs(r0.w));
    r6.zw = r6.zw * 0.875;
    r5.w = ps;
    r0.w = r0.z * r2.z + 0.1;
    ps = 5.0 * r0.w;
    r0.z = saturate(ps);
    ps = OpacityOverride.x;
    r0.z = -r0.z + 1.0;
    r2.x = saturate(ps);
    ps = r5.w;
    r2.yz = r0.zz * ModShadowGroupColor.xy;
    ps = r4.x * ps;
    r2.xyw = -r2.yzx + 1.0;
    r0.z = ps;
    ps = r5.w;
    r7.xy = r6.zw * r2.xy;
    ps = r4.y * ps;
    r2.xyz = r7.xyz + float3(0.125, 0.125, -0.005);
    r0.w = ps;
    r0.xy = r0.zw * r2.zz + r0.xy;
    r4.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r4.w = tex2D(Texture2D_1, r6.xy).x;
    r0 = r4.zxyw * r1.zxyx;
    r1.x = r0.w * r4.x;
    r1.x = min(r1.x, 1.0);
    ps = r0.y;
    r1.x = r1.x - 0.5;
    r0.y = ps;
    ps = UniformVector_0.x + r0.y;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r1.xxxx)) clip(-1.0);
    r1.y = ps;
    ps = UniformVector_0.y + r0.z;
    r1.x = float((r2.w >= 0.004));
    r1.z = ps;
    ps = UniformVector_0.z + r0.x;
    r2.x = r2.x * r2.y;
    r1.w = ps;
    r0.xyz = r2.xxx * r5.xyz + ModShadowColor.xyz;
    ps = -r3.w;
    r0.xyz = r1.yzw * r0.xyz;
    ps = OpacityOverride.x + ps;
    r2.xyz = r0.xyz - r0.xyz;
    r2.w = ps;
    oC0.w = r2.w * r1.x + r3.w;
    r0.xyz = r2.xyz * r1.xxx + r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
