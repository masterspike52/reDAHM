// ps_5b5682d5a51c6500.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 165 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000294 10040D00 0000080A 00000000 00007908 003F00FF 00000001 00003050 0000F151 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c8); // float4
float4 LightMapScale : register(c9); // float3
float4 LowerSkyColor : register(c7); // float3
float4 ModShadowAccumResolution : register(c12); // float2
float4 ModShadowColor : register(c10); // float3
float4 ModShadowGroupColor : register(c11); // float3
float4 OpacityOverride : register(c5); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UpperSkyColor : register(c6); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D LightMapTexture : register(s3);
sampler2D ModShadowAccumTexture : register(s4);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r6.xy = r1.xy * 0.1 + UniformVector_1.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.w = ps;
    r0.zw = r6.zw * abs(r8.xy);
    r11.xyz = tex2D(Texture2D_0, r1.xy).xyz;
    r10.xyz = tex2D(Texture2D_1, r6.xy).xyz;
    r7.xyz = tex2D(Texture2D_2, r1.xy).zxy;
    r6.xyw = tex2D(LightMapTexture, r0.xy).yxz;
    r0.xy = tex2D(ModShadowAccumTexture, r0.zw).xy;
    ps = -ModShadowColor.x;
    r7.w = dot(r5.zxy, r5.zxy);
    ps = 1.0 + ps;
    r6.z = dot(r4.zxy, r4.zxy);
    r8.x = ps;
    ps = -ModShadowColor.y;
    r0.w = dot(r2.zxy, r2.zxy);
    ps = 1.0 + ps;
    r9.xyz = -UniformVector_0.xyz + 1.0;
    r8.y = ps;
    ps = OpacityOverride.x;
    r8.zw = r0.xy * 0.875;
    r1.w = saturate(ps);
    ps = LightMapScale.x * r6.y;
    r0.xyz = r9.xyz * r7.yzx;
    r1.x = ps;
    ps = AmbientColorAndSkyFactor.x * r7.y;
    r12.xyz = r10.xyz + r10.xyz;
    r10.x = ps;
    ps = rsqrt(abs(r0.w));
    r13.zw = r12.yz * r11.yz;
    r6.y = ps;
    ps = rsqrt(abs(r6.z));
    r2.xyw = r6.yyy * r2.xyz;
    r6.y = ps;
    ps = AmbientColorAndSkyFactor.y * r7.z;
    r4.xyz = r6.yyy * r4.xyz;
    r10.y = ps;
    r0.w = r4.z * 2.0 - r4.z;
    ps = AmbientColorAndSkyFactor.z * r7.x;
    r6.y = r2.w + 0.1;
    r10.z = ps;
    ps = 5.0 * r6.y;
    r13.xy = r2.xy * -r4.xy;
    r6.y = saturate(ps);
    ps = 1.0 - r6.y;
    r2.xyz = r13.xzw + r13.yzw;
    r6.z = ps;
    r0.w = saturate(r2.w * r0.w + r2.x);
    ps = ModShadowGroupColor.x * r6.z;
    r2.x = dot(r12.xxxx, r11.xxxx);
    r1.y = ps;
    r2.y = dot(r12.yy, r11.yy) + r2.y;
    r2.z = dot(r12.zz, r11.zz) + r2.z;
    ps = ModShadowGroupColor.y * r6.z;
    r2.xyz = r2.xyz + UniformVector_0.xyz;
    r1.z = ps;
    ps = rsqrt(abs(r7.w));
    r1.yzw = -r1.wyz + 1.0;
    r4.y = ps;
    ps = log2(r0.w);
    r7.w = float((r1.y >= 0.004));
    r4.x = ps;
    r2.xyz = r10.xyz * r9.xyz + r2.xyz;
    ps = LightMapScale.y * r6.x;
    r4.xyz = r4.xyy * float3(15.0, -0.5, 0.5);
    r1.y = ps;
    ps = LightMapScale.z * r6.w;
    r9.zw = r8.zw * r1.zw;
    r1.z = ps;
    ps = pow(2.0, r4.x);
    r9.xy = r4.yz * r5.zz;
    r6.x = ps;
    ps = -ModShadowColor.z;
    r4.xyz = r1.xzy * r6.xxx;
    ps = 1.0 + ps;
    r6 = r9 + float4(0.5, 0.5, 0.125, 0.125);
    r8.z = ps;
    ps = r6.z * r6.w;
    r5.xw = abs(r6.xy) * abs(r6.xy);
    r6.x = ps;
    r6.xyz = r6.xxx * r8.xyz + ModShadowColor.xyz;
    r5.xyz = r0.xzy * r5.xxx;
    r8.xyz = r0.xzy * r5.www;
    r2.xyz = r8.xzy * UpperSkyColor.xyz + r2.xyz;
    r2.xyz = r5.xyz * LowerSkyColor.xzy + r2.xzy;
    r7.xyz = r4.xyz * r7.yxz + r2.xyz;
    r7.xyz = r1.xyz * r0.xyz + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xyz * r6.xyz;
    ps = OpacityOverride.x + ps;
    r0.xyz = r6.xyz - r6.xyz;
    r0.w = ps;
    oC0.w = r0.w * r7.w + r3.w;
    r6.xyz = r0.xyz * r7.www + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
