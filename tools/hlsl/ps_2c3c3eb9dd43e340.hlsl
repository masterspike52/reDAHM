// ps_2c3c3eb9dd43e340.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 126 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000001F8 10040900 00000606 00000000 000048C6 003F003F 00000001 00003050 00003151 00007254 0000F355 00007456 0000F557
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c4); // float3
float4 ModShadowAccumResolution : register(c7); // float2
float4 ModShadowColor : register(c5); // float3
float4 ModShadowGroupColor : register(c6); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformVector_0 : register(c3); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D ShadowTexture : register(s3);
sampler2D ModShadowAccumTexture : register(s4);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float2 vPos : VPOS;   // r6 (pixel parameters)
    float vFace : VFACE;  // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / r5.w;
    r0.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r1.zw = r0.zz * ScreenPositionScaleBias.xy;
    r0.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.xyz = -ModShadowColor.xyz + 1.0;
    r0.z = ps;
    r0.zw = r0.zw * abs(r6.xy);
    r1.zw = r1.zw * r5.xy + ScreenPositionScaleBias.wz;
    r3.xyz = tex2D(LightAttenuationTexture, r1.zw).xyz;
    r5.z = tex2D(ShadowTexture, r0.xy).x;
    r6.yz = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r0 = tex2D(Texture2D_1, r1.xy).xwzy;
    r5.xyw = tex2D(Texture2D_0, r1.xy).xyz;
    ps = r2.z;
    r2.w = dot(r4.zxy, r4.zxy);
    ps = 0.1 + ps;
    r1.x = dot(r2.zxy, r2.zxy);
    r1.y = ps;
    r9.xyz = r5.xyw * 2.0 - 1.0;
    ps = rsqrt(abs(r1.x));
    r3.w = saturate(r1.y * 5.0);
    r1.x = ps;
    ps = -UniformVector_0.x;
    r1.xyz = r1.xxx * r2.xyz;
    ps = 1.0 + ps;
    r1.w = dot(r9.zxy, r9.zxy);
    r2.x = ps;
    ps = rsqrt(abs(r2.w));
    r2.z = -r3.w + 1.0;
    r2.y = ps;
    r5.xy = -r2.zz * ModShadowGroupColor.xy + 1.0;
    ps = rsqrt(abs(r1.w));
    r8.xyz = r2.yyy * r4.xyz;
    r1.w = ps;
    ps = -UniformVector_0.y;
    r4.xyz = r9.xyz * r1.www;
    ps = 1.0 + ps;
    r1.w = dot(r4.zxy, r8.zxy);
    r2.y = ps;
    ps = -UniformVector_0.z;
    r9.xyz = r4.xzy * r1.www;
    r8.xyz = r9.xyz * 2.0 - r8.xzy;
    ps = 1.0 + ps;
    r1.w = saturate(dot(r1.zxy, r8.yxz));
    r2.z = ps;
    ps = log2(r1.w);
    r2.xyz = r2.xyz * r0.xwz;
    r6.x = ps;
    r0.xzw = r6.xyz * float3(15.0, 0.875, 0.875);
    r5.xy = r0.zw * r5.xy + 0.125;
    ps = pow(2.0, r0.x);
    r1.x = saturate(dot(r4.zxy, r1.zxy));
    r1.z = ps;
    ps = r1.z;
    r1.xyw = r2.xyz * r1.xxx;
    ps = r0.y * ps;
    r0.xw = r5.xz * r5.yz;
    r1.z = ps;
    r0.xyz = r0.xxx * r7.xyz + ModShadowColor.xyz;
    r1.xyz = r1.xyz + r1.zzw;
    r2.xyz = r0.www * r3.xyz;
    r1.xyz = r2.xyz * r1.xyz;
    r1.xyz = r1.xyz * LightColor.xyz;
    r0.xyz = r1.xyz * r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
