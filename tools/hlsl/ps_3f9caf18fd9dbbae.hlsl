// ps_3f9caf18fd9dbbae.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 429 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000006B4 10041600 0000090A 00000000 00008929 003F01FF 00000021 00003050 0000F151 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c26); // float4
float4 LightMapScale : register(c27); // float3
float4 LocalToWorldMatrix[3] : register(c3); // float3x3 (matrix_columns)
float4 LowerSkyColor : register(c25); // float3
float4 ModShadowAccumResolution : register(c30); // float2
float4 ModShadowColor : register(c28); // float3
float4 ModShadowGroupColor : register(c29); // float3
float4 OpacityOverride : register(c23); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_1 : register(c13); // float
float4 UniformScalar_11 : register(c21); // float
float4 UniformScalar_12 : register(c22); // float
float4 UniformScalar_2 : register(c14); // float
float4 UniformScalar_3 : register(c15); // float
float4 UniformScalar_4 : register(c16); // float
float4 UniformScalar_5 : register(c17); // float
float4 UniformScalar_6 : register(c18); // float
float4 UniformScalar_7 : register(c19); // float
float4 UniformScalar_8 : register(c20); // float
float4 UniformVector_0 : register(c6); // float4
float4 UniformVector_1 : register(c7); // float4
float4 UniformVector_2 : register(c8); // float4
float4 UniformVector_4 : register(c9); // float4
float4 UniformVector_5 : register(c10); // float4
float4 UniformVector_6 : register(c11); // float4
float4 UniformVector_7 : register(c12); // float4
float4 UpperSkyColor : register(c24); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
samplerCUBE TextureCube_0 : register(s8);
sampler2D LightMapTexture : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 r21 = 0.0;
    float4 r22 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r11 = tex2D(Texture2D_5, r1.xy);
    ps = (-0.5) + r11.w;
    r8.xy = r1.wz * 6.0;
    r7.w = ps;
    ps = UniformVector_2.x * r1.w;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r7.wwww)) clip(-1.0);
    r13.x = ps;
    r10.xyz = tex2D(Texture2D_2, r8.xy).xyw;
    ps = r3.w;
    r0.zw = r1.wz * UniformVector_7.xy;
    ps = (-4e+02) + ps;
    r7.w = dot(r4.zxy, r4.zxy);
    r8.x = ps;
    r8.yz = r10.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r7.w));
    r8.x = saturate(r8.x * 0.00022222222);
    r7.w = ps;
    ps = -r1.z;
    r17.xyz = r7.www * r4.xyz;
    ps = 1.0 + ps;
    r12.xzw = r8.xyz * float3(-0.025, 0.5, 0.5);
    r7.w = ps;
    ps = UniformVector_2.y * r7.w;
    r13.zw = r17.xy * r12.xx;
    r13.y = ps;
    ps = r1.w;
    r8.yz = r13.xy - 0.5;
    ps = UniformVector_1.x * ps;
    r21.xy = float2((r13.xy >= 0.5));
    r8.x = ps;
    ps = r1.z;
    r8.yz = r8.yz - r13.xy;
    ps = UniformVector_1.y * ps;
    r8.zw = r8.yz * r21.xy;
    r8.y = ps;
    r8 = r13.zwyx + r8.xywz;
    r4.xy = r8.wz + r8.wz;
    r20 = tex2D(Texture2D_1, r4.xy);
    r4.xyw = tex2D(Texture2D_6, r0.zw).yzx;
    r13.xyz = tex2D(Texture2D_3, r1.xy).xyz;
    r10.xyw = tex2D(Texture2D_0, r8.xy).xyz;
    r14.xyz = tex2D(Texture2D_2, r1.wz).xyw;
    r22.xyz = r7.xyz * 2.0 - 1.0;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.y = dot(r2.zxy, r2.zxy);
    r0.z = ps;
    r6 = r6.yxzw * 2.0 - 1.0;
    r19.z = r10.z * r14.z - 1.0;
    r19.xy = r14.xy * 2.0 - 1.0;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.x = dot(r6.zyx, r6.zyx);
    r0.w = ps;
    r18.xyz = r10.xyw * 2.0 - 1.0;
    r13.xyz = r13.xyz * 2.0 - 1.0;
    ps = rsqrt(abs(r7.y));
    r13.xyz = r13.xyz * UniformVector_4.xyz;
    r7.z = ps;
    ps = rsqrt(abs(r7.x));
    r13.z = -r18.z + r13.z;
    r7.y = ps;
    r14.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r7.x = dot(r22.zxy, r22.zxy);
    r14.xyz = r14.xyz * r11.xyz;
    r16.yzw = r14.xyz * r4.wxy;
    ps = OpacityOverride.x;
    r11.xyz = r7.zzz * r2.xyz;
    r7.z = saturate(ps);
    ps = rsqrt(abs(r7.x));
    r15.xyz = r6.xzy * r7.yyy;
    r7.x = ps;
    ps = r3.w;
    r10.xyz = r22.xzy * r7.xxx;
    ps = 0.0001 * ps;
    r2.xyz = r15.yzx * r10.zyx;
    r7.y = saturate(ps);
    r2.xyz = r15.xyz * r10.yxz - r2.xyz;
    ps = r17.z;
    r11.w = dot(r21.xyy, float3(1.0, 1.0, 1.0));
    r12.x = ps;
    r7.w = (r11.w == 0.0) ? r20.x : r20.y;
    ps = 0.0;
    r6.xyz = r11.zww + float3(0.1, -3.0, -2.0);
    r12.y = ps;
    ps = 5.0 * r6.x;
    r19.xy = r19.xy + r12.zw;
    r7.x = saturate(ps);
    r7.w = (r6.z == 0.0) ? r20.z : r7.w;
    r12.w = (r6.y == 0.0) ? r20.w : r7.w;
    ps = max(r12.x, r12.y);
    r7.xzw = -r7.yxz + 1.0;
    r12.z = ps;
    ps = ModShadowGroupColor.x * r7.z;
    r7.y = max(r12.w, 0.0);
    r12.x = ps;
    ps = ModShadowGroupColor.y * r7.z;
    r6.y = min(r7.y, 0.3);
    r12.y = ps;
    ps = 3.3333333 * r6.y;
    r2.yzw = r2.xyz * r6.www;
    r6.x = ps;
    r2.x = r6.x * r19.z + 1.0;
    r6.yz = r6.xx * r19.xy + r13.xy;
    r12 = -r12.wxyz + 1.0;
    ps = r12.x;
    r7.y = dot(r2.wyz, r2.wyz);
    r6.x = ps;
    ps = 2.5 * r6.x;
    r13.xy = r6.yz - r18.xy;
    r16.x = ps;
    r8.w = saturate(r16.x * r2.x - 0.5);
    r6.xy = r13.xy * r8.ww + r18.xy;
    r6.z = r13.z * r8.w - 2.0;
    r6.z = r10.w * 2.0 + r6.z;
    r6.xyz = r6.zxy * r7.xxx;
    ps = 1.0 + r6.x;
    r0.zw = r0.zw * abs(r9.xy);
    r6.w = ps;
    r6.xyz = (r7.xxx > 0.0) ? r6.yzw : float3(0.0, 0.0, 1.0);
    r6.yzw = (r7.xxx >= 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    ps = rsqrt(abs(r7.y));
    r6.x = dot(r6.wyz, r6.wyz);
    r7.x = ps;
    ps = rsqrt(abs(r6.x));
    r7.xyz = r2.ywz * r7.xxx;
    r6.x = ps;
    r13.xyz = r6.yzw * r6.xxx;
    r6.x = dot(r13.zxy, r17.zxy);
    r6.xyz = r13.xzy * r6.xxx;
    r2.yzw = r6.xyz * 2.0 - r17.xzy;
    r6.xyz = r15.zxy * r2.zzz;
    r6.xyz = r7.xzy * r2.www + r6.xyz;
    r6.xyw = r10.yzx * r2.yyy + r6.zyx;
    r7.xyz = r6.xxx * LocalToWorldMatrix[2].xyz;
    r6.xyz = r6.yyy * LocalToWorldMatrix[1].xyz + r7.xyz;
    r6.xyz = r6.www * LocalToWorldMatrix[0].xzy + r6.xzy;
    r6 = xe_cube(r6.xzy);
    ps = 1.0 / abs(r6.z);
    r7.z = r6.w;
    r7.x = ps;
    r7.xy = r6.yx * r7.xx + 1.5;
    r9.yz = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r0.w = tex2D(Texture2D_4, r1.xy).x;
    r8.xyz = tex2D(Texture2D_7, r8.xy).xyz;
    r15.xyz = texCUBE(TextureCube_0, xe_cube_dir(r7.xyz)).xyz;
    r6.xyw = tex2D(LightMapTexture, r0.xy).yxz;
    ps = UniformVector_5.x * UniformVector_5.w;
    r4.xyz = -ModShadowColor.xyz + 1.0;
    r10.y = ps;
    ps = UniformVector_5.y * UniformVector_5.w;
    r1.x = dot(r5.zxy, r5.zxy);
    r10.z = ps;
    ps = UniformVector_5.z * UniformVector_5.w;
    r6.z = UniformScalar_7.x * (-XE_FLT_MAX);
    r10.w = ps;
    ps = LightMapScale.x * r6.y;
    r7.xyz = -UniformVector_0.zxy + 1.0;
    r10.x = ps;
    ps = pow(2.0, r6.z);
    r6.y = dot(r15.zxy, float3(0.11, 0.3, 0.59));
    r6.z = ps;
    r14.xyz = r14.xyz * UniformScalar_5.xxx + UniformScalar_6.xxx;
    ps = rsqrt(abs(r1.x));
    r0.xyz = r6.yyy - r15.xyz;
    r6.y = ps;
    r1.xyz = r16.yzw * UniformScalar_11.xxx + UniformScalar_12.xxx;
    r0.xyz = r0.xyz * UniformScalar_4.xxx + r15.xyz;
    ps = log2(abs(r12.w));
    r5.xyz = r6.yyy * r5.xyz;
    r6.y = ps;
    ps = UniformScalar_1.x * r6.y;
    r15.xyz = r10.yzw * r0.xyz;
    r1.w = ps;
    r0.xyz = r16.yzw * r2.xxx - r8.xyz;
    ps = pow(2.0, r1.w);
    r7.w = float((r7.w >= 0.004));
    r1.w = ps;
    r1.w = r1.w * UniformScalar_3.x + UniformScalar_2.x;
    r0.xyz = r0.xyz * r8.www + r8.xyz;
    ps = LightMapScale.y * r6.x;
    r8.xyz = r0.xyz * r7.yzx;
    r10.y = ps;
    ps = LightMapScale.z * r6.w;
    r15.xyz = r15.xyz * r1.www;
    r10.z = ps;
    ps = UniformScalar_8.x * r6.z;
    r14.xyz = r15.xyz * r14.xyz;
    r14.w = ps;
    ps = AmbientColorAndSkyFactor.x * r7.y;
    r6 = r14 * r4.wwww;
    r14.x = ps;
    ps = AmbientColorAndSkyFactor.y * r7.z;
    r6 = r6 * r0.wwww;
    r14.y = ps;
    ps = AmbientColorAndSkyFactor.z * r7.x;
    r1.xyz = r1.xyz + r6.www;
    r14.z = ps;
    r6.xyz = r6.xyz * r8.www + UniformVector_0.xyz;
    r7.xyz = r14.xyz * r0.xyz + r6.xyz;
    r0.xyz = r10.xzy * r13.zzz;
    r9.x = dot(r5.zxy, r13.zxy);
    r5 = r9.xxyz * float4(-0.5, 0.5, 0.875, 0.875);
    r6.yz = r5.zw * r12.yz + 0.125;
    r6.x = saturate(dot(r11.zxy, r2.zyw));
    ps = log2(r6.x);
    r2.yz = r5.xy + 0.5;
    r6.x = ps;
    r2.xyw = abs(r2.yzx) * abs(r2.yzx);
    ps = 15.0 * r6.x;
    r5.xyz = r8.xzy * r2.xxx;
    r6.x = ps;
    ps = pow(2.0, r6.x);
    r9.xyz = r8.xzy * r2.yyy;
    r6.x = ps;
    ps = r2.w * r2.w;
    r2.xyz = r10.xzy * r6.xxx;
    r6.w = ps;
    r7.xyz = r9.xyz * UpperSkyColor.xzy + r7.xzy;
    r7.xyz = r5.xzy * LowerSkyColor.xyz + r7.xzy;
    r6.xw = r6.yw * r6.zw;
    r6.xyz = r6.xxx * r4.xyz + ModShadowColor.xyz;
    r1.xyz = r6.www * r1.xyz;
    r1.xyz = r1.xzy * r8.www;
    r7.xyz = r2.xyz * r1.xyz + r7.xzy;
    r7.xyz = r0.xzy * r8.xyz + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xyz * r6.xyz;
    ps = OpacityOverride.x + ps;
    r8.xyz = r6.xyz - r6.xyz;
    r8.w = ps;
    oC0.w = r8.w * r7.w + r3.w;
    r6.xyz = r8.xyz * r7.www + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
