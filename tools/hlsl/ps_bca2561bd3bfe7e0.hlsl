// ps_bca2561bd3bfe7e0.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 351 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 0000057C 10041000 0000080A 00000000 00007108 003F00FF 00000021 00003050 00003151 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A1
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c29); // float4
float4 ConstantLighting : register(c30); // float3
float4 LocalToWorldMatrix[3] : register(c3); // float3x3 (matrix_columns)
float4 LowerSkyColor : register(c28); // float3
float4 ModShadowAccumResolution : register(c33); // float2
float4 ModShadowColor : register(c31); // float3
float4 ModShadowGroupColor : register(c32); // float3
float4 OpacityOverride : register(c26); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_0 : register(c11); // float
float4 UniformScalar_1 : register(c12); // float
float4 UniformScalar_10 : register(c20); // float
float4 UniformScalar_13 : register(c21); // float
float4 UniformScalar_14 : register(c22); // float
float4 UniformScalar_15 : register(c23); // float
float4 UniformScalar_16 : register(c24); // float
float4 UniformScalar_17 : register(c25); // float
float4 UniformScalar_3 : register(c13); // float
float4 UniformScalar_4 : register(c14); // float
float4 UniformScalar_5 : register(c15); // float
float4 UniformScalar_6 : register(c16); // float
float4 UniformScalar_7 : register(c17); // float
float4 UniformScalar_8 : register(c18); // float
float4 UniformScalar_9 : register(c19); // float
float4 UniformVector_0 : register(c6); // float4
float4 UniformVector_2 : register(c7); // float4
float4 UniformVector_3 : register(c8); // float4
float4 UniformVector_4 : register(c9); // float4
float4 UniformVector_5 : register(c10); // float4
float4 UpperSkyColor : register(c27); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
samplerCUBE TextureCube_0 : register(s6);
sampler2D ModShadowAccumTexture : register(s7);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r14 = tex2D(Texture2D_3, r0.xy);
    ps = (-0.5) + r14.w;
    r0.zw = r0.xy * UniformScalar_0.xx;
    r1.z = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r1.zzzz)) clip(-1.0);
    r9.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r3.xyz = tex2D(Texture2D_0, r0.zw).xyw;
    ps = OpacityOverride.x;
    r0.w = dot(r4.zxy, r4.zxy);
    r11.x = saturate(ps);
    ps = 1.0 / ModShadowAccumResolution.x;
    r9.w = dot(r2.zxy, r2.zxy);
    r10.z = ps;
    r12 = r6.yxzw * 2.0 - 1.0;
    r11.yzw = r7.xyz * 2.0 - 1.0;
    r1.zw = r3.xy * 2.0 - 1.0;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.w = dot(r11.wyz, r11.wyz);
    r10.w = ps;
    ps = rsqrt(abs(r9.w));
    r0.z = dot(r12.zyx, r12.zyx);
    r7.x = ps;
    r6.xyz = r9.zxy * 2.0 - 1.0;
    ps = rsqrt(abs(r0.w));
    r7.xzw = r7.xxx * r2.zxy;
    r7.y = ps;
    ps = UniformVector_2.z * r6.x;
    r9.xyz = r7.yyy * r4.xyz;
    r7.y = ps;
    ps = rsqrt(abs(r0.z));
    r10.y = max(r9.z, 0.0);
    r6.x = ps;
    ps = rsqrt(abs(r6.w));
    r13.xyz = r12.xzy * r6.xxx;
    r6.x = ps;
    ps = r7.y;
    r2.xyz = r11.ywz * r6.xxx;
    r6.x = ps;
    ps = (-1.0) + r6.x;
    r4.xyz = r13.yzx * r2.zyx;
    r6.w = ps;
    r4.xyz = r13.xyz * r2.yxz - r4.xyz;
    ps = 0.1 + r7.x;
    r4.xyz = r4.xyz * r12.www;
    r6.x = ps;
    ps = 5.0 * r6.x;
    r0.z = dot(r4.zxy, r4.zxy);
    r10.x = saturate(ps);
    ps = r3.w;
    r10.xy = -r10.yx + 1.0;
    ps = 0.0001 * ps;
    r11.yz = r10.yy * ModShadowGroupColor.xy;
    r11.w = saturate(ps);
    r12 = -r11.wxyz + 1.0;
    r9.w = r12.x * UniformScalar_1.x;
    r6.yz = r9.ww * r1.zw + r6.yz;
    r6.yz = r6.yz * UniformVector_2.xy;
    ps = r1.x;
    r6.xyw = r6.yzw * r12.xxx;
    ps = UniformVector_5.x * ps;
    r6.z = r6.w + 1.0;
    r1.z = ps;
    r6.xyz = (r12.xxx > 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    r6.yzw = (r12.xxx >= 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    ps = rsqrt(abs(r0.z));
    r6.x = dot(r6.wyz, r6.wyz);
    r0.z = ps;
    ps = rsqrt(abs(r6.x));
    r11.xyz = r4.xzy * r0.zzz;
    r6.x = ps;
    ps = r1.y;
    r4.xyz = r6.yzw * r6.xxx;
    ps = UniformVector_5.y * ps;
    r6.x = dot(r4.zxy, r9.zxy);
    r1.w = ps;
    ps = r10.z;
    r6.xyz = r4.xzy * r6.xxx;
    r9.xyz = r6.xyz * 2.0 - r9.xzy;
    ps = abs(r8.x) * ps;
    r6.xyz = r13.zxy * r9.yyy;
    r0.z = ps;
    r6.xyz = r11.xzy * r9.zzz + r6.xyz;
    r6.xyw = r2.yzx * r9.xxx + r6.zyx;
    ps = r10.w;
    r2.xyz = r6.xxx * LocalToWorldMatrix[2].xyz;
    r6.xyz = r6.yyy * LocalToWorldMatrix[1].xyz + r2.xyz;
    r6.xyz = r6.www * LocalToWorldMatrix[0].xzy + r6.xzy;
    ps = abs(r8.y) * ps;
    r6 = xe_cube(r6.xzy);
    r0.w = ps;
    ps = 1.0 / abs(r6.z);
    r8.z = r6.w;
    r1.x = ps;
    r8.xy = r6.yx * r1.xx + 1.5;
    r2.x = tex2D(Texture2D_2, r0.xy).x;
    r2.yzw = tex2D(Texture2D_5, r0.xy).xyz;
    r13.xyz = texCUBE(TextureCube_0, xe_cube_dir(r8.xyz)).xyz;
    r11.xyz = tex2D(Texture2D_4, r1.zw).xyz;
    r6.xw = tex2D(ModShadowAccumTexture, r0.zw).xy;
    ps = UniformScalar_9.x;
    r10.yzw = -ModShadowColor.xyz + 1.0;
    ps = (-XE_FLT_MAX) * ps;
    r0.yzw = -UniformVector_0.xyz + 1.0;
    r6.y = ps;
    ps = UniformVector_3.x * UniformVector_3.w;
    r1.w = float((UniformScalar_14.x >= 1.0));
    r15.x = ps;
    ps = UniformVector_3.y * UniformVector_3.w;
    r4.w = float((UniformScalar_14.x > 1.0));
    r15.y = ps;
    ps = UniformVector_4.x * UniformVector_4.w;
    r6.z = float((UniformScalar_13.x >= 1.0));
    r8.x = ps;
    ps = UniformVector_4.y * UniformVector_4.w;
    r1.x = dot(r5.zxy, r5.zxy);
    r8.y = ps;
    ps = UniformVector_4.z * UniformVector_4.w;
    r0.x = float((UniformScalar_13.x > 1.0));
    r8.z = ps;
    r11.xyz = (-abs(r0.xxx) >= 0.0) ? r11.xyz : 1.0;
    ps = rsqrt(abs(r1.x));
    r0.x = dot(r13.zxy, float3(0.11, 0.3, 0.59));
    r1.x = ps;
    ps = pow(2.0, r6.y);
    r14.yzw = r8.xyz * r14.xyz;
    r6.y = ps;
    ps = UniformVector_3.z * UniformVector_3.w;
    r1.xyz = r1.xxx * r5.xyz;
    r15.z = ps;
    r8.xyz = r14.yzw * UniformScalar_7.xxx + UniformScalar_8.xxx;
    ps = (-1.0) + r3.z;
    r16.xyz = r0.xxx - r13.xyz;
    r14.x = ps;
    r11.xyz = (-abs(r6.zzz) >= 0.0) ? 1.0 : r11.xyz;
    ps = 0.875 * r6.x;
    r5.xyz = r14.yzw * r11.xyz;
    r14.y = ps;
    r13.xyz = r16.xyz * UniformScalar_6.xxx + r13.xyz;
    ps = log2(abs(r10.x));
    r13.xyz = r15.xyz * r13.xyz;
    r6.z = ps;
    ps = 0.875 * r6.w;
    r6.z = r6.z * UniformScalar_3.x;
    r14.z = ps;
    ps = pow(2.0, r6.z);
    r0.x = float((r12.y >= 0.004));
    r6.x = ps;
    r6.x = r6.x * UniformScalar_5.x + UniformScalar_4.x;
    r12.xyz = r14.xyz * r12.xzw + float3(1.0, 0.125, 0.125);
    ps = UniformScalar_10.x * r6.y;
    r6.xzw = r13.xyz * r6.xxx;
    r8.w = ps;
    r6.y = (r9.w > 0.0) ? r12.x : 1.0;
    r10.x = (r9.w >= 0.0) ? r6.y : 1.0;
    ps = r12.y * r12.z;
    r5.xyz = r5.xyz * r10.xxx;
    r11.w = ps;
    ps = r5.x;
    r8.xyz = r6.xzw * r8.xyz;
    ps = r0.y * ps;
    r2.yzw = -r5.xyz + r2.yzw;
    r0.y = ps;
    ps = r5.y;
    r6 = r11.wxww * r10.wxzy;
    ps = r0.z * ps;
    r10 = r8 * r6.yyyy;
    r0.z = ps;
    r2.yzw = r2.yzw * UniformScalar_15.xxx + r5.xyz;
    r8.xyz = r2.yzw * UniformScalar_16.xxx + UniformScalar_17.xxx;
    ps = r5.z;
    r2 = r10 * r2.xxxx;
    r2 = (-abs(r4.wwww) >= 0.0) ? r2 : 0.0;
    r2 = (-abs(r1.wwww) >= 0.0) ? 0.0 : r2;
    ps = r0.w * ps;
    r5.xyz = r2.xyz + UniformVector_0.xyz;
    r0.w = ps;
    ps = r6.w;
    r2.xyz = r8.xzy + r2.www;
    r6.y = ps;
    r8.xyz = r0.yzw * AmbientColorAndSkyFactor.xyz + r5.xyz;
    ps = ModShadowColor.x + r6.y;
    r7.z = saturate(dot(r7.xzw, r9.yxz));
    r7.x = ps;
    ps = log2(r7.z);
    r7.y = dot(r1.zxy, r4.zxy);
    r7.z = ps;
    ps = ModShadowColor.y + r6.z;
    r1.xyz = r7.yyz * float3(-0.5, 0.5, 15.0);
    r7.y = ps;
    ps = ModShadowColor.z + r6.x;
    r5.xy = r1.xy + 0.5;
    r7.z = ps;
    ps = pow(2.0, r1.z);
    r6.xy = abs(r5.xy) * abs(r5.xy);
    r6.w = ps;
    ps = ConstantLighting.x * r6.w;
    r1.xyz = r4.zzz * ConstantLighting.xyz;
    r4.x = ps;
    ps = ConstantLighting.z * r6.w;
    r5.xyz = r0.ywz * r6.xxx;
    r4.y = ps;
    ps = ConstantLighting.y * r6.w;
    r6.xyz = r0.ywz * r6.yyy;
    r4.z = ps;
    r6.xyz = r6.xyz * UpperSkyColor.xzy + r8.xzy;
    r6.xyz = r5.xzy * LowerSkyColor.xyz + r6.xzy;
    r6.xyz = r4.xyz * r2.xyz + r6.xzy;
    r6.xyz = r1.xyz * r0.yzw + r6.xzy;
    ps = -r3.w;
    r6.xyz = r6.xyz * r7.xyz;
    ps = OpacityOverride.x + ps;
    r7.xyz = r6.xyz - r6.xyz;
    r7.w = ps;
    oC0.w = r7.w * r0.x + r3.w;
    r6.xyz = r7.xyz * r0.xxx + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
