// ps_e69ccb9b3d3e82fc.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 213 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000354 10040C00 0000080A 00000000 00007108 003F00FF 00000021 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c11); // float4
float4 ModShadowAccumResolution : register(c16); // float2
float4 ModShadowColor : register(c14); // float3
float4 ModShadowGroupColor : register(c15); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c13); // float2
float4 SpotDirection : register(c12); // float3
float4 UniformScalar_0 : register(c6); // float
float4 UniformScalar_1 : register(c7); // float
float4 UniformScalar_4 : register(c8); // float
float4 UniformScalar_5 : register(c9); // float
float4 UniformScalar_6 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D ShadowTexture : register(s5);
sampler2D ModShadowAccumTexture : register(s6);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r11 = tex2D(Texture2D_2, r1.xy);
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.zw = r1.xy * UniformScalar_0.xx;
    r0.w = ps;
    ps = 1.0 / r5.w;
    r7.xy = r5.xy * ScreenPositionScaleBias.xy;
    r6.x = ps;
    r7.xw = r7.xy * r6.xx + ScreenPositionScaleBias.wz;
    ps = r11.w;
    r7.yz = r1.wz * UniformVector_2.xy;
    r6.y = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.x = r2.z + 0.1;
    r0.z = ps;
    ps = (-0.5) + r6.y;
    r0.zw = r0.zw * abs(r8.xy);
    r6.y = ps;
    ps = 0.0001 * r5.w;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.yyyy)) clip(-1.0);
    r12.z = saturate(ps);
    r8.xyz = tex2D(LightAttenuationTexture, r7.xw).xyz;
    r7.x = tex2D(ShadowTexture, r0.xy).x;
    r9.xy = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r7.yzw = tex2D(Texture2D_3, r7.yz).xyz;
    r10.yzw = tex2D(Texture2D_0, r1.xy).xyz;
    r6.yzw = tex2D(Texture2D_1, r6.zw).wxy;
    ps = UniformVector_1.x * UniformVector_1.w;
    r1.xyz = -ModShadowColor.xyz + 1.0;
    r0.y = ps;
    ps = UniformVector_1.y * UniformVector_1.w;
    r1.w = float((UniformScalar_4.x >= 1.0));
    r0.z = ps;
    ps = UniformVector_1.z * UniformVector_1.w;
    r2.w = dot(r4.zxy, r4.zxy);
    r0.w = ps;
    ps = -UniformVector_0.x;
    r4.w = dot(r2.zxy, r2.zxy);
    ps = 1.0 + ps;
    r5.x = float((UniformScalar_4.x > 1.0));
    r0.x = ps;
    r9.zw = r6.zw * 2.0 - 1.0;
    ps = (-1.0) + r6.y;
    r6.z = dot(r3.zxy, r3.zxy);
    r10.x = ps;
    ps = 1.0 - r6.z;
    r12.xyw = r10.wyz + float3(-2.0, 0.0, 0.0);
    r6.w = saturate(ps);
    r7.yzw = (-abs(r5.xxx) >= 0.0) ? r7.yzw : 1.0;
    ps = rsqrt(abs(r4.w));
    r0.yzw = r0.yzw * r11.xyz;
    r6.y = ps;
    ps = rsqrt(abs(r2.w));
    r5.xyz = r6.yyy * r2.xyz;
    r6.y = ps;
    ps = 5.0 * r6.x;
    r11.xyz = r6.yyy * r4.xyz;
    r6.y = saturate(ps);
    r7.yzw = (-abs(r1.www) >= 0.0) ? 1.0 : r7.yzw;
    ps = log2(r6.w);
    r10.yzw = r12.ywx + r10.yzw;
    r6.x = ps;
    ps = 1.0 - r6.y;
    r2.xy = r10.yz - 1.0;
    r6.y = ps;
    ps = rsqrt(abs(r6.z));
    r12.xy = r6.yy * ModShadowGroupColor.xy;
    r6.y = ps;
    ps = r0.y;
    r6.yzw = r6.yyy * -SpotDirection.xyz;
    ps = r7.y * ps;
    r6.y = dot(r6.wyz, r3.zxy);
    r7.y = ps;
    ps = r0.z;
    r4.yzw = -r12.zxy + 1.0;
    r2.zw = r10.wx * r4.yy + 1.0;
    ps = r7.z * ps;
    r4.x = r4.y * UniformScalar_1.x;
    r7.z = ps;
    r6.zw = r4.xx * r9.zw + r2.xy;
    ps = r0.w;
    r2.xy = r6.zw * r4.yy;
    r2 = (r4.xyyy > 0.0) ? r2.wxyz : float4(1.0, 0.0, 0.0, 1.0);
    r3 = (r4.xyyy >= 0.0) ? r2 : float4(1.0, 0.0, 0.0, 1.0);
    ps = r7.w * ps;
    r6.z = dot(r3.wyz, r3.wyz);
    r7.w = ps;
    ps = -SpotAngles.x - -r6.y;
    r10.xyz = r7.yzw * r3.xxx;
    r6.y = ps;
    r2.xyz = r10.xyz * UniformScalar_5.xxx + UniformScalar_6.xxx;
    ps = rsqrt(abs(r6.z));
    r7.y = saturate(r6.y * SpotAngles.y);
    r6.y = ps;
    ps = -UniformVector_0.y;
    r3.xyz = r3.yzw * r6.yyy;
    ps = 1.0 + ps;
    r6.y = dot(r3.zxy, r11.zxy);
    r0.y = ps;
    ps = -UniformVector_0.z;
    r6.yzw = r3.xyz * r6.yyy;
    r6.yzw = r6.yzw * 2.0 - r11.xyz;
    ps = 1.0 + ps;
    r6.y = saturate(dot(r5.zxy, r6.wyz));
    r0.z = ps;
    ps = log2(r6.y);
    r0.xyw = r10.xyz * r0.xyz;
    r9.z = ps;
    r6.yzw = r9.xyz * float3(0.875, 0.875, 15.0);
    r7.zw = r6.yz * r4.zw + 0.125;
    ps = pow(2.0, r6.w);
    r0.z = saturate(dot(r3.zyx, r5.zyx));
    r6.y = ps;
    r6.yzw = r2.xyz * r6.yyy;
    r0.xy = r0.xy * r0.zz + r6.yz;
    r0.z = r0.w * r0.z + r6.w;
    ps = LightColorAndFalloffExponent.w * r6.x;
    r7.xyz = r7.zxy * r7.wxy;
    r6.w = ps;
    r6.xyz = r7.xxx * r1.xyz + ModShadowColor.xyz;
    ps = pow(2.0, r6.w);
    r1.xyz = r7.yyy * r8.xyz;
    r6.w = ps;
    r1.xyz = r1.xzy * r6.www;
    r0.xyz = r1.xzy * r0.xyz;
    r0.xyz = r0.xyz * LightColorAndFalloffExponent.xyz;
    r7.xyz = r0.xzy * r7.zzz;
    r6.xyz = r7.xzy * r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
