// ps_264411920b127904.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 213 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000354 10040C00 0000070A 00000000 000068E7 001F007F 00000021 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c10); // float4
float4 ModShadowAccumResolution : register(c13); // float2
float4 ModShadowColor : register(c11); // float3
float4 ModShadowGroupColor : register(c12); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_3 : register(c7); // float
float4 UniformScalar_4 : register(c8); // float
float4 UniformScalar_5 : register(c9); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_2 : register(c4); // float4
float4 UniformVector_3 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r10 = tex2D(Texture2D_2, r0.xy);
    ps = 1.0 / r4.w;
    r5.yz = r4.xy * ScreenPositionScaleBias.xy;
    r5.x = ps;
    r6.zw = r5.yz * r5.xx + ScreenPositionScaleBias.wz;
    ps = r10.w;
    r5.xw = r0.wz * UniformVector_4.xy;
    r5.y = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r8.xw = r10.yz * UniformScalar_4.xx;
    r6.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r5.z = float((UniformScalar_3.x >= 1.0));
    r6.y = ps;
    ps = (-0.5) + r5.y;
    r6.xy = r6.xy * abs(r7.xy);
    r5.y = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r5.yyyy)) clip(-1.0);
    r7.xyz = tex2D(LightAttenuationTexture, r6.zw).xyz;
    r8.yz = tex2D(ModShadowAccumTexture, r6.xy).xy;
    r12 = tex2D(Texture2D_1, r0.xy);
    r0.xyz = tex2D(Texture2D_0, r0.xy).xyz;
    r6.xyz = tex2D(Texture2D_3, r5.xw).xyz;
    r11.w = saturate(r4.w * 0.0001);
    r5.w = dot(r3.zxy, r3.zxy);
    r6.w = dot(r1.zxy, r1.zxy);
    ps = r1.z;
    r0.w = float((UniformScalar_3.x > 1.0));
    ps = 0.1 + ps;
    r5.x = dot(r2.zxy, r2.zxy);
    r5.y = ps;
    r6.xyz = (-abs(r0.www) >= 0.0) ? r6.xyz : 1.0;
    r9.xyz = r0.xyz * 2.0 - 1.0;
    ps = rsqrt(abs(r6.w));
    r5.y = saturate(r5.y * 5.0);
    r6.w = ps;
    ps = rsqrt(abs(r5.w));
    r4.xyz = r6.www * r1.xyz;
    r5.w = ps;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r2.xyz = UniformVector_3.xyz * 2e+01;
    r1.xyz = r12.xyz * r12.www;
    r11.xyz = r9.xyz * UniformVector_2.xyz;
    r9.xyz = r5.www * r3.xyz;
    ps = 1.0 - r5.x;
    r2.xyz = r2.xyz * r1.xyz;
    r5.x = saturate(ps);
    r6.xyz = (-abs(r5.zzz) >= 0.0) ? 1.0 : r6.yxz;
    ps = log2(r5.x);
    r5.z = -r5.y + 1.0;
    r5.x = ps;
    r1.xyz = r6.yxz * r10.xyz + r2.xyz;
    r5.y = dot(r2.zxy, float3(0.11, 0.3, 0.59));
    ps = r5.y;
    r3.zw = r8.xw * r6.xz;
    ps = -r2.x + ps;
    r5.yw = -r11.zw + 1.0;
    r6.w = ps;
    r3.x = saturate(r6.w + r2.x);
    ps = r5.y;
    r1.xyz = r1.xyz * r0.xyz;
    ps = r3.x * ps;
    r0.xy = -r11.xy * r3.xx;
    r0.z = ps;
    r0.xyz = r11.zxy + r0.zxy;
    ps = (-1.0) - -r0.x;
    r2.xyz = -ModShadowColor.xyz + 1.0;
    r0.w = ps;
    ps = UniformScalar_4.x * r6.y;
    r0.xyw = r0.yzw * r5.www;
    r6.x = ps;
    ps = r6.x;
    r0.z = r0.w + 1.0;
    r6.xyz = (r5.www > 0.0) ? r0.xyz : float3(0.0, 0.0, 1.0);
    r0.xyz = (r5.www >= 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    ps = r10.x * ps;
    r5.y = dot(r0.zxy, r0.zxy);
    r3.y = ps;
    ps = rsqrt(abs(r5.y));
    r6.xyz = r3.yzw + UniformScalar_5.xxx;
    r5.y = ps;
    r0.xyz = r0.xyz * r5.yyy;
    r5.y = dot(r0.zxy, r9.zxy);
    ps = ModShadowGroupColor.x * r5.z;
    r10.xyz = r0.xyz * r5.yyy;
    r3.y = ps;
    r9.xyz = r10.xyz * 2.0 - r9.xyz;
    ps = ModShadowGroupColor.y * r5.z;
    r5.y = saturate(dot(r4.zxy, r9.zxy));
    r3.z = ps;
    ps = log2(r5.y);
    r3.xyz = -r3.xyz + 1.0;
    r8.x = ps;
    ps = LightColorAndFalloffExponent.w * r5.x;
    r8.xyz = r8.xyz * float3(15.0, 0.875, 0.875);
    r5.y = ps;
    ps = pow(2.0, r8.x);
    r6.xyz = r6.xyz * r3.xxx;
    r5.z = ps;
    r5.xw = r8.yz * r3.yz + 0.125;
    ps = r5.x * r5.w;
    r6.w = saturate(dot(r0.zyx, r4.zyx));
    r5.x = ps;
    ps = pow(2.0, r5.y);
    r6.xyz = r6.xyz * r5.zzz;
    r5.w = ps;
    r5.xyz = r5.xxx * r2.xyz + ModShadowColor.xyz;
    r6.xy = r1.xy * r6.ww + r6.xy;
    r6.z = r1.z * r6.w + r6.z;
    r6.xyz = r6.xyz * r5.www;
    r6.xyz = r6.xyz * r7.xyz;
    r6.xyz = r6.xyz * LightColorAndFalloffExponent.xyz;
    r5.xyz = r6.xzy * r5.xzy;
    oC0.xyz = r5.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
