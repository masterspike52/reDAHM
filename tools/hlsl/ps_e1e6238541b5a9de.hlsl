// ps_e1e6238541b5a9de.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 315 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000004EC 10041100 00000606 00000000 000050C6 003F003F 00000021 00003050 00003151 0000F254 0000F356 0000F457 0000F558
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c14); // float4
float4 ConstantLighting : register(c15); // float3
float4 LowerSkyColor : register(c13); // float3
float4 ModShadowAccumResolution : register(c18); // float2
float4 ModShadowColor : register(c16); // float3
float4 ModShadowGroupColor : register(c17); // float3
float4 OpacityOverride : register(c11); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_3 : register(c9); // float
float4 UniformScalar_4 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UpperSkyColor : register(c12); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
sampler2D ModShadowAccumTexture : register(s8);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float2 vPos : VPOS;   // r6 (pixel parameters)
    float vFace : VFACE;  // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r9 = tex2D(Texture2D_5, r0.xy);
    ps = (-0.5) + r9.w;
    r0.zw = r1.xy * 6.0;
    r1.z = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r1.zzzz)) clip(-1.0);
    r7.xyz = tex2D(Texture2D_2, r0.zw).xyw;
    ps = r1.x;
    r1.zw = r1.xy * UniformVector_6.xy;
    ps = UniformVector_2.x * ps;
    r10.xy = r1.xy * UniformVector_1.xy;
    r12.x = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r0.w = r3.w - 4e+02;
    r11.y = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r0.z = dot(r4.zxy, r4.zxy);
    r11.z = ps;
    r12.zw = r7.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r0.z));
    r12.y = saturate(r0.w * 0.00022222222);
    r0.z = ps;
    ps = -r1.y;
    r8.xyz = r0.zzz * r4.xyz;
    ps = 1.0 + ps;
    r4.xyw = r12.zyw * float3(0.5, -0.025, 0.5);
    r0.z = ps;
    ps = UniformVector_2.y * r0.z;
    r12.zw = r8.xy * r4.yy;
    r12.y = ps;
    r0.zw = r12.xy - 0.5;
    r7.xy = float2((r12.xy >= 0.5));
    ps = r11.y;
    r0.zw = r0.zw - r12.xy;
    ps = abs(r6.x) * ps;
    r10.zw = r0.zw * r7.xy;
    r11.x = ps;
    ps = r11.z;
    r10 = r12.zwxy + r10;
    ps = abs(r6.y) * ps;
    r0.zw = r10.zw + r10.zw;
    r11.y = ps;
    r4.yz = tex2D(ModShadowAccumTexture, r11.xy).xy;
    r11.xyz = tex2D(Texture2D_4, r10.xy).xyz;
    r15.yzw = tex2D(Texture2D_6, r1.zw).xyz;
    r17 = tex2D(Texture2D_1, r0.zw);
    r10.xyw = tex2D(Texture2D_0, r10.xy).xyz;
    r6.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r14.xyz = tex2D(Texture2D_2, r1.xy).xyw;
    r13.xyz = tex2D(Texture2D_7, r0.xy).xyz;
    ps = OpacityOverride.x;
    r1.z = saturate(r3.w * 0.0001);
    r7.w = saturate(ps);
    ps = UniformVector_5.x * UniformVector_5.w;
    r1.x = dot(r5.zxy, r5.zxy);
    r12.x = ps;
    ps = UniformVector_5.y * UniformVector_5.w;
    r0.z = dot(r2.zxy, r2.zxy);
    r12.y = ps;
    ps = UniformVector_5.z * UniformVector_5.w;
    r0.xyw = -UniformVector_0.yxz + 1.0;
    r12.z = ps;
    r13.yzw = r13.xyz * UniformScalar_3.xxx + UniformScalar_4.xxx;
    r15.x = r7.z * r14.z - 1.0;
    r14.zw = r14.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r0.z));
    r16.xyz = r12.xyz * r9.xyz;
    r0.z = ps;
    r6.xyz = r6.zxy * 2.0 - 1.0;
    r12.xyz = r10.wxy * 2.0 - 1.0;
    r13.x = r6.x * UniformVector_4.z - r12.x;
    ps = AmbientColorAndSkyFactor.x * r0.y;
    r9.xyz = r0.zzz * r2.xyz;
    r14.x = ps;
    ps = AmbientColorAndSkyFactor.y * r0.x;
    r9.w = dot(r7.xyy, float3(1.0, 1.0, 1.0));
    r14.y = ps;
    r0.z = (r9.w == 0.0) ? r17.x : r17.y;
    ps = rsqrt(abs(r1.x));
    r2.xyz = r9.zww + float3(0.1, -3.0, -2.0);
    r1.x = ps;
    ps = 5.0 * r2.x;
    r10.xyz = r1.xxx * r5.xyz;
    r1.y = saturate(ps);
    r0.z = (r2.z == 0.0) ? r17.z : r0.z;
    r7.z = (r2.y == 0.0) ? r17.w : r0.z;
    ps = 1.0 - r1.y;
    r5.yzw = r16.xyz * r15.yzw;
    r0.z = ps;
    ps = 1.0 - r1.z;
    r1.x = max(r7.z, 0.0);
    r2.x = ps;
    ps = ModShadowGroupColor.x * r0.z;
    r1.x = min(r1.x, 0.3);
    r7.x = ps;
    ps = 3.3333333 * r1.x;
    r15.yz = r14.zw + r4.xw;
    r1.x = ps;
    ps = ModShadowGroupColor.y * r0.z;
    r1.xyz = r1.xxx * r15.xyz;
    r7.y = ps;
    ps = AmbientColorAndSkyFactor.z * r0.w;
    r7 = -r7.wxyz + 1.0;
    r14.z = ps;
    r1.yz = r6.yz * UniformVector_4.xy + r1.yz;
    ps = r7.w;
    r1.w = r1.x + 1.0;
    r0.z = ps;
    ps = 2.5 * r0.z;
    r1.x = abs(r1.w) * abs(r1.w);
    r5.x = ps;
    ps = r1.x * r1.x;
    r6.yz = r1.yz - r12.yz;
    r6.x = ps;
    r1.xyz = r5.yzw * r1.www - r11.xyz;
    r6.w = saturate(r5.x * r1.w - 0.5);
    r11.xyz = r1.xyz * r6.www + r11.xyz;
    r5.xyz = r14.xyz * r11.xyz + UniformVector_0.xyz;
    r1.xyz = r6.zxy * r6.wxw;
    ps = r12.y;
    r2.y = float((r7.x >= 0.004));
    ps = r1.z + ps;
    r13.yzw = r1.yyy * r13.yzw;
    r1.y = ps;
    ps = r12.z;
    r6 = r13.xywz * r6.wwww;
    ps = r1.x + ps;
    r0.z = r6.x - 2.0;
    r1.z = ps;
    r1.x = r10.w * 2.0 + r0.z;
    r1.yzw = r1.xyz * r2.xxx;
    ps = 1.0 + r1.y;
    r1.x = ps;
    r1.xyz = (r2.xxx > 0.0) ? r1.xzw : float3(1.0, 0.0, 0.0);
    r1.xyz = (r2.xxx >= 0.0) ? r1.xyz : float3(1.0, 0.0, 0.0);
    r0.z = dot(r1.xyz, r1.xyz);
    ps = rsqrt(abs(r0.z));
    r2.xzw = r11.xyz * r0.yxw;
    r0.x = ps;
    ps = -ModShadowColor.x;
    r0.xyw = r1.yzx * r0.xxx;
    ps = 1.0 + ps;
    r4.x = dot(r10.zxy, r0.wxy);
    r1.x = ps;
    ps = -ModShadowColor.y;
    r0.z = dot(r0.wxy, r8.zxy);
    ps = 1.0 + ps;
    r10.xyz = r0.xyw * r0.zzz;
    r1.y = ps;
    ps = -ModShadowColor.z;
    r4 = r4.xxyz * float4(-0.5, 0.5, 0.875, 0.875);
    r8.xyz = r10.xyz * 2.0 - r8.xyz;
    ps = 1.0 + ps;
    r1.w = saturate(dot(r9.zxy, r8.zxy));
    r1.z = ps;
    r7.xy = r4.zw * r7.yz + 0.125;
    ps = ConstantLighting.x * r0.w;
    r4.yz = r4.xy + 0.5;
    r4.x = ps;
    ps = ConstantLighting.y * r0.w;
    r4.zw = abs(r4.yz) * abs(r4.yz);
    r4.y = ps;
    ps = log2(r1.w);
    r0.z = r7.x * r7.y;
    r0.x = ps;
    r1.xyz = r0.zzz * r1.xyz + ModShadowColor.xyz;
    ps = 15.0 * r0.x;
    r7.xyz = r2.xwz * r4.zzz;
    r0.z = ps;
    ps = pow(2.0, r0.z);
    r8.xyz = r2.xwz * r4.www;
    r0.z = ps;
    r5.xyz = r8.xyz * UpperSkyColor.xzy + r5.xzy;
    r5.xyz = r7.xzy * LowerSkyColor.xyz + r5.xzy;
    ps = ConstantLighting.z * r0.w;
    r7.xyz = r0.zzz * ConstantLighting.xzy;
    r4.z = ps;
    r0.xyz = r7.xyz * r6.yzw + r5.xzy;
    r0.xyz = r4.xyz * r2.xzw + r0.xzy;
    ps = -r3.w;
    r0.xyz = r0.xyz * r1.xyz;
    ps = OpacityOverride.x + ps;
    r1.xyz = r0.xyz - r0.xyz;
    r1.w = ps;
    oC0.w = r1.w * r2.y + r3.w;
    r0.xyz = r1.xyz * r2.yyy + r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
