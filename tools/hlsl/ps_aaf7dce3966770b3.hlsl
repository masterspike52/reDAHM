// ps_aaf7dce3966770b3.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 324 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000510 10041000 0000080A 00000000 00006908 003F00FF 00000021 00003050 00003151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c13); // float4
float4 ModShadowAccumResolution : register(c18); // float2
float4 ModShadowColor : register(c16); // float3
float4 ModShadowGroupColor : register(c17); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c15); // float2
float4 SpotDirection : register(c14); // float3
float4 UniformScalar_11 : register(c11); // float
float4 UniformScalar_12 : register(c12); // float
float4 UniformScalar_7 : register(c9); // float
float4 UniformScalar_8 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_6 : register(c7); // float4
float4 UniformVector_7 : register(c8); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D Texture2D_8 : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r12 = tex2D(Texture2D_5, r0.xy);
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.z = r12.w - 0.5;
    r6.y = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r13.xyz = -ModShadowColor.xyz + 1.0;
    r6.x = ps;
    r6.xy = r6.xy * abs(r8.xy);
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.zzzz)) clip(-1.0);
    r6.yz = tex2D(ModShadowAccumTexture, r6.xy).xy;
    r8.x = r1.x * UniformVector_2.x;
    ps = r5.w;
    r7.xy = r1.xy * 6.0;
    ps = (-4e+02) + ps;
    r6.w = dot(r4.zxy, r4.zxy);
    r6.x = ps;
    ps = rsqrt(abs(r6.w));
    r6.x = saturate(r6.x * 0.00022222222);
    r6.w = ps;
    ps = -r1.y;
    r9.xyz = r6.www * r4.xyz;
    ps = 1.0 + ps;
    r14.xyz = r6.yzx * float3(0.875, 0.875, -0.025);
    r6.x = ps;
    ps = UniformVector_2.y * r6.x;
    r8.zw = r9.xy * r14.zz;
    r8.y = ps;
    ps = r1.x;
    r6.xz = r8.xy - 0.5;
    ps = UniformVector_1.x * ps;
    r6.yw = float2((r8.xy >= 0.5));
    r4.x = ps;
    ps = r1.y;
    r6.xz = r6.xz - r8.xy;
    ps = UniformVector_1.y * ps;
    r4.zw = r6.xz * r6.yw;
    r4.y = ps;
    r10 = r8.zwxy + r4;
    r6.xz = r10.zw + r10.zw;
    r16 = tex2D(Texture2D_1, r6.xz);
    r8.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r11.xzw = tex2D(Texture2D_2, r7.xy).wxy;
    r4.xyw = tex2D(Texture2D_2, r1.xy).xwy;
    r7.xyw = tex2D(Texture2D_0, r10.xy).xyz;
    ps = r5.w;
    r6.z = r2.z + 0.1;
    ps = 0.0001 * ps;
    r15.xy = r1.xy * UniformVector_7.xy;
    r6.x = saturate(ps);
    r7.xyz = r7.xyw * 2.0 - 1.0;
    r1.x = r11.x * r4.y - 1.0;
    ps = r4.x + r4.x;
    r11.xy = r5.xy * ScreenPositionScaleBias.xy;
    r1.z = ps;
    r4.xyz = r8.xyz * 2.0 - 1.0;
    ps = r4.w + r4.w;
    r6.z = saturate(r6.z * 5.0);
    r1.w = ps;
    ps = 1.0 - r6.z;
    r8.xyw = r4.xyz * UniformVector_4.xyz;
    r6.z = ps;
    ps = 1.0 - r6.x;
    r1.y = dot(r6.yww, float3(1.0, 1.0, 1.0));
    r0.z = ps;
    r6.x = (r1.y == 0.0) ? r16.x : r16.y;
    ps = ModShadowGroupColor.x * r6.z;
    r4 = r1.zwyy + float4(-1.0, -1.0, -2.0, -3.0);
    r1.y = ps;
    r6.x = (r4.z == 0.0) ? r16.z : r6.x;
    r1.w = (r4.w == 0.0) ? r16.w : r6.x;
    ps = ModShadowGroupColor.y * r6.z;
    r6.y = max(r1.w, 0.0);
    r1.z = ps;
    r6.xzw = -r1.yzw + 1.0;
    ps = r11.z + r11.z;
    r6.y = min(r6.y, 0.3);
    r14.z = ps;
    ps = r11.w + r11.w;
    r14.xy = r14.xy * r6.xz;
    r14.w = ps;
    ps = 3.3333333 * r6.y;
    r14 = r14 + float4(0.125, 0.125, -1.0, -1.0);
    r4.w = ps;
    r1.yw = r14.zw * 0.5 + r4.xy;
    r6.xy = r4.ww * r1.yw + r8.xy;
    ps = 1.0 / r5.w;
    r11.zw = r6.xy - r7.xy;
    r6.x = ps;
    r6.xy = r11.xy * r6.xx + ScreenPositionScaleBias.wz;
    r4.xyz = tex2D(LightAttenuationTexture, r6.xy).xyz;
    r1.z = tex2D(Texture2D_4, r0.xy).x;
    r10.xyz = tex2D(Texture2D_7, r10.xy).xyz;
    r6.xyz = tex2D(Texture2D_6, r15.xy).yzx;
    r5.xyz = tex2D(Texture2D_8, r0.xy).xyz;
    r0.w = dot(r2.zxy, r2.zxy);
    r0.x = dot(r3.zxy, r3.zxy);
    r5.xyz = r5.xyz * UniformScalar_11.xxx + UniformScalar_12.xxx;
    ps = rsqrt(abs(r0.w));
    r0.y = saturate(-r0.x + 1.0);
    r0.w = ps;
    ps = rsqrt(abs(r0.x));
    r8.xyz = r0.www * r2.xyz;
    r0.x = ps;
    r2.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r2.xyz = r2.xyz * r12.xyz;
    ps = log2(r0.y);
    r12.xyz = r2.xyz * r6.zxy;
    r6.x = ps;
    ps = -r7.z;
    r0.xyw = r0.xxx * -SpotDirection.xyz;
    ps = r8.w + ps;
    r6.y = dot(r0.wxy, r3.zxy);
    r2.w = ps;
    ps = 2.5 * r6.w;
    r0.x = r14.x * r14.y;
    r12.w = ps;
    r0.xyw = r0.xxx * r13.xyz + ModShadowColor.xyz;
    r1.x = r4.w * r1.x + 1.0;
    r6.w = saturate(r12.w * r1.x - 0.5);
    r2.xyz = r12.xyz * r1.xxx - r10.xyz;
    r10.xyz = r2.xyz * r6.www + r10.xyz;
    r7.xy = r11.zw * r6.ww + r7.xy;
    r7.z = r2.w * r6.w - 2.0;
    r7.z = r7.w * 2.0 + r7.z;
    r7.xyz = r7.zxy * r0.zzz;
    ps = 1.0 + r7.x;
    r3.xyz = -UniformVector_0.xyz + 1.0;
    r7.w = ps;
    r7.xyz = (r0.zzz > 0.0) ? r7.yzw : float3(0.0, 0.0, 1.0);
    r2.xyz = (r0.zzz >= 0.0) ? r7.xyz : float3(0.0, 0.0, 1.0);
    r7.x = dot(r2.zxy, r2.zxy);
    ps = rsqrt(abs(r7.x));
    r7.yzw = r10.xyz * r3.xyz;
    r7.x = ps;
    r3.xyz = r2.xyz * r7.xxx;
    ps = UniformScalar_8.x * r6.z;
    r7.x = dot(r3.zxy, r9.zxy);
    r2.x = ps;
    ps = -SpotAngles.x - -r6.y;
    r2.yzw = r3.xyz * r7.xxx;
    r6.z = ps;
    r2.yzw = r2.yzw * 2.0 - r9.xyz;
    ps = SpotAngles.y * r6.z;
    r6.y = saturate(dot(r8.zxy, r2.wyz));
    r2.y = saturate(ps);
    ps = log2(r6.y);
    r7.x = saturate(dot(r3.zyx, r8.zyx));
    r6.y = ps;
    ps = abs(r1.x) * abs(r1.x);
    r6.z = r6.y * UniformScalar_7.x;
    r2.w = ps;
    ps = pow(2.0, r6.z);
    r6.y = r6.y * 15.0;
    r2.z = ps;
    r1.xyw = r2.xwy * r2.zwy;
    r2.xw = r1.xy * r1.zy;
    r2.xyz = r5.xyz + r2.xxx;
    ps = LightColorAndFalloffExponent.w * r6.x;
    r2.xyz = r2.www * r2.xyz;
    r6.x = ps;
    ps = pow(2.0, r6.y);
    r2.xyz = r2.xyz * r6.www;
    r6.y = ps;
    ps = pow(2.0, r6.x);
    r6.yzw = r2.xyz * r6.yyy;
    r6.x = ps;
    r6.yz = r7.yz * r7.xx + r6.yz;
    r6.w = r7.w * r7.x + r6.w;
    r6.xyz = r6.yzw * r6.xxx;
    r6.xyz = r6.xyz * r4.xyz;
    r6.xyz = r6.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r6.xzy * r1.www;
    r6.xyz = r6.xzy * r0.xyw;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
