// ps_aa3197cf3f611431.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 189 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002F4 10041000 0000080A 00000000 00008108 003F00FF 00000001 0000F050 0000F153 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD3 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c13); // float4
float4 LowerSkyColor : register(c12); // float3
float4 ModShadowAccumResolution : register(c16); // float2
float4 ModShadowColor : register(c14); // float3
float4 ModShadowGroupColor : register(c15); // float3
float4 OpacityOverride : register(c10); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 TwoSidedSign : register(c3); // float
float4 UniformVector_0 : register(c4); // float4
float4 UniformVector_1 : register(c5); // float4
float4 UniformVector_2 : register(c6); // float4
float4 UniformVector_3 : register(c7); // float4
float4 UniformVector_4 : register(c8); // float4
float4 UniformVector_5 : register(c9); // float4
float4 UpperSkyColor : register(c11); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord3 : TEXCOORD3; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord3;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r6.xy = r0.xy + UniformVector_2.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.xy = UniformVector_3.yx - 0.5;
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.xz = r7.xy + r0.yx;
    r6.w = ps;
    r6.zw = r6.zw * abs(r8.xy);
    r7.y = dot(r7.xz, UniformVector_5.yx) + 0.5;
    r7.x = dot(r7.xz, UniformVector_4.yx) + 0.5;
    r9.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r12.yz = tex2D(ModShadowAccumTexture, r6.zw).xy;
    r6.xyz = tex2D(Texture2D_2, r6.xy).xzy;
    r13.xyz = tex2D(Texture2D_2, r7.xy).xyz;
    r11.xyw = tex2D(Texture2D_0, r0.xy).xyz;
    r7.xyz = tex2D(Texture2D_1, r0.xy).zxy;
    r0.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r10.xyz = -ModShadowColor.xyz + 1.0;
    r7.w = dot(r2.zxy, r2.zxy);
    r0.w = dot(r5.zxy, r5.zxy);
    r8.w = dot(r4.zxy, r4.zxy);
    r14.xyz = -UniformVector_0.xyz + 1.0;
    ps = OpacityOverride.x;
    r15.xyz = r0.xyz * AmbientColorAndSkyFactor.xyz;
    r11.z = saturate(ps);
    ps = UniformVector_0.x + r7.y;
    r8.xyz = r14.xyz * r0.xyz;
    r16.x = ps;
    ps = UniformVector_0.y + r7.z;
    r6.w = dot(r11.wxy, r11.wxy);
    r16.y = ps;
    ps = UniformVector_0.z + r7.x;
    r0.xyz = r6.zxy * r13.yxz;
    r16.z = ps;
    ps = rsqrt(abs(r8.w));
    r12.xw = r0.yz * r13.xz;
    r8.w = ps;
    ps = rsqrt(abs(r0.w));
    r13.xyz = r8.www * r4.xyz;
    r0.w = ps;
    ps = rsqrt(abs(r7.w));
    r4.xyz = r0.www * r5.xyz;
    r7.w = ps;
    ps = r0.x * r0.x;
    r0.yzw = r7.www * r2.xzy;
    r6.z = ps;
    ps = rsqrt(abs(r6.w));
    r6.xy = r12.wx * r6.yx;
    r6.w = ps;
    ps = 0.1 - -r0.z;
    r2.xyz = r6.www * r11.wxy;
    r0.x = ps;
    ps = 5.0 * r0.x;
    r2.xyz = r2.xyz * TwoSidedSign.xxx;
    r7.w = saturate(ps);
    ps = UniformVector_1.x * r6.y;
    r12.x = dot(r4.zxy, r2.xyz);
    r4.x = ps;
    ps = UniformVector_1.y * r6.z;
    r6.w = dot(r2.xyz, r13.zxy);
    r4.y = ps;
    ps = UniformVector_1.z * r6.x;
    r6.z = -r7.w + 1.0;
    r4.z = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r6.xyw = r2.yzx * r6.www;
    r11.x = ps;
    r7.xyz = r4.xyz * r7.yzx + r16.xyz;
    r4.xyz = r15.xyz * r14.xyz + r7.xyz;
    r5.xyz = r6.xyw * 2.0 - r13.xyz;
    ps = ModShadowGroupColor.y * r6.z;
    r7 = r12.xxyz * float4(0.5, -0.5, 0.875, 0.875);
    r11.y = ps;
    ps = r7.y;
    r2.xyz = r2.xxx * r1.xyz;
    r6.x = ps;
    ps = 0.5 + r6.x;
    r0.z = saturate(dot(r0.zyw, r5.zxy));
    r0.x = ps;
    ps = 0.5 + r7.x;
    r6.xyw = -r11.xyz + 1.0;
    r0.y = ps;
    r6.yz = r7.zw * r6.xy + 0.125;
    ps = log2(r0.z);
    r0.xy = abs(r0.xy) * abs(r0.xy);
    r6.x = ps;
    ps = 15.0 * r6.x;
    r6.w = float((r6.w >= 0.004));
    r7.w = ps;
    ps = r6.y * r6.z;
    r7.xyz = r8.xzy * r0.yyy;
    r6.x = ps;
    r6.xyz = r6.xxx * r10.xyz + ModShadowColor.xyz;
    r7.xyz = r7.xyz * UpperSkyColor.xzy + r4.xzy;
    ps = pow(2.0, r7.w);
    r4.xyz = r8.xzy * r0.xxx;
    r7.w = ps;
    r0.xyz = r7.www * r1.xzy;
    r7.xyz = r4.xzy * LowerSkyColor.xyz + r7.xzy;
    r7.xyz = r0.xyz * r9.xzy + r7.xzy;
    r7.xyz = r2.xyz * r8.xyz + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xyz * r6.xyz;
    ps = OpacityOverride.x + ps;
    r7.xyz = r6.xyz - r6.xyz;
    r7.w = ps;
    oC0.w = r7.w * r6.w + r3.w;
    r6.xyz = r7.xyz * r6.www + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
