// ps_bf84f72ecd695562.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 189 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002F4 10040E00 0000080A 00000000 00008108 003F00FF 00000001 0000F050 0000F153 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD3 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c14); // float4
float4 LowerSkyColor : register(c13); // float3
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 OpacityOverride : register(c11); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_0 : register(c5); // float
float4 UniformScalar_1 : register(c6); // float
float4 UniformScalar_2 : register(c7); // float
float4 UniformScalar_3 : register(c8); // float
float4 UniformScalar_4 : register(c9); // float
float4 UniformScalar_8 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UpperSkyColor : register(c12); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
samplerCUBE TextureCube_0 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord3 : TEXCOORD3; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord3;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r6.y = dot(r4.zxy, r4.zxy);
    r6.x = ps;
    ps = rsqrt(abs(r6.y));
    r9.xy = UniformVector_1.xy;
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.x = r6.z * r4.z;
    r6.y = ps;
    ps = r6.x;
    r13.xy = r6.zz * -r4.xy;
    ps = abs(r8.x) * ps;
    r7.w = r7.x + r7.x;
    r7.y = ps;
    ps = r6.y;
    r13.z = r7.w - r7.x;
    ps = abs(r8.y) * ps;
    r6 = xe_cube(r13.xyz);
    r7.z = ps;
    ps = 1.0 / abs(r6.z);
    r4.z = r6.w;
    r4.x = ps;
    r4.xy = r6.yx * r4.xx + 1.5;
    r4.xyz = texCUBE(TextureCube_0, xe_cube_dir(r4.xyz)).xyz;
    r12.yz = tex2D(ModShadowAccumTexture, r7.yz).xy;
    r7.z = tex2D(Texture2D_2, r0.xy).x;
    r11.xyz = tex2D(Texture2D_3, r9.xy).xyz;
    r8.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r10.xyz = tex2D(Texture2D_0, r0.xy).xyz;
    r9.xyz = -ModShadowColor.xyz + 1.0;
    r6.x = dot(r5.zxy, r5.zxy);
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r6.y = dot(r2.zxy, r2.zxy);
    r14.xyz = r10.xyz * r8.xyz - r10.xyz;
    ps = rsqrt(abs(r6.y));
    r8.xyz = r0.xyz * AmbientColorAndSkyFactor.xyz;
    r6.y = ps;
    ps = rsqrt(abs(r6.x));
    r6.yzw = r6.yyy * r2.xyz;
    r12.x = ps;
    r10.xyz = r14.xyz * UniformScalar_0.xxx + r10.xyz;
    r2.xyz = r10.xyz * UniformScalar_1.xxx + UniformScalar_2.xxx;
    ps = 0.1 - -r6.w;
    r11.xyz = r7.zzz * r11.xyz;
    r6.x = ps;
    ps = 5.0 * r6.x;
    r12 = r12.xxyz * float4(-0.5, 0.5, 0.875, 0.875);
    r7.y = saturate(ps);
    ps = r7.w;
    r5.zw = r12.xy * r5.zz;
    ps = -r7.x + ps;
    r2.w = saturate(dot(r6.wyz, r13.zxy));
    r7.x = saturate(ps);
    ps = OpacityOverride.x;
    r6.xy = -r7.xy + 1.0;
    r7.w = saturate(ps);
    ps = log2(abs(r6.x));
    r7.xy = r6.yy * ModShadowGroupColor.xy;
    r6.y = ps;
    ps = UniformScalar_3.x * r6.y;
    r7 = -r7.wyxz + 1.0;
    r6.x = ps;
    ps = pow(2.0, r6.x);
    r5.xy = r12.zw * r7.zy;
    r6.x = ps;
    r6.xyw = r6.xxx * r4.yzx;
    ps = UniformScalar_8.x;
    r4.xyz = r7.www * r10.xyz;
    r7.z = ps;
    ps = 0.0001;
    r7.x = float((r7.x >= 0.004));
    r7.w = ps;
    r4.xyz = r11.xyz * r10.xyz + r4.xyz;
    ps = UniformScalar_4.x * r6.w;
    r6.yz = r6.xy * r2.yz;
    r6.x = ps;
    ps = max(r7.z, r7.w);
    r7.y = r6.x * r2.x;
    r0.w = ps;
    ps = UniformScalar_4.x * r6.y;
    r0.xyz = r4.xyz * r0.xyz;
    r7.z = ps;
    ps = UniformScalar_4.x * r6.z;
    r5 = r5.zwxy + float4(0.5, 0.5, 0.125, 0.125);
    r7.w = ps;
    ps = r5.z * r5.w;
    r5.xy = abs(r5.xy) * abs(r5.xy);
    r6.x = ps;
    r6.xyz = r6.xxx * r9.xyz + ModShadowColor.xyz;
    ps = log2(r2.w);
    r7.yzw = r7.yzw + UniformVector_0.xyz;
    r5.z = ps;
    r7.yzw = r8.xyz * r4.xyz + r7.yzw;
    r4 = r0.xzyw * r5.yyyz;
    r7.yzw = r4.xzy * UpperSkyColor.xyz + r7.yzw;
    ps = pow(2.0, r4.w);
    r5.xyz = r0.xzy * r5.xxx;
    r6.w = ps;
    r4.xyz = r6.www * r1.xzy;
    r7.yzw = r5.xyz * LowerSkyColor.xzy + r7.ywz;
    r7.yzw = r4.xzy * r2.xyz + r7.ywz;
    r7.yzw = r0.xzy * r1.xzy + r7.ywz;
    ps = -r3.w;
    r6.xyz = r7.ywz * r6.xyz;
    ps = OpacityOverride.x + ps;
    r0.xyz = r6.xzy - r6.xzy;
    r0.w = ps;
    oC0.w = r0.w * r7.x + r3.w;
    r6.xyz = r0.xyz * r7.xxx + r6.xzy;
    oC0.xyz = r6.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
