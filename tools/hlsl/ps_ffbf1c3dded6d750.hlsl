// ps_ffbf1c3dded6d750.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 180 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002D0 10040D00 0000080A 00000000 00007108 003F00FF 00000001 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c9); // float4
float4 ModShadowAccumResolution : register(c14); // float2
float4 ModShadowColor : register(c12); // float3
float4 ModShadowGroupColor : register(c13); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c11); // float2
float4 SpotDirection : register(c10); // float3
float4 UniformScalar_0 : register(c4); // float
float4 UniformScalar_1 : register(c5); // float
float4 UniformScalar_2 : register(c6); // float
float4 UniformScalar_3 : register(c7); // float
float4 UniformScalar_9 : register(c8); // float
float4 UniformVector_0 : register(c3); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D ShadowTexture : register(s5);
sampler2D ModShadowAccumTexture : register(s6);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / r5.w;
    r6.x = r2.z + 0.1;
    r6.y = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.xy = r6.yy * ScreenPositionScaleBias.xy;
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r9.xyz = -UniformVector_0.xyz + 1.0;
    r6.y = ps;
    r6.yz = r6.yz * abs(r8.xy);
    r7.xy = r7.xy * r5.xy + ScreenPositionScaleBias.wz;
    r5.xyz = tex2D(LightAttenuationTexture, r7.xy).xyz;
    r8.x = tex2D(ShadowTexture, r0.xy).x;
    r8.yzw = tex2D(Texture2D_1, r1.xy).xyz;
    r11.xyz = tex2D(Texture2D_2, r1.xy).xyz;
    r10.xyz = tex2D(Texture2D_3, r1.xy).xyz;
    r7.xyz = tex2D(Texture2D_0, r1.xy).xyz;
    r6.zw = tex2D(ModShadowAccumTexture, r6.yz).xy;
    ps = -ModShadowColor.x;
    r7.w = float((UniformScalar_0.x >= UniformScalar_1.x));
    ps = 1.0 + ps;
    r2.w = float((UniformScalar_0.x > UniformScalar_1.x));
    r0.x = ps;
    ps = -ModShadowColor.y;
    r0.w = dot(r4.zxy, r4.zxy);
    ps = 1.0 + ps;
    r6.y = dot(r3.zxy, r3.zxy);
    r0.y = ps;
    ps = 1.0 - r6.y;
    r1.x = dot(r2.zxy, r2.zxy);
    r1.w = saturate(ps);
    ps = 0.875 * r6.z;
    r12.xyz = r7.zxy + float3(-2.0, 0.0, 0.0);
    r0.z = ps;
    ps = rsqrt(abs(r1.x));
    r13.xyz = r11.xyz * r10.xyz;
    r1.x = ps;
    ps = rsqrt(abs(r0.w));
    r1.xyz = r1.xxx * r2.xyz;
    r0.w = ps;
    ps = rsqrt(abs(r6.y));
    r4.xyz = r0.www * r4.xyz;
    r6.y = ps;
    ps = 0.875 * r6.w;
    r10.xyz = r6.yyy * -SpotDirection.xyz;
    r0.w = ps;
    r2.xyz = (-abs(r2.www) >= 0.0) ? r11.xyz : r13.xyz;
    ps = log2(r1.w);
    r7.xyz = r12.yzx + r7.xyz;
    r6.y = ps;
    r2.xyz = (-abs(r7.www) >= 0.0) ? r11.xyz : r2.xyz;
    ps = 5.0 * r6.x;
    r7.xyz = r7.xyz + float3(-1.0, -1.0, 0.0);
    r6.x = saturate(ps);
    ps = 1.0 - r6.x;
    r6.z = dot(r10.zxy, r3.zxy);
    r6.x = ps;
    r6.xw = -r6.xx * ModShadowGroupColor.xy + 1.0;
    r6.xw = r0.zw * r6.xw + 0.125;
    ps = -ModShadowColor.z;
    r7.xyz = r7.zxy * UniformScalar_0.xxx;
    ps = 1.0 + ps;
    r2.xyz = r2.xyz - r8.yzw;
    r0.z = ps;
    r2.xyz = r2.xyz * UniformScalar_0.xxx + r8.yzw;
    ps = 1.0 + r7.x;
    r6.x = r6.x * r6.w;
    r7.w = ps;
    r0.xyz = r6.xxx * r0.xyz + ModShadowColor.xyz;
    r3.xyz = r2.xyz * UniformScalar_2.xxx + UniformScalar_3.xxx;
    ps = UniformScalar_9.x;
    r6.x = dot(r7.wyz, r7.wyz);
    r8.y = ps;
    ps = rsqrt(abs(r6.x));
    r2.xyz = r2.xyz * r9.xyz;
    r6.x = ps;
    ps = 0.0001;
    r7.xyz = r7.yzw * r6.xxx;
    r8.z = ps;
    ps = max(r8.y, r8.z);
    r6.x = dot(r7.zxy, r4.zxy);
    r8.y = ps;
    ps = -SpotAngles.x - -r6.z;
    r9.xyz = r7.xzy * r6.xxx;
    r6.x = ps;
    r4.xyz = r9.xyz * 2.0 - r4.xzy;
    ps = SpotAngles.y * r6.x;
    r6.z = saturate(dot(r1.zxy, r4.yxz));
    r8.w = saturate(ps);
    ps = log2(r6.z);
    r7.z = saturate(dot(r7.zxy, r1.zxy));
    r8.z = ps;
    ps = LightColorAndFalloffExponent.w * r6.y;
    r6.xzw = r8.wxy * r8.wxz;
    r6.y = ps;
    ps = pow(2.0, r6.y);
    r7.xyw = r6.zzz * r5.xyz;
    r6.y = ps;
    ps = pow(2.0, r6.w);
    r1.xyz = r7.xwy * r6.yyy;
    r6.y = ps;
    r6.yzw = r3.xyz * r6.yyy;
    r7.xy = r2.xy * r7.zz + r6.yz;
    r7.z = r2.z * r7.z + r6.w;
    r7.xyz = r1.xzy * r7.xyz;
    r7.xyz = r7.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r7.xzy * r6.xxx;
    r6.xyz = r6.xzy * r0.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
