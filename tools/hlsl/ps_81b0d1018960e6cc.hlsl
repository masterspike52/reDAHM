// ps_81b0d1018960e6cc.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 270 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000438 10040900 00000106 00000001 00001021 00010001 00000001 0000F050
...
float4 FalloffExponent : register(c34); // float
float4 InvRandomAngleTextureSize : register(c27); // float2
float4 LightPosition : register(c29); // float4
...
float4 ScreenToWorld[4] : register(c30); // float4x4 (matrix_columns)
float4 ShadowModulateColor : register(c28); // float4
float4 SpotAngles : register(c36); // float2
float4 SpotDirection : register(c35); // float3
...
PS_OUTPUT main(PS_INPUT In)
{
    ...
    for (int xe_iter0 = 0, aL = 0; xe_iter0 < 8; xe_iter0++, aL += 0)
    {
        ...
        if (p0)
        {
            if (p0)
            {
                ...
                r3 = float4((r3 > r0.zzzz));
                r1.x = dot(r3.wzxy, float4(0.03125, 0.03125, 0.03125, 0.03125));
                ps = 1.0 + r0.x;
                r0.y = r1.x + r0.y;
                r0.x = ps;
            }
    ...
    r1.xyz = r0.xyz * LightPosition.www;
