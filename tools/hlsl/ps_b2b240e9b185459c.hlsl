// ps_b2b240e9b185459c.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 324 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000510 10041200 0000090A 00000000 00008129 003F01FF 00000021 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c14); // float4
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c8); // float
float4 UniformScalar_1 : register(c9); // float
float4 UniformScalar_10 : register(c11); // float
float4 UniformScalar_11 : register(c12); // float
float4 UniformScalar_12 : register(c13); // float
float4 UniformScalar_9 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_6 : register(c7); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D ShadowTexture : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r12 = tex2D(Texture2D_6, r1.xy);
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.x = -r1.z + 1.0;
    r8.x = ps;
    r7.xy = r1.wz * 6.0;
    r7.zw = r1.xy * UniformScalar_0.xx;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.z = r2.z + 0.1;
    r8.y = ps;
    ps = r5.w;
    r11.xy = r1.wz * UniformVector_1.xy;
    ps = (-4e+02) + ps;
    r8.z = dot(r4.zxy, r4.zxy);
    r6.y = ps;
    ps = rsqrt(abs(r8.z));
    r6.w = r12.w - 0.5;
    r8.z = ps;
    ps = 0.00022222222 * r6.y;
    r10.xyz = r8.zzz * r4.xyz;
    r6.y = saturate(ps);
    ps = 5.0 * r6.z;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.wwww)) clip(-1.0);
    r6.z = saturate(ps);
    ps = (-0.025) * r6.y;
    r4.x = r1.w * UniformVector_2.x;
    r6.w = ps;
    ps = UniformVector_2.y * r6.x;
    r4.zw = r10.xy * r6.ww;
    r4.y = ps;
    r8.zw = r4.xy - 0.5;
    r6.xw = float2((r4.xy >= 0.5));
    ps = r8.x;
    r8.zw = r8.zw - r4.xy;
    ps = abs(r9.x) * ps;
    r11.zw = r8.zw * r6.xw;
    r8.z = ps;
    ps = r8.y;
    r14 = r4.zwxy + r11;
    ps = abs(r9.y) * ps;
    r8.xy = r14.zw + r14.zw;
    r8.w = ps;
    r15.yz = tex2D(ModShadowAccumTexture, r8.zw).xy;
    r17 = tex2D(Texture2D_1, r8.xy);
    r4.xyw = tex2D(Texture2D_3, r7.zw).xyw;
    r11.xyz = tex2D(Texture2D_2, r7.xy).wxy;
    r9.xyw = tex2D(Texture2D_2, r1.wz).xyw;
    r8.xyz = tex2D(Texture2D_4, r1.xy).xyz;
    r7.xyz = tex2D(Texture2D_0, r14.xy).xyz;
    r7.xyw = r7.xyz * 2.0 - 1.0;
    r8.xyw = r8.xyz * 2.0 - 1.0;
    ps = r11.y + r11.y;
    r13.xy = r5.xy * ScreenPositionScaleBias.xy;
    r16.x = ps;
    ps = r11.z + r11.z;
    r16.zw = r4.xy + r4.xy;
    r16.y = ps;
    ps = r9.x + r9.x;
    r16 = r16 - 1.0;
    r18.y = ps;
    ps = r9.y + r9.y;
    r18.x = dot(r6.xww, float3(1.0, 1.0, 1.0));
    r18.z = ps;
    r6.x = (r18.x == 0.0) ? r17.x : r17.y;
    r18 = r18.xxyz + float4(-3.0, -2.0, -1.0, -1.0);
    r0.zw = r16.xy * 0.5 + r18.zw;
    r6.x = (r18.y == 0.0) ? r17.z : r6.x;
    r6.x = (r18.x == 0.0) ? r17.w : r6.x;
    ps = r5.w;
    r6.w = max(r6.x, 0.0);
    ps = 0.0001 * ps;
    r6.xz = -r6.xz + 1.0;
    r17.z = saturate(ps);
    ps = ModShadowGroupColor.x * r6.z;
    r15.x = min(r6.w, 0.3);
    r17.x = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r15.xyz = r15.xyz * float3(3.3333333, 0.875, 0.875);
    r17.y = ps;
    r11.yzw = -r17.yxz + 1.0;
    ps = 1.0 / r5.w;
    r6.z = r11.w * UniformScalar_1.x;
    r6.y = ps;
    r6.zw = r6.zz * r16.zw + r8.xy;
    r6.zw = r6.zw * UniformVector_4.xy - r7.xy;
    r13.zw = r15.xx * r0.zw + r6.zw;
    r6.yz = r13.xy * r6.yy + ScreenPositionScaleBias.wz;
    r4.xyz = tex2D(LightAttenuationTexture, r6.yz).xyz;
    r5.z = tex2D(Texture2D_5, r1.xy).x;
    r5.w = tex2D(ShadowTexture, r0.xy).x;
    r9.xyz = tex2D(Texture2D_7, r14.xy).xyz;
    ps = UniformVector_6.x * UniformVector_6.w;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r1.x = ps;
    ps = UniformVector_6.y * UniformVector_6.w;
    r6.z = dot(r3.zxy, r3.zxy);
    r1.y = ps;
    ps = UniformVector_6.z * UniformVector_6.w;
    r6.y = dot(r2.zxy, r2.zxy);
    r1.z = ps;
    r11.x = r11.x * r9.w - 1.0;
    ps = rsqrt(abs(r6.y));
    r15.w = r4.w - 1.0;
    r6.y = ps;
    ps = 1.0 - r6.z;
    r8.xyz = -ModShadowColor.xyz + 1.0;
    r6.z = saturate(ps);
    r12.w = r8.w * UniformVector_4.z - r7.w;
    ps = log2(r6.z);
    r1.xyz = r1.xyz * r12.xyz;
    r6.z = ps;
    ps = 2.5 * r6.x;
    r3.xyz = r6.yyy * r2.xyz;
    r14.w = ps;
    r2 = r15 * r11.xzyw + float4(1.0, 0.125, 0.125, 1.0);
    ps = r2.y * r2.z;
    r14.xyz = r1.xyz * r2.www;
    r6.x = ps;
    r8.xyz = r6.xxx * r8.xyz + ModShadowColor.xyz;
    r1.xyz = r14.xyz * UniformScalar_11.xxx + UniformScalar_12.xxx;
    r12.xyz = r14.xyz * r2.xxx - r9.xyz;
    r8.w = saturate(r14.w * r2.x - 0.5);
    r6.xy = r13.zw * r8.ww + r7.xy;
    r9.xyz = r12.xyz * r8.www + r9.xyz;
    r6.w = r12.w * r8.w - 2.0;
    r6.w = r7.z * 2.0 + r6.w;
    r7.xyz = r6.wxy * r11.www;
    ps = 1.0 + r7.x;
    r7.w = ps;
    r6.xyw = (r11.www > 0.0) ? r7.yzw : float3(0.0, 0.0, 1.0);
    r7.xyz = (r11.www >= 0.0) ? r6.xyw : float3(0.0, 0.0, 1.0);
    r6.x = dot(r7.zxy, r7.zxy);
    ps = rsqrt(abs(r6.x));
    r0.xyz = r9.xyz * r0.xyz;
    r6.x = ps;
    r7.xyz = r7.xyz * r6.xxx;
    r6.x = dot(r7.zxy, r10.zxy);
    r6.xyw = r7.xyz * r6.xxx;
    r6.xyw = r6.xyw * 2.0 - r10.xyz;
    r6.x = saturate(dot(r3.zxy, r6.wxy));
    ps = log2(r6.x);
    r6.w = saturate(dot(r7.zyx, r3.zyx));
    r6.y = ps;
    ps = UniformScalar_9.x * r6.y;
    r7.x = ps;
    ps = pow(2.0, r7.x);
    r6.x = r6.y * 15.0;
    r6.y = ps;
    ps = abs(r2.x) * abs(r2.x);
    r7.x = r6.y * UniformScalar_10.x;
    r6.y = ps;
    ps = r6.y * r6.y;
    r5.x = r7.x * r2.w;
    r5.y = ps;
    r7.xyw = r5.wxy * r5.wzy;
    ps = LightColorAndFalloffExponent.w * r6.z;
    r1.xyz = r1.xyz + r7.yyy;
    r6.y = ps;
    ps = pow(2.0, r6.y);
    r2.xyz = r7.xxx * r4.xyz;
    r6.y = ps;
    r7.xyz = r2.xzy * r6.yyy;
    r1.xyz = r7.www * r1.xyz;
    ps = pow(2.0, r6.x);
    r1.xyz = r1.xyz * r8.www;
    r6.x = ps;
    r6.xyz = r1.xyz * r6.xxx;
    r6.xy = r0.xy * r6.ww + r6.xy;
    r6.z = r0.z * r6.w + r6.z;
    r6.xyz = r7.xzy * r6.xyz;
    r6.xyz = r6.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r6.xzy * r8.xzy;
    oC0.xyz = r6.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
