// ps_aafb612e0486cbf2.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 159 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 0000027C 10040C00 0000070A 00000000 000070E7 001F007F 00000001 0000F050 0000F154 0000F256 0000F357 0000F458 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c10); // float4
float4 ConstantLighting : register(c11); // float3
float4 LowerSkyColor : register(c9); // float3
float4 ModShadowAccumResolution : register(c14); // float2
float4 ModShadowColor : register(c12); // float3
float4 ModShadowGroupColor : register(c13); // float3
float4 OpacityOverride : register(c7); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_3 : register(c6); // float4
float4 UpperSkyColor : register(c8); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D ModShadowAccumTexture : register(s1);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord6 : TEXCOORD6; // r2
    float4 texcoord7 : TEXCOORD7; // r3
    float4 texcoord8 : TEXCOORD8; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord6;
    float4 r3 = In.texcoord7;
    float4 r4 = In.texcoord8;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r5.yz = r0.xy * 2.0 + UniformVector_1.xy;
    r6.xy = r0.xy * 0.5 + UniformVector_2.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r5.x = dot(r1.zxy, r1.zxy);
    r6.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r8.xyz = AmbientColorAndSkyFactor.xyz * UniformVector_3.xyz;
    r6.w = ps;
    r6.zw = r6.zw * abs(r7.xy);
    r9.zw = tex2D(ModShadowAccumTexture, r6.zw).xy;
    r12.xyz = tex2D(Texture2D_0, r6.xy).xyz;
    r6.xyz = tex2D(Texture2D_0, r5.yz).xyz;
    ps = OpacityOverride.x;
    r5.y = dot(r4.zxy, r4.zxy);
    r10.w = saturate(ps);
    ps = ConstantLighting.z;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    ps = 0.21952 * ps;
    r5.z = dot(r3.zxy, r3.zxy);
    r10.x = ps;
    r6.xyz = r6.zxy * 4.0 - 4.0;
    ps = rsqrt(abs(r5.z));
    r7.xyz = r0.xyz * UniformVector_3.xyz;
    r5.z = ps;
    ps = rsqrt(abs(r5.y));
    r11.xyz = r5.zzz * r3.xyz;
    r5.y = ps;
    ps = rsqrt(abs(r5.x));
    r5.yzw = r5.yyy * r4.xyz;
    r5.x = ps;
    ps = -ModShadowColor.x;
    r3.xyz = r5.xxx * r1.xyz;
    r6.yzw = r12.zxy * 4.0 + r6.xyz;
    ps = 1.0 + ps;
    r6.x = dot(r6.yzw, r6.yzw);
    r9.x = ps;
    ps = rsqrt(abs(r6.x));
    r5.x = r3.z + 0.1;
    r6.x = ps;
    ps = 5.0 * r5.x;
    r6.xyw = r6.zwy * r6.xxx;
    r5.x = saturate(ps);
    ps = 1.0 - r5.x;
    r9.y = dot(r5.wyz, r6.wxy);
    r5.z = ps;
    ps = ModShadowGroupColor.x * r5.z;
    r5.x = dot(r6.wxy, r11.zxy);
    r10.y = ps;
    ps = ModShadowGroupColor.y * r5.z;
    r5.xyw = r6.xyw * r5.xxx;
    r10.z = ps;
    r1.xyz = r5.xyw * 2.0 - r11.xyz;
    ps = -ModShadowColor.y;
    r5.xyw = -r10.yzw + 1.0;
    ps = 1.0 + ps;
    r4 = r9.yyzw * float4(-0.5, 0.5, 0.875, 0.875);
    r9.y = ps;
    ps = -ModShadowColor.z;
    r5.w = float((r5.w >= 0.004));
    r10.yz = r4.zw * r5.xy + 0.125;
    ps = 1.0 + ps;
    r5.xz = r4.xy + 0.5;
    r9.z = ps;
    ps = ConstantLighting.x * r6.w;
    r5.y = saturate(dot(r3.zxy, r1.zxy));
    r4.x = ps;
    ps = log2(r5.y);
    r5.xz = abs(r5.xz) * abs(r5.xz);
    r5.y = ps;
    ps = ConstantLighting.y * r6.w;
    r5.y = r5.y * 15.0;
    r4.y = ps;
    ps = pow(2.0, r5.y);
    r1.xyz = r7.xzy * r5.zzz;
    r10.w = ps;
    ps = ConstantLighting.z * r6.w;
    r5.z = r10.w * 0.21952;
    r4.z = ps;
    ps = ConstantLighting.x * r5.z;
    r3.xyz = r7.xzy * r5.xxx;
    r6.x = ps;
    ps = ConstantLighting.y * r5.z;
    r6.zw = r10.xy * r10.wz;
    r6.y = ps;
    r5.xyz = r6.www * r9.xyz + ModShadowColor.xyz;
    r6.xyz = r4.xyz * r7.xyz + r6.xyz;
    r6.xyz = r3.xzy * LowerSkyColor.xyz + r6.xyz;
    r6.xyz = r1.xzy * UpperSkyColor.xyz + r6.xyz;
    r6.xyz = r8.xyz * r0.xyz + r6.xyz;
    r6.xyz = r6.xyz + UniformVector_0.xyz;
    ps = -r2.w;
    r5.xyz = r6.xyz * r5.xyz;
    ps = OpacityOverride.x + ps;
    r6.xyz = r5.xyz - r5.xyz;
    r6.w = ps;
    oC0.w = r6.w * r5.w + r2.w;
    r5.xyz = r6.xyz * r5.www + r5.xyz;
    oC0.xyz = r5.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
