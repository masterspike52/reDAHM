// ps_cfda52c191422be4.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 432 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000006C0 10041700 0000080A 00000000 00007908 003F00FF 00000021 00003050 0000F151 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c19); // float4
float4 LightMapScale : register(c20); // float3
float4 LowerSkyColor : register(c18); // float3
float4 ModShadowAccumResolution : register(c23); // float2
float4 ModShadowColor : register(c21); // float3
float4 ModShadowGroupColor : register(c22); // float3
float4 OpacityOverride : register(c16); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_0 : register(c7); // float
float4 UniformScalar_1 : register(c8); // float
float4 UniformScalar_10 : register(c15); // float
float4 UniformScalar_4 : register(c9); // float
float4 UniformScalar_5 : register(c10); // float
float4 UniformScalar_6 : register(c11); // float
float4 UniformScalar_7 : register(c12); // float
float4 UniformScalar_8 : register(c13); // float
float4 UniformScalar_9 : register(c14); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_3 : register(c6); // float4
float4 UpperSkyColor : register(c17); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
sampler2D LightMapTexture : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 r21 = 0.0;
    float4 r22 = 0.0;
    float4 r23 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r12 = r1.wzwz * float4(6.0, 6.0, 4.0, 4.0);
    r21.xyz = tex2D(Texture2D_3, r12.xy).xyw;
    ps = r1.w;
    r6.w = -r1.z + 1.0;
    ps = UniformVector_1.x * ps;
    r6.y = r3.w - 4e+02;
    r10.x = ps;
    ps = r1.w;
    r13.xy = r1.xy * UniformScalar_0.xx;
    ps = UniformVector_3.x * ps;
    r6.x = dot(r4.zxy, r4.zxy);
    r11.x = ps;
    ps = rsqrt(abs(r6.x));
    r21.w = saturate(r6.y * 0.00022222222);
    r6.x = ps;
    ps = r1.z;
    r14.xyz = r6.xxx * r4.xyz;
    ps = UniformVector_3.y * ps;
    r20.xw = r21.xw * float2(4.0, -0.125);
    r11.y = ps;
    r6.xz = r14.xy * r20.ww + r1.wz;
    ps = r1.x;
    r6.y = -r6.z + 1.0;
    ps = 3.0 * ps;
    r10.yzw = r6.wyx * UniformVector_1.yyx;
    r9.x = ps;
    ps = r1.y;
    r4 = r10.wzxy - 0.5;
    ps = 3.0 * ps;
    r23 = float4((r10.wzxy >= 0.5));
    r9.y = ps;
    ps = 2.5 * r6.x;
    r4 = r4 - r10.wzxy;
    r0.z = ps;
    r4 = r4.zwxy * r23.zwxy + r10.xywz;
    ps = 2.5 * r6.z;
    r4 = r4.xywz + r4.xywz;
    r0.w = ps;
    r17.xyz = tex2D(Texture2D_6, r9.xy).xyz;
    r10.xyz = tex2D(Texture2D_4, r1.xy).xyz;
    r9 = tex2D(Texture2D_2, r4.xy);
    r22 = tex2D(Texture2D_2, r4.wz);
    r11.xyz = tex2D(Texture2D_7, r11.xy).xyz;
    r15.yzw = tex2D(Texture2D_0, r13.xy).xyw;
    r4 = tex2D(Texture2D_3, r1.wz).zxyw;
    r6.x = tex2D(Texture2D_3, r12.zw).z;
    r13.xyz = tex2D(Texture2D_1, r0.zw).xyz;
    ps = UniformScalar_8.x - r7.w;
    r12.y = ps;
    ps = OpacityOverride.x;
    r17.w = saturate(r7.w * 1e+01);
    r6.w = saturate(ps);
    ps = r3.w;
    r1.x = float((UniformScalar_4.x >= 1.0));
    ps = 0.0001 * ps;
    r1.w = float((UniformScalar_4.x > 1.0));
    r6.z = saturate(ps);
    ps = r13.z + r13.z;
    r6.y = dot(r2.zxy, r2.zxy);
    r7.x = ps;
    r19.z = r4.y * 4.0 - 4.0;
    ps = rsqrt(abs(r6.y));
    r1.yz = r15.yz + r15.yz;
    r6.y = ps;
    r11.xyz = (-abs(r1.www) >= 0.0) ? r11.xyz : 1.0;
    r16.xyz = (-abs(r1.xxx) >= 0.0) ? 1.0 : r11.xyz;
    ps = r4.z + r4.z;
    r2.xyz = r6.yyy * r2.xzy;
    r1.x = ps;
    ps = 0.1 - -r2.y;
    r11.xzw = r1.xyz + float3(-2.0, -1.0, -1.0);
    r6.y = ps;
    r18.w = r21.y * 2.0 + r11.x;
    ps = r6.x;
    r6.y = saturate(r6.y * 5.0);
    ps = r7.w * ps;
    r1.z = dot(r23.zww, float3(1.0, 1.0, 1.0));
    r1.x = ps;
    ps = r21.z;
    r1.w = dot(r23.xyy, float3(1.0, 1.0, 1.0));
    r1.y = (r1.w == 0.0) ? r22.x : r22.y;
    r6.x = (r1.z == 0.0) ? r9.x : r9.y;
    ps = r4.w * ps;
    r21 = r1.zzww + float4(-3.0, -2.0, -2.0, -3.0);
    r4.y = ps;
    r6.x = (r21.y == 0.0) ? r9.z : r6.x;
    r1.y = (r21.z == 0.0) ? r22.z : r1.y;
    r19.y = (r21.w == 0.0) ? r22.w : r1.y;
    r19.w = (r21.x == 0.0) ? r9.w : r6.x;
    ps = (-1.0) - -r4.y;
    r6.x = max(r19.w, 0.0);
    r18.x = ps;
    ps = 1.0 - r6.w;
    r6.x = min(r6.x, 0.3);
    r1.w = ps;
    ps = 1.0 - r6.y;
    r19.x = r6.x * 3.3333333;
    r1.z = ps;
    ps = 1.0 - r6.z;
    r9.xyz = -r19.xyw + 1.0;
    r1.y = ps;
    ps = ModShadowGroupColor.x * r1.z;
    r20.yz = r17.ww * r9.xy;
    r4.z = ps;
    ps = r20.z;
    r6.yzw = UniformVector_2.xyz * UniformVector_2.www;
    ps = r19.y + ps;
    r18.yz = r19.zx + r20.xy;
    r10.w = ps;
    r10 = r10.zxyw * 2.0 + float4(-1.0, -1.0, -1.0, -0.75);
    ps = ModShadowGroupColor.y * r1.z;
    r9.xyw = r18.xzy * r18.zwz;
    r4.w = ps;
    ps = r10.y;
    r12.z = float((UniformScalar_7.x >= 1.0));
    r9.y = r9.y * 2.0 + r10.z;
    ps = r9.w + ps;
    r6.x = r9.x + 1.0;
    r9.x = ps;
    ps = abs(r6.x) * abs(r6.x);
    r17.xyz = r6.yzw * r17.xyz;
    r12.x = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.y = r6.x * r9.z;
    r6.x = ps;
    r16.w = r6.y * r4.x - r6.y;
    ps = 1.0 / ModShadowAccumResolution.y;
    r11.xy = r12.xy * r12.xz;
    r6.z = ps;
    ps = r11.y;
    r12.xy = r6.xz * abs(r8.xy);
    ps = r7.w + ps;
    r8 = r17 * r16;
    r6.x = ps;
    ps = r6.y;
    r6.x = -r6.x + r7.z;
    ps = r8.w + ps;
    r6.x = max(r6.x, UniformScalar_9.x);
    r13.w = ps;
    ps = 1.0 / UniformScalar_10.x;
    r7.yzw = r13.xwy * float3(4.0, 2.5, 4.0);
    r6.w = ps;
    ps = (-0.5) + r7.z;
    r4.x = min(r6.x, 1.0);
    r2.w = saturate(ps);
    ps = r4.x;
    r6.xyz = r7.xyw + float3(-1.0, -2.0, -2.0);
    ps = r6.w * ps;
    r9.yz = -r6.yz + r9.xy;
    r4.x = saturate(ps);
    ps = 0.5 * r6.x;
    r13 = -r4.yxzw + 1.0;
    r4.z = ps;
    ps = r10.x;
    r6.x = r1.x * r13.y;
    ps = -r4.z + ps;
    r6.y = float((r6.x >= 0.05));
    r9.x = ps;
    ps = 1.0 - r6.y;
    r16.yzw = r9.xyz * r2.www;
    r6.x = ps;
    r6.y = (UniformScalar_8.x > 0.0) ? r6.x : 1.0;
    r16.x = (UniformScalar_8.x >= 0.0) ? r6.y : r6.x;
    r9.xzw = r16.xzw + float3(-0.5, -2.0, -2.0);
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r9.xxxx)) clip(-1.0);
    r12.xy = tex2D(ModShadowAccumTexture, r12.xy).xy;
    r17.xyz = tex2D(Texture2D_5, r0.zw).xyz;
    r0.xyz = tex2D(LightMapTexture, r0.xy).xyz;
    r10.xyz = -ModShadowColor.xyz + 1.0;
    r6.y = dot(r5.zxy, r5.zxy);
    r6.xzw = -UniformVector_0.yxz + 1.0;
    ps = rsqrt(abs(r6.y));
    r4.xyw = r0.xzy * LightMapScale.xzy;
    r6.y = ps;
    ps = AmbientColorAndSkyFactor.x * r6.z;
    r5.xyz = r6.yyy * r5.xyz;
    r9.x = ps;
    ps = AmbientColorAndSkyFactor.y * r6.x;
    r0.x = float((r1.w >= 0.004));
    r9.y = ps;
    r6.y = saturate(r13.x * 0.2 + r10.w);
    ps = UniformScalar_1.x * r1.y;
    r0.yzw = r6.yyy * r17.xyz;
    r1.x = ps;
    ps = r0.y;
    r15.x = r4.z + r16.y;
    ps = r15.w * ps;
    r12.zw = r15.xw - 1.0;
    r0.y = ps;
    r16.zw = r12.zw * r1.yy + 1.0;
    r9.zw = r1.xx * r11.zw + r9.zw;
    ps = r0.z;
    r7.xy = r9.zw + r7.yw;
    ps = r15.w * ps;
    r16.xy = r7.xy * r1.yy;
    r0.z = ps;
    r7 = (r1.xyyy > 0.0) ? r16.wxyz : float4(1.0, 0.0, 0.0, 1.0);
    r7 = (r1.yyyx >= 0.0) ? r7.yzwx : float4(0.0, 0.0, 1.0, 1.0);
    ps = r0.w;
    r6.y = dot(r7.zxy, r7.zxy);
    ps = r15.w * ps;
    r1.xyz = r8.xyz * r7.www;
    r0.w = ps;
    r8.xyz = r1.xyz * UniformScalar_5.xxx + UniformScalar_6.xxx;
    ps = rsqrt(abs(r6.y));
    r1.xyz = r1.xyz - r0.yzw;
    r6.y = ps;
    r0.yzw = r1.xyz * r2.www + r0.yzw;
    ps = AmbientColorAndSkyFactor.z * r6.w;
    r7.xyw = r7.xyz * r6.yyy;
    r9.z = ps;
    r9.xyz = r9.xyz * r0.yzw + UniformVector_0.xyz;
    r12.z = dot(r5.zxy, r7.wxy);
    r6.y = dot(r7.wxy, r14.zxy);
    ps = r4.x;
    r1.xyz = r7.xyw * r6.yyy;
    r5.xyz = r1.xyz * 2.0 - r14.xyz;
    ps = r7.w * ps;
    r12 = r12.xyzz * float4(0.875, 0.875, -0.5, 0.5);
    r1.x = ps;
    r11.zw = r12.xy * r13.zw + 0.125;
    ps = r4.y;
    r1.zw = r12.zw + 0.5;
    ps = r7.w * ps;
    r6.y = saturate(dot(r2.yxz, r5.zxy));
    r1.y = ps;
    ps = log2(r6.y);
    r7.xy = abs(r1.zw) * abs(r1.zw);
    r6.y = ps;
    ps = 15.0 * r6.y;
    r0.yzw = r0.yzw * r6.zxw;
    r6.w = ps;
    ps = r11.x * r11.x;
    r5.xyz = r0.ywz * r7.xxx;
    r1.z = ps;
    ps = r11.z * r11.w;
    r2.xyz = r0.ywz * r7.yyy;
    r6.x = ps;
    r6.xyz = r6.xxx * r10.xyz + ModShadowColor.xyz;
    r7.xyz = r2.xyz * UpperSkyColor.xzy + r9.xzy;
    ps = pow(2.0, r6.w);
    r2.xyz = r1.zzz * r8.xyz;
    r6.w = ps;
    ps = r4.w;
    r4.xyz = r4.xyw * r6.www;
    ps = r7.w * ps;
    r2.xyz = r2.xzy * r2.www;
    r1.z = ps;
    r7.xyz = r5.xzy * LowerSkyColor.xyz + r7.xzy;
    r7.xyz = r4.xyz * r2.xyz + r7.xzy;
    r7.xyz = r1.xzy * r0.yzw + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xyz * r6.xyz;
    ps = OpacityOverride.x + ps;
    r7.xyz = r6.xyz - r6.xyz;
    r7.w = ps;
    oC0.w = r7.w * r0.x + r3.w;
    r6.xyz = r7.xyz * r0.xxx + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
