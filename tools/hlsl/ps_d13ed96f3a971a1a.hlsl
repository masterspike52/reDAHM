// ps_d13ed96f3a971a1a.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 168 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002A0 10040D00 00000506 00000000 000040A5 001F001F 00000001 00003050 00007154 0000F255 00007356 0000F457
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c4); // float3
float4 ModShadowAccumResolution : register(c7); // float2
float4 ModShadowColor : register(c5); // float3
float4 ModShadowGroupColor : register(c6); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformVector_0 : register(c3); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_6 : register(s4);
sampler2D Texture2D_7 : register(s5);
sampler2D Texture2D_8 : register(s6);
sampler2D Texture2D_9 : register(s7);
samplerCUBE TextureCube_0 : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float2 vPos : VPOS;   // r5 (pixel parameters)
    float vFace : VFACE;  // r5
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r6 = 0.0;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r2.xyz = tex2D(Texture2D_0, r0.xy).xyz;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.y = ps;
    ps = 1.0 / r4.w;
    r0.z = r1.z + 0.1;
    r1.w = ps;
    ps = 5.0 * r0.z;
    r0.w = dot(r3.zxy, r3.zxy);
    r0.z = saturate(ps);
    r8.xyz = r2.xyz * 2.0 - 1.0;
    ps = rsqrt(abs(r0.w));
    r2.xy = r1.ww * ScreenPositionScaleBias.xy;
    r0.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.xyz = -UniformVector_0.xyz + 1.0;
    r7.x = ps;
    r4.xy = r2.xy * r4.xy + ScreenPositionScaleBias.wz;
    ps = r7.x;
    r2.xyz = r0.www * r3.xyz;
    ps = abs(r5.x) * ps;
    r1.w = dot(r8.zxy, r8.zxy);
    r3.x = ps;
    ps = rsqrt(abs(r1.w));
    r0.w = saturate(dot(r2.zxy, r8.zxy));
    r1.w = ps;
    ps = r7.y;
    r10.xyz = r8.xyz * r1.www;
    ps = abs(r5.y) * ps;
    r1.w = dot(r10.zxy, r2.zxy);
    r3.y = ps;
    ps = 1.0 - r0.z;
    r5.xyz = r10.xzy * r1.www;
    r5.w = ps;
    r9.xyz = r5.xyz * 2.0 - r2.xzy;
    ps = 1.0 - r0.w;
    r2 = xe_cube(r9.xzy);
    r0.z = ps;
    ps = 1.0 / abs(r2.z);
    r5.z = r2.w;
    r0.w = ps;
    r5.xy = r2.yx * r0.ww + 1.5;
    r8.xyz = tex2D(Texture2D_6, r0.xy).xyz;
    r11.xyz = tex2D(Texture2D_7, r0.xy).xyz;
    r13.z = tex2D(Texture2D_9, r0.xy).x;
    r12.xyz = texCUBE(TextureCube_0, xe_cube_dir(r5.xyz)).xyz;
    r13.xyw = tex2D(Texture2D_1, r0.xy).yzx;
    r2.xyz = tex2D(LightAttenuationTexture, r4.xy).zxy;
    r3.xy = tex2D(ModShadowAccumTexture, r3.xy).xy;
    r4.xyz = tex2D(Texture2D_2, r0.zz).yxz;
    r0.xyz = tex2D(Texture2D_8, r0.xy).yxz;
    ps = 0.7 * r0.y;
    r5.xyz = -ModShadowColor.xyz + 1.0;
    r1.w = ps;
    ps = 2e+01 * r4.y;
    r0.w = dot(r1.zxy, r1.zxy);
    r2.w = ps;
    ps = 0.875 * r3.x;
    r13.xy = r13.xy * r4.xz;
    r7.z = ps;
    ps = 0.875 * r3.y;
    r0.xy = r12.yz * r0.xz;
    r7.w = ps;
    ps = LightColor.x * r2.y;
    r4.x = r2.w * r13.w;
    r3.x = ps;
    ps = rsqrt(abs(r0.w));
    r12.x = r1.w * r12.x;
    r0.z = ps;
    ps = 0.65 * r0.x;
    r3.yzw = r0.zzz * r1.xyz;
    r12.y = ps;
    ps = 0.5 * r0.y;
    r4.yzw = r13.xyz * float3(2e+01, 2e+01, 1e+01);
    r12.z = ps;
    ps = r4.w;
    r1.yzw = r12.xyz + r11.xyz;
    r0.y = ps;
    ps = 0.0001;
    r0.w = saturate(dot(r10.zxy, r3.wyz));
    r0.z = ps;
    r7.xy = -r5.ww * ModShadowGroupColor.xy + 1.0;
    r7.yz = r7.zw * r7.xy + 0.125;
    ps = max(r0.y, r0.z);
    r0.x = saturate(dot(r3.wyz, r9.yxz));
    r7.x = ps;
    ps = log2(r0.x);
    r4.xyz = r4.xyz + r8.xyz;
    r7.w = ps;
    ps = LightColor.z * r2.x;
    r3.zw = r7.xy * r7.wz;
    r3.y = ps;
    r0.xyz = r3.www * r5.xyz + ModShadowColor.xyz;
    ps = pow(2.0, r3.z);
    r4.xyz = r4.xyz * r6.xyz;
    r1.x = ps;
    ps = LightColor.y * r2.z;
    r1.xyz = r1.ywz * r1.xxx;
    r3.z = ps;
    r1.y = r4.z * r0.w + r1.y;
    r1.xz = r4.xy * r0.ww + r1.xz;
    r1.xyz = r3.xyz * r1.xyz;
    r0.xyz = r1.xyz * r0.xzy;
    oC0.xyz = r0.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
