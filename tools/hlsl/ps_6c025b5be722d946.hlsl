// ps_6c025b5be722d946.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 120 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000001E0 10040800 00000806 00000000 00007108 00FF00FF 00000021 00003050 00003151 0000F252 0000F353 0000F454 0000F556 0000F657 0000F758
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD2 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD3 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r6 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r7 <-> TEXCOORD8 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 ModShadowAccumResolution : register(c10); // float2
float4 ModShadowColor : register(c8); // float3
float4 ModShadowGroupColor : register(c9); // float3
float4 OpacityOverride : register(c7); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_3 : register(c6); // float4
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D ModShadowAccumTexture : register(s2);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord2 : TEXCOORD2; // r2
    float4 texcoord3 : TEXCOORD3; // r3
    float4 texcoord4 : TEXCOORD4; // r4
    float4 texcoord6 : TEXCOORD6; // r5
    float4 texcoord7 : TEXCOORD7; // r6
    float4 texcoord8 : TEXCOORD8; // r7
    float2 vPos : VPOS;   // r8 (pixel parameters)
    float vFace : VFACE;  // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord2;
    float4 r3 = In.texcoord3;
    float4 r4 = In.texcoord4;
    float4 r5 = In.texcoord6;
    float4 r6 = In.texcoord7;
    float4 r7 = In.texcoord8;
    float4 r8 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 oC0 = 0.0;
    float ps = 0.0;

    r0.zw = r0.xy * 0.2 + UniformVector_1.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r2.xyz = -ModShadowColor.xyz + 1.0;
    r1.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.xz = r0.yx - 0.5;
    r1.y = ps;
    r1.xy = r1.xy * abs(r8.xy);
    r1.xy = tex2D(ModShadowAccumTexture, r1.xy).xy;
    r1.z = tex2D(Texture2D_0, r0.zw).x;
    r0.w = dot(r6.zxy, r6.zxy);
    r0.z = dot(r4.zxy, r4.zxy);
    r7.y = dot(r7.xz, UniformVector_3.yx) + 0.5;
    r7.x = dot(r7.xz, UniformVector_2.yx) + 0.5;
    ps = rsqrt(abs(r0.z));
    r8.z = r1.z * 0.25;
    r0.z = ps;
    ps = rsqrt(abs(r0.w));
    r7.zw = r1.xy * 0.875;
    r1.w = ps;
    r0.w = r0.z * r4.z + 0.1;
    ps = 5.0 * r0.w;
    r0.z = saturate(ps);
    ps = OpacityOverride.x;
    r0.z = -r0.z + 1.0;
    r1.x = saturate(ps);
    ps = r1.w;
    r1.yz = r0.zz * ModShadowGroupColor.xy;
    ps = r6.x * ps;
    r1.xyz = -r1.xyz + 1.0;
    r0.z = ps;
    ps = r1.w;
    r8.xy = r7.zw * r1.yz;
    ps = r6.y * ps;
    r4.xyz = r8.xyz + float3(0.125, 0.125, -0.005);
    r0.w = ps;
    r0.xy = r0.zw * r4.zz + r0.xy;
    r6.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r6.w = tex2D(Texture2D_1, r7.xy).x;
    r0 = r6.zxyw * r3.zxyx;
    r1.w = r0.w * r6.x;
    r1.w = min(r1.w, 1.0);
    ps = r0.y;
    r1.w = r1.w - 0.5;
    r0.y = ps;
    ps = UniformVector_0.x + r0.y;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r1.wwww)) clip(-1.0);
    r1.y = ps;
    ps = UniformVector_0.y + r0.z;
    r1.x = float((r1.x >= 0.004));
    r1.z = ps;
    ps = UniformVector_0.z + r0.x;
    r2.w = r4.x * r4.y;
    r1.w = ps;
    r0.xyz = r2.www * r2.xyz + ModShadowColor.xyz;
    ps = -r5.w;
    r0.xyz = r1.yzw * r0.xyz;
    ps = OpacityOverride.x + ps;
    r2.xyz = r0.xyz - r0.xyz;
    r2.w = ps;
    oC0.w = r2.w * r1.x + r5.w;
    r0.xyz = r2.xyz * r1.xxx + r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
