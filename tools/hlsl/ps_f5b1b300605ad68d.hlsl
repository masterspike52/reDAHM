// ps_f5b1b300605ad68d.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 213 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000354 10040D00 0000080A 00000000 00007108 003F00FF 00000021 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c10); // float3
float4 ModShadowAccumResolution : register(c13); // float2
float4 ModShadowColor : register(c11); // float3
float4 ModShadowGroupColor : register(c12); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_3 : register(c7); // float
float4 UniformScalar_4 : register(c8); // float
float4 UniformScalar_5 : register(c9); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_2 : register(c4); // float4
float4 UniformVector_3 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D ShadowTexture : register(s5);
sampler2D ModShadowAccumTexture : register(s6);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r10 = tex2D(Texture2D_2, r1.xy);
    ps = 1.0 / r5.w;
    r6.yz = r5.xy * ScreenPositionScaleBias.xy;
    r6.x = ps;
    r7.zw = r6.yz * r6.xx + ScreenPositionScaleBias.wz;
    ps = r10.w;
    r6.xz = r1.wz * UniformVector_4.xy;
    r6.y = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r12.xy = r10.yz * UniformScalar_4.xx;
    r7.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r11.xyz = -UniformVector_0.xyz + 1.0;
    r7.y = ps;
    ps = (-0.5) + r6.y;
    r7.xy = r7.xy * abs(r8.xy);
    r6.y = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.yyyy)) clip(-1.0);
    r3.xyz = tex2D(LightAttenuationTexture, r7.zw).xyz;
    r8.z = tex2D(ShadowTexture, r0.xy).x;
    r8.yw = tex2D(ModShadowAccumTexture, r7.xy).xy;
    r13 = tex2D(Texture2D_1, r1.xy);
    r6.yzw = tex2D(Texture2D_3, r6.xz).xyz;
    r9.xyz = tex2D(Texture2D_0, r1.xy).xyz;
    r0.x = dot(r2.zxy, r2.zxy);
    r0.y = dot(r4.zxy, r4.zxy);
    r1.xyz = UniformVector_3.xyz * 2e+01;
    r7.y = float((UniformScalar_3.x >= 1.0));
    r7.x = float((UniformScalar_3.x > 1.0));
    r9.xyz = r9.xyz * 2.0 - 1.0;
    r6.yzw = (-abs(r7.xxx) >= 0.0) ? r6.yzw : 1.0;
    r7.xzw = r13.xyz * r13.www;
    r7.xzw = r1.xyz * r7.xzw;
    r1.xyz = (-abs(r7.yyy) >= 0.0) ? 1.0 : r6.yzw;
    r6.yzw = r1.xyz * r10.xyz + r7.xzw;
    ps = rsqrt(abs(r0.y));
    r7.y = r1.x * UniformScalar_4.x;
    r0.w = ps;
    ps = rsqrt(abs(r0.x));
    r7.y = r7.y * r10.x;
    r1.w = ps;
    r0.xyz = -ModShadowColor.xyz + 1.0;
    r10.xyz = r9.xyz * UniformVector_2.xyz;
    ps = 0.1 - -r2.z;
    r2.xyz = r1.www * r2.xyz;
    r6.x = ps;
    ps = r5.w;
    r9.xyz = r0.www * r4.xyz;
    ps = 0.0001 * ps;
    r0.w = dot(r7.wxz, float3(0.11, 0.3, 0.59));
    r10.w = saturate(ps);
    ps = r0.w;
    r7.zw = r12.xy * r1.yz;
    ps = -r7.x + ps;
    r4.xy = -r10.zw + 1.0;
    r0.w = ps;
    r5.x = saturate(r0.w + r7.x);
    ps = r4.x;
    r1.xyz = r6.yzw * r11.xyz;
    ps = r5.x * ps;
    r6.yz = -r10.xy * r5.xx;
    r6.w = ps;
    r10.xyz = r10.xyz + r6.yzw;
    ps = 5.0 * r6.x;
    r10.w = r10.z - 1.0;
    r6.x = saturate(ps);
    ps = 1.0 - r6.x;
    r10.xyw = r10.xyw * r4.yyy;
    r6.z = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r10.z = r10.w + 1.0;
    r5.y = ps;
    r6.xyw = (r4.yyy > 0.0) ? r10.xyz : float3(0.0, 0.0, 1.0);
    r4.xyz = (r4.yyy >= 0.0) ? r6.xyw : float3(0.0, 0.0, 1.0);
    ps = ModShadowGroupColor.y * r6.z;
    r6.x = dot(r4.zxy, r4.zxy);
    r5.z = ps;
    ps = rsqrt(abs(r6.x));
    r5.xyz = -r5.xyz + 1.0;
    r6.x = ps;
    r4.xyz = r4.xyz * r6.xxx;
    ps = UniformScalar_5.x + r7.y;
    r6.x = dot(r4.zxy, r9.zxy);
    r6.y = ps;
    ps = UniformScalar_5.x + r7.z;
    r10.xyz = r4.xyz * r6.xxx;
    r6.z = ps;
    r9.xyz = r10.xyz * 2.0 - r9.xyz;
    ps = UniformScalar_5.x + r7.w;
    r6.x = saturate(dot(r2.zxy, r9.zxy));
    r6.w = ps;
    ps = log2(r6.x);
    r7.xyz = r6.yzw * r5.xxx;
    r8.x = ps;
    r6.xyw = r8.xwy * float3(15.0, 0.875, 0.875);
    r8.xy = r6.wy * r5.yz + 0.125;
    ps = pow(2.0, r6.x);
    r6.w = saturate(dot(r4.zyx, r2.zyx));
    r6.x = ps;
    r6.xyz = r7.xyz * r6.xxx;
    r7.xy = r1.xy * r6.ww + r6.xy;
    r7.z = r1.z * r6.w + r6.z;
    r6.xw = r8.xz * r8.yz;
    r6.xyz = r6.xxx * r0.xyz + ModShadowColor.xyz;
    r0.xyz = r6.www * r3.xyz;
    r7.xyz = r0.xzy * r7.xzy;
    r7.xyz = r7.xyz * LightColor.xzy;
    r6.xyz = r7.xzy * r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
