// ps_69e6d5a04b32857d.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 195 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 0000030C 10040C00 0000080A 00000000 00007108 003F00FF 00000001 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c10); // float4
float4 ModShadowAccumResolution : register(c13); // float2
float4 ModShadowColor : register(c11); // float3
float4 ModShadowGroupColor : register(c12); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_2 : register(c6); // float
float4 UniformScalar_3 : register(c7); // float
float4 UniformScalar_4 : register(c8); // float
float4 UniformScalar_5 : register(c9); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D ShadowTexture : register(s6);
sampler2D ModShadowAccumTexture : register(s7);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.y;
    r6.y = ps;
    ps = 1.0 / r5.w;
    r6.z = ps;
    ps = ScreenPositionScaleBias.x * r6.z;
    r7.xy = r1.xy * UniformVector_1.xy;
    r7.z = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r9.xyz = -UniformVector_0.xyz + 1.0;
    r6.x = ps;
    ps = ScreenPositionScaleBias.y * r6.z;
    r6.xy = r6.xy * abs(r8.xy);
    r7.w = ps;
    r6.zw = r7.zw * r5.xy + ScreenPositionScaleBias.wz;
    r5.xyz = tex2D(LightAttenuationTexture, r6.zw).xyz;
    r0.x = tex2D(ShadowTexture, r0.xy).x;
    r11.xyz = tex2D(Texture2D_3, r7.xy).xyz;
    r12.xyz = tex2D(Texture2D_0, r7.xy).xyz;
    r6.w = tex2D(Texture2D_2, r1.xy).x;
    r0.yzw = tex2D(Texture2D_4, r1.xy).xyz;
    r6.yz = tex2D(ModShadowAccumTexture, r6.xy).xy;
    r1.xyz = tex2D(Texture2D_1, r1.xy).xyz;
    ps = r2.z;
    r8.xyz = -ModShadowColor.xyz + 1.0;
    ps = 0.1 + ps;
    r6.x = dot(r3.zxy, r3.zxy);
    r7.w = ps;
    ps = 1.0 - r6.x;
    r7.y = dot(r4.zxy, r4.zxy);
    r7.x = saturate(ps);
    ps = r1.z + r1.z;
    r10.xy = r1.xy * 4.0;
    r3.w = ps;
    ps = 0.875 * r6.y;
    r7.z = dot(r2.zxy, r2.zxy);
    r1.y = ps;
    ps = 0.875 * r6.z;
    r0.yzw = r0.yzw + r0.yzw;
    r1.z = ps;
    ps = rsqrt(abs(r7.z));
    r6.x = saturate(r7.w * 5.0);
    r6.y = ps;
    ps = 1.0 - r6.x;
    r3.xyz = r12.xyz + r12.xyz;
    r6.z = ps;
    ps = rsqrt(abs(r7.y));
    r2.xyz = r6.yyy * r2.xyz;
    r6.x = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r4.xyz = r6.xxx * r4.xyz;
    r6.x = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r3 = r3.xzyw - 1.0;
    r6.y = ps;
    ps = UniformVector_2.x * r6.w;
    r7.yzw = -r6.xyw + 1.0;
    r1.x = ps;
    ps = UniformVector_2.y * r6.w;
    r10.zw = r1.yz * r7.yz;
    r1.y = ps;
    ps = UniformVector_2.z * r6.w;
    r7.yzw = r7.www * r11.xyz;
    r1.z = ps;
    r7.yzw = r1.xyz * r11.xyz + r7.yzw;
    r1.xyz = r7.yzw * r0.yzw - r7.yzw;
    ps = r3.w - r3.y;
    r6 = r10 + float4(-2.0, -2.0, 0.125, 0.125);
    r1.w = ps;
    ps = r6.z * r6.w;
    r0.yz = r6.xy - r3.xz;
    r6.x = ps;
    r6.xyz = r6.xxx * r8.xyz + ModShadowColor.xyz;
    r0.yz = r0.yz * UniformScalar_2.xx + r3.xz;
    r0.w = r1.w * UniformScalar_2.x + r3.y;
    r1.xyz = r1.xyz * UniformScalar_2.xxx + r7.yzw;
    r7.yzw = r1.xyz * UniformScalar_3.xxx + UniformScalar_4.xxx;
    ps = log2(r7.x);
    r6.w = dot(r0.wyz, r0.wyz);
    r7.x = ps;
    ps = rsqrt(abs(r6.w));
    r7.x = r7.x * LightColorAndFalloffExponent.w;
    r6.w = ps;
    r0.yzw = r0.yzw * r6.www;
    r6.w = dot(r0.wyz, r0.wyz);
    ps = rsqrt(abs(r6.w));
    r1.xyz = r1.zxy * r9.zxy;
    r6.w = ps;
    r3.xyz = r0.yzw * r6.www;
    ps = UniformScalar_5.x;
    r6.w = dot(r3.zxy, r4.zxy);
    r0.y = ps;
    ps = 0.0001;
    r8.xyz = r3.xyz * r6.www;
    r0.w = ps;
    r4.xyz = r8.xyz * 2.0 - r4.xyz;
    ps = max(r0.y, r0.w);
    r0.z = saturate(dot(r2.zxy, r4.zxy));
    r0.y = ps;
    ps = log2(r0.z);
    r6.w = saturate(dot(r3.zyx, r2.zyx));
    r0.z = ps;
    r0.xw = r0.xy * r0.xz;
    ps = pow(2.0, r7.x);
    r2.xyz = r0.xxx * r5.xyz;
    r7.x = ps;
    ps = pow(2.0, r0.w);
    r0.xyz = r2.xzy * r7.xxx;
    r7.x = ps;
    r7.xyz = r7.yzw * r7.xxx;
    r7.xy = r1.yz * r6.ww + r7.xy;
    r7.z = r1.x * r6.w + r7.z;
    r7.xyz = r0.xzy * r7.xyz;
    r7.xyz = r7.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r7.xzy * r6.xzy;
    oC0.xyz = r6.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
