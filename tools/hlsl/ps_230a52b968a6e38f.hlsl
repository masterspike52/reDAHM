// ps_230a52b968a6e38f.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 318 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000004F8 10040F00 00000606 00000000 000048C6 003F003F 00000021 00003050 00003151 00007254 0000F355 00007456 0000F557
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c12); // float4
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c14); // float2
float4 SpotDirection : register(c13); // float3
float4 UniformScalar_0 : register(c8); // float
float4 UniformScalar_1 : register(c9); // float
float4 UniformScalar_3 : register(c10); // float
float4 UniformScalar_4 : register(c11); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D ModShadowAccumTexture : register(s8);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float2 vPos : VPOS;   // r6 (pixel parameters)
    float vFace : VFACE;  // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r13 = tex2D(Texture2D_6, r0.xy);
    r0.zw = r1.xy * 6.0;
    r1.z = r13.w - 0.5;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r1.zzzz)) clip(-1.0);
    r7.xyw = tex2D(Texture2D_2, r0.zw).xyw;
    ps = r1.x;
    r1.zw = r0.xy * UniformScalar_0.xx;
    ps = UniformVector_2.x * ps;
    r10.xy = r1.xy * UniformVector_1.xy;
    r11.x = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r0.w = r5.w - 4e+02;
    r8.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r0.z = dot(r4.zxy, r4.zxy);
    r8.y = ps;
    r9.yz = r7.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r0.z));
    r9.x = saturate(r0.w * 0.00022222222);
    r0.z = ps;
    ps = -r1.y;
    r7.xyz = r0.zzz * r4.xyz;
    ps = 1.0 + ps;
    r9.xyz = r9.yzx * float3(0.5, 0.5, -0.025);
    r0.z = ps;
    ps = UniformVector_2.y * r0.z;
    r11.zw = r7.xy * r9.zz;
    r11.y = ps;
    r0.zw = r11.xy - 0.5;
    r14.xy = float2((r11.xy >= 0.5));
    ps = r8.x;
    r0.zw = r0.zw - r11.xy;
    ps = abs(r6.x) * ps;
    r10.zw = r0.zw * r14.xy;
    r4.x = ps;
    ps = r8.y;
    r12 = r11.zwxy + r10;
    ps = abs(r6.y) * ps;
    r0.zw = r12.zw + r12.zw;
    r4.y = ps;
    r11.yz = tex2D(ModShadowAccumTexture, r4.xy).xy;
    r15 = tex2D(Texture2D_1, r0.zw);
    r4.xyw = tex2D(Texture2D_0, r12.xy).xyz;
    r10.xyw = tex2D(Texture2D_2, r1.xy).xyw;
    r1.xyz = tex2D(Texture2D_3, r1.zw).wxy;
    r6.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r8.xyz = r6.xyz * 2.0 - 1.0;
    r14.zw = r1.yz * 2.0 - 1.0;
    ps = 0.0001 * r5.w;
    r0.x = r2.z + 0.1;
    r0.z = saturate(ps);
    ps = (-1.0) + r1.x;
    r0.y = saturate(r0.x * 5.0);
    r0.x = ps;
    ps = r10.x + r10.x;
    r9.zw = r5.xy * ScreenPositionScaleBias.xy;
    r1.y = ps;
    ps = r10.y + r10.y;
    r0.yz = -r0.zy + 1.0;
    r1.z = ps;
    ps = UniformScalar_1.x * r0.y;
    r6.xyz = r4.xyw + r4.xyw;
    r11.w = ps;
    ps = r0.x;
    r1.x = dot(r14.xyy, float3(1.0, 1.0, 1.0));
    r0.x = (r1.x == 0.0) ? r15.x : r15.y;
    ps = r0.y * ps;
    r1 = r1.zxyx + float4(-1.0, -2.0, -1.0, -3.0);
    r6.w = ps;
    ps = r1.z;
    r6 = r6 + float4(-1.0, -1.0, -1.0, 1.0);
    r0.x = (r1.y == 0.0) ? r15.z : r0.x;
    r1.w = (r1.w == 0.0) ? r15.w : r0.x;
    ps = r9.x + ps;
    r0.x = max(r1.w, 0.0);
    r14.x = ps;
    ps = r1.x;
    r11.x = min(r0.x, 0.3);
    ps = r9.y + ps;
    r11.xyz = r11.xyz * float3(3.3333333, 0.875, 0.875);
    r14.y = ps;
    r14 = r11.xxww * r14;
    r0.xw = r8.xy + r14.zw;
    r0.xw = r0.xw * UniformVector_4.xy + r14.xy;
    ps = 1.0 / r5.w;
    r9.xy = r0.xw - r6.xy;
    r0.x = ps;
    r0.xw = r9.zw * r0.xx + ScreenPositionScaleBias.wz;
    r4.xyz = tex2D(LightAttenuationTexture, r0.xw).xyz;
    r12.xyz = tex2D(Texture2D_5, r12.xy).xyz;
    r10.xyz = -UniformVector_0.xyz + 1.0;
    ps = UniformVector_5.x * UniformVector_5.w;
    r0.x = dot(r2.zxy, r2.zxy);
    r5.x = ps;
    ps = UniformVector_5.y * UniformVector_5.w;
    r1.x = dot(r3.zxy, r3.zxy);
    r5.y = ps;
    r3.w = r7.w * r10.w - 1.0;
    ps = rsqrt(abs(r1.x));
    r0.w = saturate(-r1.x + 1.0);
    r1.x = ps;
    ps = log2(r0.w);
    r14.xyz = r1.xxx * -SpotDirection.xyz;
    r1.x = ps;
    ps = UniformVector_5.z * UniformVector_5.w;
    r0.w = dot(r14.zxy, r3.zxy);
    r5.z = ps;
    ps = ModShadowGroupColor.x * r0.z;
    r3.xyz = r5.xyz * r13.xyz;
    r1.y = ps;
    r13.w = r8.z * UniformVector_4.z - r6.z;
    ps = ModShadowGroupColor.y * r0.z;
    r14.xyz = r3.xyz * r6.www;
    r1.z = ps;
    r8.xyz = r14.xyz * UniformScalar_3.xxx + UniformScalar_4.xxx;
    ps = rsqrt(abs(r0.x));
    r3.xyz = -r1.zyw + 1.0;
    r0.x = ps;
    ps = 2.5 * r3.z;
    r1.yzw = -ModShadowColor.xyz + 1.0;
    r14.w = ps;
    r11.yzw = r11.xyz * r3.wyx + float3(1.0, 0.125, 0.125);
    ps = abs(r11.y) * abs(r11.y);
    r5.xyz = r0.xxx * r2.xyz;
    r11.x = ps;
    r2.x = saturate(r14.w * r11.y - 0.5);
    r13.xyz = r14.xyz * r11.yyy - r12.xyz;
    r3.w = r13.w * r2.x - 2.0;
    r3.xyz = r13.xyz * r2.xxx + r12.xyz;
    ps = -SpotAngles.x - -r0.w;
    r2.yw = r11.xz * r11.xw;
    r0.x = ps;
    r1.yzw = r2.www * r1.yzw + ModShadowColor.xyz;
    ps = SpotAngles.y * r0.x;
    r3.xyz = r3.xyz * r10.xyz;
    r2.z = saturate(ps);
    r6.z = r4.w * 2.0 + r3.w;
    r6.xy = r9.xy * r2.xx + r6.xy;
    r6.xyw = r6.xyz * r0.yyy;
    ps = r2.y * r2.y;
    r6.z = r6.w + 1.0;
    r0.w = ps;
    r6.xyz = (r0.yyy > 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    r6.xyz = (r0.yyy >= 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    ps = r2.z * r2.z;
    r0.y = dot(r6.zxy, r6.zxy);
    r0.x = ps;
    ps = rsqrt(abs(r0.y));
    r2.yzw = r0.www * r8.xyz;
    r0.y = ps;
    r6.xyz = r6.xyz * r0.yyy;
    r0.y = dot(r6.zxy, r7.zxy);
    r0.yzw = r6.xyz * r0.yyy;
    r0.yzw = r0.yzw * 2.0 - r7.xyz;
    r0.y = saturate(dot(r5.zxy, r0.wyz));
    ps = log2(r0.y);
    r2.xyz = r2.yzw * r2.xxx;
    r0.y = ps;
    ps = LightColorAndFalloffExponent.w * r1.x;
    r0.w = r0.y * 15.0;
    r0.y = ps;
    ps = pow(2.0, r0.w);
    r0.z = saturate(dot(r6.zyx, r5.zyx));
    r0.w = ps;
    ps = pow(2.0, r0.y);
    r2.xyz = r2.xyz * r0.www;
    r0.y = ps;
    r2.xy = r3.xy * r0.zz + r2.xy;
    r2.z = r3.z * r0.z + r2.z;
    r0.yzw = r2.xyz * r0.yyy;
    r0.yzw = r0.yzw * r4.xyz;
    r0.yzw = r0.yzw * LightColorAndFalloffExponent.xyz;
    r0.xyz = r0.ywz * r0.xxx;
    r0.xyz = r0.xzy * r1.yzw;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
