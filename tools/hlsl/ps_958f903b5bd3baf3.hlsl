// ps_958f903b5bd3baf3.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 297 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000004A4 10040F00 0000090A 00000000 00008129 003F01FF 00000021 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c13); // float3
float4 ModShadowAccumResolution : register(c16); // float2
float4 ModShadowColor : register(c14); // float3
float4 ModShadowGroupColor : register(c15); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_11 : register(c11); // float
float4 UniformScalar_12 : register(c12); // float
float4 UniformScalar_7 : register(c9); // float
float4 UniformScalar_8 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_6 : register(c7); // float4
float4 UniformVector_7 : register(c8); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D Texture2D_8 : register(s9);
sampler2D ShadowTexture : register(s10);
sampler2D ModShadowAccumTexture : register(s11);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r11 = tex2D(Texture2D_5, r1.xy);
    ps = (-0.5) + r11.w;
    r6.xy = r1.wz * 6.0;
    r6.z = ps;
    ps = UniformVector_2.x * r1.w;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.zzzz)) clip(-1.0);
    r12.x = ps;
    r7.xyw = tex2D(Texture2D_2, r6.xy).xyw;
    r3.xy = r1.wz * UniformVector_1.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.x = r5.w - 4e+02;
    r8.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.w = dot(r4.zxy, r4.zxy);
    r8.y = ps;
    r6.yz = r7.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r6.w));
    r6.x = saturate(r6.x * 0.00022222222);
    r6.w = ps;
    ps = -r1.z;
    r4.xyz = r6.www * r4.xyz;
    ps = 1.0 + ps;
    r10.xyz = r6.xyz * float3(-0.025, 0.5, 0.5);
    r6.x = ps;
    ps = UniformVector_2.y * r6.x;
    r12.zw = r4.xy * r10.xx;
    r12.y = ps;
    r6.xy = r12.xy - 0.5;
    r0.zw = float2((r12.xy >= 0.5));
    ps = r8.x;
    r6.xy = r6.xy - r12.xy;
    ps = abs(r9.x) * ps;
    r3.zw = r6.xy * r0.zw;
    r8.z = ps;
    ps = r8.y;
    r6 = r12 + r3.zwxy;
    ps = abs(r9.y) * ps;
    r8.xy = r6.xy + r6.xy;
    r8.w = ps;
    r9.yz = tex2D(ModShadowAccumTexture, r8.zw).xy;
    r15 = tex2D(Texture2D_1, r8.xy);
    r7.xyz = tex2D(Texture2D_2, r1.wz).xwy;
    r3.xyz = tex2D(Texture2D_3, r1.xy).xyz;
    r8.xyw = tex2D(Texture2D_0, r6.zw).xyz;
    r6.xy = r1.wz * UniformVector_7.xy;
    r12.xyz = r8.xyw * 2.0 - 1.0;
    r3.xyz = r3.xyz * 2.0 - 1.0;
    ps = r7.x + r7.x;
    r13.zw = r5.xy * ScreenPositionScaleBias.xy;
    r14.y = ps;
    ps = r7.z + r7.z;
    r14.x = dot(r0.zww, float3(1.0, 1.0, 1.0));
    r14.z = ps;
    r8.z = (r14.x == 0.0) ? r15.x : r15.y;
    r14 = r14.xxyz + float4(-3.0, -2.0, -1.0, -1.0);
    ps = r14.z;
    r3.xyw = r3.zxy * UniformVector_4.zxy;
    r8.z = (r14.y == 0.0) ? r15.z : r8.z;
    r7.x = (r14.x == 0.0) ? r15.w : r8.z;
    ps = r10.y + ps;
    r8.z = max(r7.x, 0.0);
    r0.z = ps;
    ps = r14.w;
    r9.x = min(r8.z, 0.3);
    ps = r10.z + ps;
    r9.xyz = r9.yxz * float3(0.875, 3.3333333, 0.875);
    r0.w = ps;
    r0.zw = r9.yy * r0.zw + r3.yw;
    ps = 1.0 / r5.w;
    r13.xy = r0.zw - r12.xy;
    r8.z = ps;
    r10.xy = r13.zw * r8.zz + ScreenPositionScaleBias.wz;
    r0.w = tex2D(Texture2D_4, r1.xy).x;
    r8.xyz = tex2D(LightAttenuationTexture, r10.xy).xyz;
    r3.y = tex2D(ShadowTexture, r0.xy).x;
    r10.xyz = tex2D(Texture2D_7, r6.zw).xyz;
    r6.yzw = tex2D(Texture2D_6, r6.xy).xyz;
    r0.xyz = tex2D(Texture2D_8, r1.xy).xyz;
    ps = r5.w;
    r1.w = dot(r2.zxy, r2.zxy);
    ps = 0.0001 * ps;
    r6.x = r2.z + 0.1;
    r7.z = saturate(ps);
    r1.xyz = r0.xyz * UniformScalar_11.xxx + UniformScalar_12.xxx;
    ps = UniformScalar_8.x * r6.y;
    r0.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r3.w = ps;
    r7.y = r7.w * r7.y - 1.0;
    ps = 5.0 * r6.x;
    r0.xyz = r0.xyz * r11.xyz;
    r7.w = saturate(ps);
    ps = -r12.z;
    r5.xy = -r7.zw + 1.0;
    ps = r3.x + ps;
    r7.zw = r5.yy * ModShadowGroupColor.xy;
    r11.w = ps;
    ps = rsqrt(abs(r1.w));
    r7.xzw = -r7.xwz + 1.0;
    r6.x = ps;
    ps = 2.5 * r7.x;
    r14.xyz = r0.xyz * r6.yzw;
    r14.w = ps;
    r9.xyz = r9.yxz * r7.ywz + float3(1.0, 0.125, 0.125);
    r11.xyz = r14.xyz * r9.xxx - r10.xyz;
    ps = abs(r9.x) * abs(r9.x);
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r9.w = ps;
    r7.y = saturate(r14.w * r9.x - 0.5);
    r7.xz = r13.xy * r7.yy + r12.xy;
    ps = r9.y * r9.z;
    r6.yzw = r6.xxx * r2.xyz;
    r3.z = ps;
    r6.x = r11.w * r7.y - 2.0;
    r2.xyz = r11.xyz * r7.yyy + r10.xyz;
    ps = r9.w * r9.w;
    r0.xyz = r2.xyz * r0.xyz;
    r3.x = ps;
    r7.w = r8.w * 2.0 + r6.x;
    r2.xyw = r7.xzw * r5.xxx;
    ps = r3.x * r3.x;
    r2.z = r2.w + 1.0;
    r7.z = ps;
    r2.xyz = (r5.xxx > 0.0) ? r2.xyz : float3(0.0, 0.0, 1.0);
    r2.xyz = (r5.xxx >= 0.0) ? r2.xyz : float3(0.0, 0.0, 1.0);
    ps = r3.y * r3.y;
    r6.x = dot(r2.zxy, r2.zxy);
    r7.x = ps;
    ps = rsqrt(abs(r6.x));
    r8.xyz = r7.xxx * r8.xyz;
    r6.x = ps;
    r2.xyz = r2.xyz * r6.xxx;
    r6.x = dot(r2.zxy, r4.zxy);
    r5.xyz = r2.xyz * r6.xxx;
    r4.xyz = r5.xyz * 2.0 - r4.xyz;
    r6.x = saturate(dot(r6.wyz, r4.zxy));
    ps = log2(r6.x);
    r8.w = saturate(dot(r2.zyx, r6.wzy));
    r6.y = ps;
    ps = UniformScalar_7.x * r6.y;
    r2.yzw = -ModShadowColor.xyz + 1.0;
    r6.x = ps;
    ps = pow(2.0, r6.x);
    r7.x = r6.y * 15.0;
    r2.x = ps;
    r6 = r3.zwzz * r2.wxzy;
    r1.xyz = r6.yyy * r0.www + r1.xyz;
    ps = r6.w;
    r1.xyz = r7.zzz * r1.xyz;
    r6.y = ps;
    ps = pow(2.0, r7.x);
    r7.yzw = r1.xyz * r7.yyy;
    r7.x = ps;
    ps = ModShadowColor.x + r6.y;
    r7.yzw = r7.yzw * r7.xxx;
    r7.x = ps;
    r7.yz = r0.xy * r8.ww + r7.yz;
    r7.w = r0.z * r8.w + r7.w;
    ps = ModShadowColor.y + r6.z;
    r8.xyz = r8.xyz * r7.yzw;
    r7.y = ps;
    ps = ModShadowColor.z + r6.x;
    r8.xyz = r8.xzy * LightColor.xzy;
    r7.z = ps;
    r6.xyz = r8.xzy * r7.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
