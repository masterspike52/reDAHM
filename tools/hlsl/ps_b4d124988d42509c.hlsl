// ps_b4d124988d42509c.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 441 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000006E4 10041900 0000070A 00000000 000068E7 001F007F 00000021 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c17); // float4
float4 ModShadowAccumResolution : register(c22); // float2
float4 ModShadowColor : register(c20); // float3
float4 ModShadowGroupColor : register(c21); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 SpotAngles : register(c19); // float2
float4 SpotDirection : register(c18); // float3
float4 UniformScalar_1 : register(c8); // float
float4 UniformScalar_10 : register(c11); // float
float4 UniformScalar_11 : register(c12); // float
float4 UniformScalar_12 : register(c13); // float
float4 UniformScalar_13 : register(c14); // float
float4 UniformScalar_14 : register(c15); // float
float4 UniformScalar_16 : register(c16); // float
float4 UniformScalar_8 : register(c9); // float
float4 UniformScalar_9 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_3 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D Texture2D_8 : register(s9);
sampler2D Texture2D_9 : register(s10);
sampler2D ModShadowAccumTexture : register(s11);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 r21 = 0.0;
    float4 r22 = 0.0;
    float4 r23 = 0.0;
    float4 r24 = 0.0;
    float4 r25 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / r4.w;
    r14.xy = r4.xy * ScreenPositionScaleBias.xy;
    r5.x = ps;
    ps = UniformScalar_12.x - r6.w;
    r25.xyz = UniformVector_5.xzy * 2e+01;
    r14.z = ps;
    r5.xy = r14.xy * r5.xx + ScreenPositionScaleBias.wz;
    r8.xyz = tex2D(LightAttenuationTexture, r5.xy).xyz;
    r5.z = dot(r3.zxy, r3.zxy);
    ps = UniformScalar_16.x;
    r1.w = dot(r2.zxy, r2.zxy);
    ps = 0.05 * ps;
    r5.y = r4.w - 4e+02;
    r9.w = ps;
    ps = 0.00022222222 * r5.y;
    r5.x = saturate(-r1.w + 1.0);
    r5.y = saturate(ps);
    ps = log2(r5.x);
    r9.xyz = r8.xyz * r2.www;
    r5.x = ps;
    ps = rsqrt(abs(r5.z));
    r5.x = r5.x * LightColorAndFalloffExponent.w;
    r5.z = ps;
    ps = pow(2.0, r5.x);
    r10.xyz = r5.zzz * r3.xyz;
    r5.x = ps;
    ps = -r0.z;
    r3 = r9.xzyw * r5.xxxy;
    r11.xy = r10.xy * r3.ww + r0.wz;
    ps = 1.0 + ps;
    r11.z = -r11.y + 1.0;
    r11.w = ps;
    r8.xyz = r11.xzw * UniformVector_1.xyy;
    ps = UniformVector_1.x * r0.w;
    r23.xyz = UniformVector_4.xzy * 2e+01;
    r8.w = ps;
    r9 = r8 - 0.5;
    r5 = float4((r8.xzwy >= 0.5));
    r9 = r9 - r8;
    r8 = r9 * r5.xwyz + r8;
    r13 = r8.wxyz + r8.wxyz;
    r9 = tex2D(Texture2D_9, r0.xy);
    ps = 0.0001 * r4.w;
    r12 = r0.wzwz * float4(3.0, 3.0, 18.0, 18.0);
    r23.w = saturate(ps);
    r24 = tex2D(Texture2D_4, r12.xy);
    r8 = tex2D(Texture2D_3, r13.xw);
    r15.yzw = tex2D(Texture2D_4, r12.zw).xyw;
    ps = 1.0 / ModShadowAccumResolution.x;
    r19.x = saturate(r6.w * 15.0);
    r12.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r10.w = float((UniformScalar_11.x >= 1.0));
    r12.y = ps;
    r10.w = r14.z * r10.w + r6.w;
    ps = -r10.w;
    r7.zw = r12.xy * abs(r7.xy);
    ps = r6.z + ps;
    r16.x = dot(r5.xww, float3(1.0, 1.0, 1.0));
    r5.x = ps;
    ps = r5.x;
    r16.y = dot(r5.zyy, float3(1.0, 1.0, 1.0));
    r5.y = ps;
    r5.x = (r16.y == 0.0) ? r8.x : r8.y;
    ps = UniformScalar_13.x;
    r12 = r16.yyxx + float4(-2.0, -3.0, -3.0, -2.0);
    r5.z = ps;
    r5.x = (r12.x == 0.0) ? r8.z : r5.x;
    r7.y = (r12.y == 0.0) ? r8.w : r5.x;
    ps = max(r5.y, r5.z);
    r5.x = max(r7.y, 0.0);
    r5.y = ps;
    ps = 1.0 / UniformScalar_14.x;
    r5.xw = min(r5.yx, float2(1.0, 0.3));
    r5.y = ps;
    r19.y = saturate(r5.x * r5.y);
    r8.xz = -r19.yx + 1.0;
    r15.x = r8.x * r6.w;
    ps = r0.w + r0.w;
    r5.xz = r15.xw * r24.zw;
    r6.x = ps;
    ps = r0.z + r0.z;
    r7.x = float((r5.x >= 0.05));
    r6.y = ps;
    ps = r15.y + r15.y;
    r0.zw = -r7.yx + 1.0;
    r14.x = ps;
    r5.y = (UniformScalar_12.x > 0.0) ? r0.w : 1.0;
    r5.y = (UniformScalar_12.x >= 0.0) ? r5.y : r0.w;
    ps = r15.z + r15.z;
    r14.z = r5.y * r9.w;
    r14.y = ps;
    ps = r11.x + r11.x;
    r15.xyz = r14.xyz + float3(-1.0, -1.0, -0.5);
    r7.x = ps;
    ps = r11.y + r11.y;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r15.zzzz)) clip(-1.0);
    r7.y = ps;
    r8.xy = tex2D(ModShadowAccumTexture, r7.zw).xy;
    r11 = tex2D(Texture2D_2, r6.xy);
    r18.xyz = tex2D(Texture2D_8, r7.xy).xyz;
    r22 = tex2D(Texture2D_3, r13.yz);
    r13 = tex2D(Texture2D_7, r0.xy);
    r20.xyz = tex2D(Texture2D_1, r6.xy).xyz;
    r21 = tex2D(Texture2D_6, r0.xy).xzyw;
    r6.xyz = tex2D(Texture2D_5, r0.xy).xyz;
    r7.xyw = tex2D(Texture2D_0, r7.xy).xyz;
    r4.xyz = -ModShadowColor.xyz + 1.0;
    r6.w = float((UniformScalar_8.x >= UniformScalar_1.x));
    r5.y = dot(r1.zxy, r1.zxy);
    r17.xyz = r9.xyz * UniformScalar_9.xxx + UniformScalar_10.xxx;
    r14.xyz = r7.xyw * 2.0 - 1.0;
    r16.zw = r24.xy * 2.0 - 1.0;
    r6.xyz = r6.xyz * 2.0 - 1.0;
    ps = r21.x * r21.w;
    r20.xyz = r20.xyz + r20.xyz;
    r21.x = ps;
    ps = rsqrt(abs(r5.y));
    r13.xyz = r13.xzy * r13.www;
    r5.y = ps;
    ps = rsqrt(abs(r1.w));
    r7.xyz = r5.yyy * r1.xyz;
    r5.y = ps;
    ps = r21.y * r21.w;
    r24.xyz = r5.yyy * -SpotDirection.xyz;
    r21.y = ps;
    ps = r21.z * r21.w;
    r13.xyz = r25.xyz * r13.xyz;
    r21.z = ps;
    r23.xyz = r23.xyz * r21.xyz;
    ps = r1.z;
    r21.xyz = r6.xyz * UniformVector_3.xyz;
    r6.xyz = (-abs(r6.www) >= 0.0) ? r13.yxz : r23.yxz;
    ps = 0.1 + ps;
    r5.y = dot(r24.zxy, r2.zxy);
    r5.x = ps;
    r13.xyz = (abs(r6.www) > 0.0) ? r23.xzy : r13.xzy;
    ps = 5.0 * r5.x;
    r0.x = dot(r13.zxy, float3(0.11, 0.3, 0.59));
    r21.w = saturate(ps);
    ps = -SpotAngles.x - -r5.y;
    r6.w = saturate(dot(r6.zxy, float3(0.59, 0.11, 0.3)));
    r5.x = ps;
    r12.xy = -r21.xy * r6.ww + r21.xy;
    ps = SpotAngles.y * r5.x;
    r6.xz = -r21.zw + 1.0;
    r6.y = saturate(ps);
    ps = ModShadowGroupColor.x * r6.z;
    r5.y = r0.x - r13.x;
    r23.y = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r23.x = saturate(r5.y + r13.x);
    r23.z = ps;
    ps = r6.x * r6.w;
    r1 = -r23.wxyz + 1.0;
    r5.y = ps;
    r0.x = (r16.x == 0.0) ? r22.x : r22.y;
    r0.x = (r12.w == 0.0) ? r22.z : r0.x;
    r16.x = (r12.z == 0.0) ? r22.w : r0.x;
    ps = r6.y * r6.y;
    r16.y = r5.w * 3.3333333;
    r5.w = ps;
    ps = r21.z;
    r6.xy = -r16.xy + 1.0;
    r6.z = dot(r19.xx, r6.xx) + r16.x;
    ps = r5.y + ps;
    r20.w = r6.z + r16.x;
    r12.z = ps;
    ps = r19.x;
    r2 = r20 + float4(-1.0, -1.0, -1.0, 0.25);
    ps = r6.y * ps;
    r5.y = saturate(r2.w - r5.z);
    r15.z = ps;
    ps = (-1.0) - -r5.z;
    r6.xyw = r5.yyy * r18.xyz;
    r15.w = ps;
    ps = -r6.w;
    r18.xzw = r2.zxy - r14.zxy;
    ps = r11.z + ps;
    r5.xy = -r6.xy + r11.xy;
    r18.y = ps;
    r2.xy = r5.xy * r11.ww + r6.xy;
    ps = 2.5 * r0.z;
    r11 = r18.zwxy * r11.wwww;
    r0.y = ps;
    ps = r6.w;
    r6.xyz = r17.xyz * r1.yyy;
    ps = r11.w + ps;
    r15.xy = r15.xy * 0.5;
    r2.z = ps;
    ps = r11.z;
    r15.xyz = r16.zyw + r15.xzy;
    r5.x = ps;
    r12.xy = r15.xy * r15.yz + r12.xy;
    r0.x = r15.w * r15.y + 1.0;
    ps = r0.x * r0.z;
    r11.xyz = r14.xyz + r11.xyz;
    r5.y = ps;
    ps = r0.y * r0.x;
    r12.xyz = -r11.xyz + r12.xyz;
    r5.z = ps;
    ps = (-2.0) + r5.x;
    r5.yz = r5.yz * r8.zz;
    r11.w = ps;
    ps = (-0.5) + r5.z;
    r5.x = float((r5.y >= 0.9));
    r5.y = saturate(ps);
    r9.xyz = r5.xxx * r13.xyz + r9.xyz;
    r0.xyz = r12.xyz * r5.yyy + r11.xyw;
    r0.w = r7.w * 2.0 + r0.z;
    r9.xyz = r9.xyz - r2.xyz;
    r9.xyz = r9.xyz * r5.yyy + r2.xyz;
    r0.xyw = r0.xyw * r1.xxx;
    ps = -UniformVector_0.x;
    r0.z = r0.w + 1.0;
    r0.xyz = (r1.xxx > 0.0) ? r0.xyz : float3(0.0, 0.0, 1.0);
    r0.yzw = (r1.xxx >= 0.0) ? r0.xyz : float3(0.0, 0.0, 1.0);
    ps = 1.0 + ps;
    r5.x = dot(r0.wyz, r0.wyz);
    r0.x = ps;
    ps = rsqrt(abs(r5.x));
    r6.xyz = r6.xyz * r5.yyy;
    r5.x = ps;
    ps = -UniformVector_0.y;
    r2.xyz = r0.yzw * r5.xxx;
    ps = 1.0 + ps;
    r5.x = dot(r2.zxy, r10.zxy);
    r0.y = ps;
    ps = -UniformVector_0.z;
    r5.xyz = r2.xyz * r5.xxx;
    r5.xyz = r5.xyz * 2.0 - r10.xyz;
    ps = 1.0 + ps;
    r5.x = saturate(dot(r7.zxy, r5.zxy));
    r0.z = ps;
    ps = log2(r5.x);
    r0.xyz = r9.xyz * r0.xyz;
    r8.z = ps;
    r5.xyz = r8.xyz * float3(0.875, 0.875, 15.0);
    ps = pow(2.0, r5.z);
    r6.w = saturate(dot(r2.zyx, r7.zyx));
    r5.z = ps;
    r5.xy = r5.xy * r1.zw + 0.125;
    ps = r5.x * r5.y;
    r6.xyz = r6.xyz * r5.zzz;
    r5.x = ps;
    r5.xyz = r5.xxx * r4.xyz + ModShadowColor.xyz;
    r6.xy = r0.xy * r6.ww + r6.xy;
    r6.z = r0.z * r6.w + r6.z;
    r6.xyz = r3.xzy * r6.xyz;
    r6.xyz = r6.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r6.xzy * r5.www;
    r5.xyz = r6.xzy * r5.xyz;
    oC0.xyz = r5.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
