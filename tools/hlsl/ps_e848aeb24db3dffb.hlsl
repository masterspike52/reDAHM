// ps_e848aeb24db3dffb.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 396 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000630 10041800 0000070A 00000000 000068E7 001F007F 00000021 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c14); // float3
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_10 : register(c13); // float
float4 UniformScalar_5 : register(c10); // float
float4 UniformScalar_8 : register(c11); // float
float4 UniformScalar_9 : register(c12); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UniformVector_7 : register(c9); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D Texture2D_8 : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 r21 = 0.0;
    float4 r22 = 0.0;
    float4 r23 = 0.0;
    float4 r24 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r9 = tex2D(Texture2D_7, r0.xy);
    ps = (-0.5) + r9.w;
    r5.xy = r0.wz * UniformVector_7.xy;
    r5.z = ps;
    ps = UniformVector_2.x * r0.w;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r5.zzzz)) clip(-1.0);
    r14.x = ps;
    r8.xyz = tex2D(Texture2D_8, r5.xy).xyz;
    r10.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    ps = r4.w;
    r6.xw = r0.wz * 6.0;
    ps = (-4e+02) + ps;
    r5.y = r1.z + 0.1;
    r5.x = ps;
    ps = r0.w;
    r12.xy = r4.xy * ScreenPositionScaleBias.xy;
    ps = UniformVector_1.x * ps;
    r6.y = float((UniformScalar_8.x >= 1.0));
    r5.z = ps;
    ps = r0.z;
    r1.w = float((UniformScalar_8.x > 1.0));
    ps = UniformVector_1.y * ps;
    r6.z = dot(r3.zxy, r3.zxy);
    r5.w = ps;
    ps = rsqrt(abs(r6.z));
    r11.xyz = r10.xyz + r10.xyz;
    r6.z = ps;
    r8.xyz = (-abs(r1.www) >= 0.0) ? r8.xyz : 1.0;
    r10.yzw = (-abs(r6.yyy) >= 0.0) ? 1.0 : r8.xyz;
    ps = 0.00022222222 * r5.x;
    r13.zw = r9.yz * UniformScalar_9.xx;
    r6.y = saturate(ps);
    ps = 5.0 * r5.y;
    r8.xyz = r6.zzz * r3.xyz;
    r6.z = saturate(ps);
    ps = 1.0 / UniformVector_1.y;
    r5.x = r6.y * (-0.025);
    r10.x = ps;
    r13.xy = r8.xy * r5.xx + r5.zw;
    ps = 1.0 / ModShadowAccumResolution.x;
    r17.yzw = r13.zwy * r10.zwx;
    r5.x = ps;
    ps = 1.0 / UniformVector_1.x;
    r14.y = -r17.w + 1.0;
    r14.w = ps;
    ps = 1.0 - r0.z;
    r22.xyz = UniformVector_6.xzy * 2e+01;
    r14.z = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r14.yzw = r14.zyw * UniformVector_2.yyx;
    r5.y = ps;
    ps = r14.w;
    r5.xy = r5.xy * abs(r7.xy);
    ps = r13.x * ps;
    r3.xyz = r14.zxy - 0.5;
    r11.w = ps;
    r5.w = float((r11.w >= 0.5));
    r19.xyz = float3((r14.zxy >= 0.5));
    r3.xyz = r3.xzy - r14.zyx;
    r20 = r11 + float4(-1.0, -1.0, -1.0, -0.5);
    r3.yzw = r3.xzy * r19.xyz + r14.zxy;
    ps = 1.0 / r4.w;
    r12.z = r20.w - r11.w;
    r5.z = ps;
    r7.xy = r12.xy * r5.zz + ScreenPositionScaleBias.wz;
    r3.x = r12.z * r5.w + r11.w;
    r3 = r3.zwxy + r3.zwxy;
    r12.xyz = tex2D(Texture2D_6, r13.xy).xyz;
    r16.xyw = tex2D(LightAttenuationTexture, r7.xy).xyz;
    r7 = tex2D(Texture2D_1, r3.xy);
    r18 = tex2D(Texture2D_1, r3.zw);
    r14.xy = tex2D(ModShadowAccumTexture, r5.xy).xy;
    r24 = tex2D(Texture2D_4, r0.xy);
    r21 = tex2D(Texture2D_5, r0.xy).xzyw;
    r11.xyw = tex2D(Texture2D_2, r6.xw).xyw;
    r5.xyz = tex2D(Texture2D_2, r0.wz).wxy;
    r3.xyw = tex2D(Texture2D_0, r13.xy).xyz;
    r13.xyz = r3.xyw * 2.0 - 1.0;
    r15.xy = r5.yz * 2.0 - 1.0;
    r14.zw = r11.xy * 2.0 - 1.0;
    ps = r21.x * r21.w;
    r23.xyz = UniformVector_5.xzy * 2e+01;
    r21.x = ps;
    ps = r21.y * r21.w;
    r0.xyz = r24.xzy * r24.www;
    r21.y = ps;
    ps = r21.z * r21.w;
    r0.xyz = r23.xyz * r0.xyz;
    r21.z = ps;
    ps = (UniformScalar_5.x > 0.0) ? 1.0 : 0.0;
    r21.xyz = r22.xyz * r21.xyz;
    r6.x = ps;
    r22.xyz = (abs(r6.xxx) > 0.0) ? r0.xzy : r21.xzy;
    r0.xyz = (-abs(r6.xxx) >= 0.0) ? r21.yxz : r0.yxz;
    ps = UniformScalar_9.x * r10.y;
    r6.w = dot(r1.zxy, r1.zxy);
    r6.x = ps;
    ps = (UniformScalar_5.x >= 0.0) ? 1.0 : 0.0;
    r17.x = r6.x * r9.x;
    r6.x = ps;
    r0.xyz = (-abs(r6.xxx) >= 0.0) ? r21.yxz : r0.xyz;
    r11.xyz = (abs(r6.xxx) > 0.0) ? r22.xyz : r21.xzy;
    r6.x = dot(r11.zxy, float3(0.11, 0.3, 0.59));
    r6.x = r6.x - r11.x;
    r16.z = saturate(r6.x + r11.x);
    r14 = r14 * float4(0.875, 0.875, 0.5, 0.5);
    r15.yz = r15.xy + r14.zw;
    r15.w = saturate(dot(r0.zxy, float3(0.59, 0.11, 0.3)));
    ps = r4.w;
    r19.y = dot(r19.yzz, float3(1.0, 1.0, 1.0));
    r19.x = r19.x * 2.0 + r5.w;
    r1.w = (r19.x == 0.0) ? r18.x : r18.y;
    r6.x = (r19.y == 0.0) ? r7.x : r7.y;
    ps = 0.0001 * ps;
    r0.xyz = r20.xyz * UniformVector_4.xyz;
    r0.w = saturate(ps);
    r5.zw = -r0.xy * r15.ww + r0.xy;
    ps = r11.w;
    r4 = r19.xxyy + float4(-3.0, -2.0, -2.0, -3.0);
    r6.x = (r4.z == 0.0) ? r7.z : r6.x;
    r1.w = (r4.y == 0.0) ? r18.z : r1.w;
    r14.z = (r4.x == 0.0) ? r18.w : r1.w;
    r6.x = (r4.w == 0.0) ? r7.w : r6.x;
    ps = r5.x * ps;
    r5.y = max(r6.x, 0.0);
    r5.x = ps;
    ps = (-1.0) - -r5.x;
    r5.y = min(r5.y, 0.3);
    r15.x = ps;
    ps = 3.3333333 * r5.y;
    r7.xz = -r0.zw + 1.0;
    r7.y = ps;
    r15 = r7.yyxy * r15.yzwx;
    ps = rsqrt(abs(r6.w));
    r15.z = r0.z + r15.z;
    r5.y = ps;
    ps = 1.0 - r6.x;
    r4.xyz = -ModShadowColor.xyz + 1.0;
    r6.x = ps;
    ps = 1.0 - r6.z;
    r0.xyz = r16.xyw * r2.www;
    r6.z = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r3.xyz = r5.yyy * r1.xyz;
    r16.x = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r2.xyz = r17.xyz + UniformScalar_10.xxx;
    r16.y = ps;
    ps = 2.5 * r6.x;
    r1.xyz = -r16.xyz + 1.0;
    r6.w = ps;
    ps = r15.w;
    r15.xy = r5.zw + r15.xy;
    r5.z = ps;
    ps = 1.0 + r5.z;
    r14.xy = r14.xy * r1.xy;
    r6.y = ps;
    r5.yz = r6.yw * r6.xy;
    ps = (-0.5) + r5.z;
    r6.xyz = r14.xzy + float3(0.125, 0.25, 0.125);
    r6.w = saturate(ps);
    ps = r6.x * r6.z;
    r2.xyz = r2.xyz * r1.zzz;
    r6.x = ps;
    ps = r6.y;
    r1.xyz = -r13.xyz + r15.xyz;
    ps = r14.z + ps;
    r0.w = float((r5.y > 0.9));
    r5.w = ps;
    r6.xyz = r6.xxx * r4.xyz + ModShadowColor.xyz;
    r5.yz = r1.xy * r6.ww + r13.xy;
    r5.x = saturate(r5.w - r5.x);
    r5.w = r1.z * r6.w - 2.0;
    r5.w = r3.w * 2.0 + r5.w;
    r1.xyz = r5.xxx * r12.xyz;
    r4.xyz = r0.www * r11.xyz - r1.xyz;
    r5.xyz = r5.wyz * r7.zzz;
    r4.xyz = r10.yzw * r9.xyz + r4.xyz;
    r4.xyz = r4.xyz * r6.www + r1.xyz;
    ps = 1.0 + r5.x;
    r1.xyz = -UniformVector_0.xyz + 1.0;
    r5.w = ps;
    r5.xyz = (r7.zzz > 0.0) ? r5.yzw : float3(0.0, 0.0, 1.0);
    r5.yzw = (r7.zzz >= 0.0) ? r5.xyz : float3(0.0, 0.0, 1.0);
    r5.x = dot(r5.wyz, r5.wyz);
    ps = rsqrt(abs(r5.x));
    r2.xyz = r2.xyz * r6.www;
    r5.x = ps;
    r5.yzw = r5.yzw * r5.xxx;
    r5.x = dot(r5.wyz, r8.zxy);
    r7.xyz = r5.yzw * r5.xxx;
    r7.xyz = r7.xyz * 2.0 - r8.xyz;
    r5.x = saturate(dot(r3.zxy, r7.zxy));
    ps = log2(r5.x);
    r1.xyz = r4.xyz * r1.xyz;
    r5.x = ps;
    ps = 15.0 * r5.x;
    r5.x = ps;
    ps = pow(2.0, r5.x);
    r5.w = saturate(dot(r5.wzy, r3.zyx));
    r5.x = ps;
    r5.xyz = r2.xyz * r5.xxx;
    r5.xy = r1.xy * r5.ww + r5.xy;
    r5.z = r1.z * r5.w + r5.z;
    r5.xyz = r0.xzy * r5.xzy;
    r5.xyz = r5.xyz * LightColor.xzy;
    r5.xyz = r5.xzy * r6.xyz;
    oC0.xyz = r5.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
