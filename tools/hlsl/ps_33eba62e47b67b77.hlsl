// ps_33eba62e47b67b77.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 210 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000348 10040D00 00000506 00000000 000048A5 001F001F 00000001 00003050 0000F154 0000F256 0000F357 0000F458
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD8 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c13); // float4
float4 ConstantLighting : register(c14); // float3
float4 LowerSkyColor : register(c12); // float3
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 OpacityOverride : register(c10); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_2 : register(c6); // float
float4 UniformScalar_3 : register(c7); // float
float4 UniformScalar_4 : register(c8); // float
float4 UniformScalar_5 : register(c9); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UpperSkyColor : register(c11); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord6 : TEXCOORD6; // r2
    float4 texcoord7 : TEXCOORD7; // r3
    float4 texcoord8 : TEXCOORD8; // r4
    float2 vPos : VPOS;   // r5 (pixel parameters)
    float vFace : VFACE;  // r5
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord6;
    float4 r3 = In.texcoord7;
    float4 r4 = In.texcoord8;
    float4 r5 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r6 = 0.0;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.y;
    r6.xw = r0.xy * UniformVector_1.xy;
    r0.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r11.xyz = -UniformVector_0.xyz + 1.0;
    r0.z = ps;
    r0.zw = r0.zw * abs(r5.xy);
    r6.yz = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r12.xyz = tex2D(Texture2D_3, r6.xw).xyz;
    r0.w = tex2D(Texture2D_2, r0.xy).x;
    r7.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r0.xyz = tex2D(Texture2D_1, r0.xy).zxy;
    r5.xyz = tex2D(Texture2D_0, r6.xw).xyz;
    r6.x = dot(r3.zxy, r3.zxy);
    r6.w = dot(r4.zxy, r4.zxy);
    r5.w = dot(r1.zxy, r1.zxy);
    r5.xyz = r5.zxy * 2.0 - 1.0;
    ps = rsqrt(abs(r5.w));
    r0.x = r0.x + r0.x;
    r5.w = ps;
    ps = rsqrt(abs(r6.w));
    r0.yz = r0.yz * 4.0;
    r6.w = ps;
    ps = rsqrt(abs(r6.x));
    r10.xyz = r6.www * r4.xyz;
    r4.w = ps;
    r9.x = max(UniformScalar_5.x, 0.0001);
    r8.xyz = -ModShadowColor.xyz + 1.0;
    r4.xyz = r7.xyz + r7.xyz;
    r13.xyz = r0.www * UniformVector_2.xzy;
    r7.xyz = r4.www * r3.xyz;
    r1.xyz = r5.www * r1.xzy;
    ps = 0.1 - -r1.y;
    r3.xyz = r0.xyz + float3(-1.0, -2.0, -2.0);
    r0.x = ps;
    ps = 5.0 * r0.x;
    r3.xyz = r3.xyz - r5.xyz;
    r0.z = saturate(ps);
    r3.xyz = r3.xyz * UniformScalar_2.xxx + r5.xyz;
    ps = OpacityOverride.x;
    r0.xz = -r0.wz + 1.0;
    r5.x = saturate(ps);
    ps = ModShadowGroupColor.x * r0.z;
    r0.y = dot(r3.xyz, r3.xyz);
    r5.y = ps;
    ps = rsqrt(abs(r0.y));
    r9.yzw = r0.xxx * r12.xyz;
    r0.y = ps;
    ps = ModShadowGroupColor.y * r0.z;
    r3.xyz = r3.xyz * r0.yyy;
    r5.z = ps;
    r0.xyz = r13.xyz * r12.xzy + r9.ywz;
    r4.xyz = r0.xyz * r4.xzy - r0.xyz;
    r9.yzw = -r5.xyz + 1.0;
    r1.w = dot(r3.xyz, r3.xyz);
    ps = rsqrt(abs(r1.w));
    r0.w = float((r9.y >= 0.004));
    r1.w = ps;
    r0.xyz = r4.xzy * UniformScalar_2.xxx + r0.xzy;
    r5.xyz = r0.xzy * UniformScalar_3.xxx + UniformScalar_4.xxx;
    r4.xyz = r3.xyz * r1.www;
    r3.xyz = r0.xyz * r11.xyz;
    r0.xyz = r3.xyz * AmbientColorAndSkyFactor.xyz + UniformVector_0.xyz;
    r6.x = dot(r10.zxy, r4.xyz);
    r1.w = dot(r4.xyz, r7.zxy);
    r10.xyz = r4.yzx * r1.www;
    r7.xyz = r10.xyz * 2.0 - r7.xyz;
    r6 = r6.xxyz * float4(-0.5, 0.5, 0.875, 0.875);
    r9.yz = r6.zw * r9.zw + 0.125;
    r1.w = saturate(dot(r1.yxz, r7.zxy));
    r1.xy = r6.xy + 0.5;
    r1.xy = abs(r1.xy) * abs(r1.xy);
    r7.xyz = r3.xzy * r1.xxx;
    ps = log2(r1.w);
    r1.xyz = r3.xzy * r1.yyy;
    r9.w = ps;
    r6.xyz = r1.xyz * UpperSkyColor.xzy + r0.xzy;
    r1.xw = r9.yx * r9.zw;
    r0.xyz = r1.xxx * r8.xyz + ModShadowColor.xyz;
    r1.xyz = r7.xzy * LowerSkyColor.xyz + r6.xzy;
    ps = pow(2.0, r1.w);
    r4.xyz = r4.xxx * ConstantLighting.xyz;
    r1.w = ps;
    r6.xyz = r1.www * ConstantLighting.xzy;
    r1.xyz = r6.xyz * r5.xyz + r1.xzy;
    r1.xyz = r4.xyz * r3.xyz + r1.xzy;
    ps = -r2.w;
    r0.xyz = r1.xyz * r0.xyz;
    ps = OpacityOverride.x + ps;
    r1.xyz = r0.xyz - r0.xyz;
    r1.w = ps;
    oC0.w = r1.w * r0.w + r2.w;
    r0.xyz = r1.xyz * r0.www + r0.xyz;
    oC0.xyz = r0.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
