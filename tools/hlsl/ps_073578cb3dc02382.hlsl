// ps_073578cb3dc02382.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 291 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 0000048C 10041000 0000090A 00000000 00008129 003F01FF 00000021 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c12); // float3
float4 ModShadowAccumResolution : register(c15); // float2
float4 ModShadowColor : register(c13); // float3
float4 ModShadowGroupColor : register(c14); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_10 : register(c11); // float
float4 UniformScalar_7 : register(c8); // float
float4 UniformScalar_8 : register(c9); // float
float4 UniformScalar_9 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_6 : register(c7); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D ShadowTexture : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r3 = tex2D(Texture2D_5, r1.xy);
    ps = (-0.5) + r3.w;
    r6.xy = r1.wz * 6.0;
    r6.z = ps;
    ps = UniformVector_2.x * r1.w;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.zzzz)) clip(-1.0);
    r8.x = ps;
    r11.x = tex2D(ShadowTexture, r0.xy).x;
    r6.yzw = tex2D(Texture2D_2, r6.xy).xwy;
    r7.xy = r1.wz * UniformVector_1.xy;
    ps = 1.0 / ModShadowAccumResolution.x;
    r7.w = r5.w - 4e+02;
    r0.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.x = -r1.z + 1.0;
    r0.y = ps;
    ps = 1.0 / r5.w;
    r11.yz = r5.xy * ScreenPositionScaleBias.xy;
    r11.w = ps;
    ps = UniformVector_2.y * r6.x;
    r7.z = dot(r4.zxy, r4.zxy);
    r8.y = ps;
    r12.yz = r6.yw * 2.0 - 1.0;
    ps = rsqrt(abs(r7.z));
    r12.x = saturate(r7.w * 0.00022222222);
    r6.x = ps;
    ps = r11.x * r11.x;
    r10.xyz = r6.xxx * r4.xyz;
    r6.w = ps;
    ps = r11.y * r11.w;
    r13.xyw = r12.yxz * float3(0.5, -0.025, 0.5);
    r6.y = ps;
    ps = r11.z * r11.w;
    r8.zw = r10.xy * r13.yy;
    r6.x = ps;
    ps = r0.x;
    r7.zw = r8.xy - 0.5;
    ps = abs(r9.x) * ps;
    r16.xy = float2((r8.xy >= 0.5));
    r0.z = ps;
    ps = r0.y;
    r7.zw = r7.zw - r8.xy;
    ps = abs(r9.y) * ps;
    r7.zw = r7.zw * r16.xy;
    r0.w = ps;
    ps = ScreenPositionScaleBias.w + r6.y;
    r7 = r8.zwxy + r7;
    r8.x = ps;
    ps = ScreenPositionScaleBias.z + r6.x;
    r0.xy = r7.zw + r7.zw;
    r8.y = ps;
    r11.xyz = tex2D(Texture2D_6, r7.xy).xyz;
    r8.xyz = tex2D(LightAttenuationTexture, r8.xy).xyz;
    r13.yz = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r14 = tex2D(Texture2D_1, r0.xy);
    r7.xyw = tex2D(Texture2D_0, r7.xy).xyz;
    r4.xyz = tex2D(Texture2D_3, r1.xy).xyz;
    r15.xyz = tex2D(Texture2D_2, r1.wz).xyw;
    r6.y = tex2D(Texture2D_4, r1.xy).x;
    r5.y = saturate(r5.w * 0.0001);
    r1.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r0.xyz = -ModShadowColor.xyz + 1.0;
    r6.x = dot(r2.zxy, r2.zxy);
    r5.x = r6.z * r15.z - 1.0;
    r9.xyw = r4.xyz * 2.0 - 1.0;
    r7.xyz = r7.xyw * 2.0 - 1.0;
    ps = rsqrt(abs(r6.x));
    r12.xyz = r1.xyz * r3.xyz;
    r6.x = ps;
    r3.xyz = r12.xyz * UniformScalar_9.xxx + UniformScalar_10.xxx;
    ps = r15.x + r15.x;
    r4.xyz = r6.xxx * r2.xyz;
    r1.y = ps;
    r9.w = r9.w * UniformVector_4.z - r7.z;
    ps = r15.y + r15.y;
    r1.x = dot(r16.xyy, float3(1.0, 1.0, 1.0));
    r1.z = ps;
    r6.z = (r1.x == 0.0) ? r14.x : r14.y;
    ps = r2.z;
    r1 = r1.xxyz + float4(-3.0, -2.0, -1.0, -1.0);
    ps = 0.1 + ps;
    r2.xy = r1.zw + r13.xw;
    r6.x = ps;
    r6.z = (r1.y == 0.0) ? r14.z : r6.z;
    r1.x = (r1.x == 0.0) ? r14.w : r6.z;
    ps = 5.0 * r6.x;
    r8.w = max(r1.x, 0.0);
    r1.y = saturate(ps);
    r6.xz = -r1.xy + 1.0;
    ps = ModShadowGroupColor.x * r6.z;
    r13.x = min(r8.w, 0.3);
    r5.z = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r1.xyz = r13.yxz * float3(0.875, 3.3333333, 0.875);
    r5.w = ps;
    ps = 2.5 * r6.x;
    r5.yzw = -r5.wzy + 1.0;
    r12.w = ps;
    r6.xz = r1.yy * r2.xy - r7.xy;
    r6.xz = r9.xy * UniformVector_4.xy + r6.xz;
    r1.xyz = r1.yxz * r5.xzy + float3(1.0, 0.125, 0.125);
    ps = abs(r1.x) * abs(r1.x);
    r8.xyz = r6.www * r8.xyz;
    r1.w = ps;
    r9.xyz = r12.xyz * r1.xxx - r11.xyz;
    r8.w = saturate(r12.w * r1.x - 0.5);
    r7.xy = r6.xz * r8.ww + r7.xy;
    r2.xyz = r9.xyz * r8.www + r11.xyz;
    r7.z = r9.w * r8.w - 2.0;
    r1.yw = r1.wy * r1.wz;
    r6.xzw = r1.www * r0.xyz + ModShadowColor.xyz;
    r7.z = r7.w * 2.0 + r7.z;
    r7.xyz = r7.zxy * r5.www;
    ps = 1.0 + r7.x;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r7.w = ps;
    r7.xyz = (r5.www > 0.0) ? r7.yzw : float3(0.0, 0.0, 1.0);
    r7.yzw = (r5.www >= 0.0) ? r7.xyz : float3(0.0, 0.0, 1.0);
    r7.x = dot(r7.wyz, r7.wyz);
    ps = rsqrt(abs(r7.x));
    r0.xyz = r2.xyz * r0.xyz;
    r7.x = ps;
    r7.yzw = r7.yzw * r7.xxx;
    r7.x = dot(r7.wyz, r10.zxy);
    r2.xyz = r7.yzw * r7.xxx;
    r2.xyz = r2.xyz * 2.0 - r10.xyz;
    r7.x = saturate(dot(r4.zxy, r2.zxy));
    ps = log2(r7.x);
    r7.w = saturate(dot(r7.wzy, r4.zyx));
    r7.y = ps;
    ps = UniformScalar_8.x * r6.y;
    r7.x = r7.y * UniformScalar_7.x;
    r1.x = ps;
    ps = pow(2.0, r7.x);
    r6.y = r7.y * 15.0;
    r1.z = ps;
    r1.xy = r1.xy * r1.zy;
    r7.xyz = r3.xyz + r1.xxx;
    r7.xyz = r1.yyy * r7.xyz;
    ps = pow(2.0, r6.y);
    r7.xyz = r7.xyz * r8.www;
    r6.y = ps;
    r7.xyz = r7.xyz * r6.yyy;
    r7.xy = r0.xy * r7.ww + r7.xy;
    r7.z = r0.z * r7.w + r7.z;
    r7.xyz = r8.xyz * r7.xyz;
    r7.xyz = r7.xzy * LightColor.xzy;
    r6.xyz = r7.xzy * r6.xzw;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
