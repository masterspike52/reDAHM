// ps_4f96847ccfa4df34.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 318 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000004F8 10041300 0000080A 00000000 00008108 003F00FF 00000021 0000F050 0000F153 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD3 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c14); // float4
float4 LowerSkyColor : register(c13); // float3
float4 ModShadowAccumResolution : register(c17); // float2
float4 ModShadowColor : register(c15); // float3
float4 ModShadowGroupColor : register(c16); // float3
float4 OpacityOverride : register(c11); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_3 : register(c9); // float
float4 UniformScalar_4 : register(c10); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UpperSkyColor : register(c12); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
sampler2D ModShadowAccumTexture : register(s8);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord3 : TEXCOORD3; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord3;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r9 = tex2D(Texture2D_5, r0.xy);
    ps = (-0.5) + r9.w;
    r6.xy = r0.wz * 6.0;
    r6.z = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r6.zzzz)) clip(-1.0);
    r10.xyw = tex2D(Texture2D_2, r6.xy).xyw;
    ps = r0.w;
    r7.zw = r0.wz * UniformVector_6.xy;
    ps = UniformVector_2.x * ps;
    r13.xy = r0.wz * UniformVector_1.xy;
    r14.x = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.x = r3.w - 4e+02;
    r7.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.w = dot(r4.zxy, r4.zxy);
    r7.y = ps;
    r6.yz = r10.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r6.w));
    r6.x = saturate(r6.x * 0.00022222222);
    r6.w = ps;
    ps = -r0.z;
    r10.xyz = r6.www * r4.xyz;
    ps = 1.0 + ps;
    r11.xyw = r6.yxz * float3(0.5, -0.025, 0.5);
    r6.x = ps;
    ps = UniformVector_2.y * r6.x;
    r14.zw = r10.xy * r11.yy;
    r14.y = ps;
    r6.xy = r14.xy - 0.5;
    r12.xy = float2((r14.xy >= 0.5));
    ps = r7.x;
    r6.xy = r6.xy - r14.xy;
    ps = abs(r8.x) * ps;
    r13.zw = r6.xy * r12.xy;
    r4.x = ps;
    ps = r7.y;
    r6 = r14.zwxy + r13;
    ps = abs(r8.y) * ps;
    r7.xy = r6.zw + r6.zw;
    r4.y = ps;
    r11.yz = tex2D(ModShadowAccumTexture, r4.xy).xy;
    r16.xyz = tex2D(Texture2D_4, r6.xy).xyz;
    r17.yzw = tex2D(Texture2D_6, r7.zw).xyz;
    r19 = tex2D(Texture2D_1, r7.xy);
    r7.xyw = tex2D(Texture2D_0, r6.xy).xyz;
    r4.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r8.xyz = tex2D(Texture2D_2, r0.wz).xyw;
    r13.xyz = tex2D(Texture2D_7, r0.xy).xyz;
    ps = OpacityOverride.x;
    r7.z = saturate(r3.w * 0.0001);
    r8.w = saturate(ps);
    ps = UniformVector_5.x * UniformVector_5.w;
    r0.w = dot(r5.zxy, r5.zxy);
    r0.x = ps;
    ps = UniformVector_5.y * UniformVector_5.w;
    r6.z = dot(r2.zxy, r2.zxy);
    r0.y = ps;
    ps = UniformVector_5.z * UniformVector_5.w;
    r6.xyw = -UniformVector_0.yxz + 1.0;
    r0.z = ps;
    r14.yzw = r13.xyz * UniformScalar_3.xxx + UniformScalar_4.xxx;
    r17.x = r10.w * r8.z - 1.0;
    r15.zw = r8.xy * 2.0 - 1.0;
    ps = rsqrt(abs(r6.z));
    r18.xyz = r0.xyz * r9.xyz;
    r6.z = ps;
    r4.xyz = r4.zxy * 2.0 - 1.0;
    r13.xyz = r7.wxy * 2.0 - 1.0;
    r14.x = r4.x * UniformVector_4.z - r13.x;
    ps = AmbientColorAndSkyFactor.x * r6.y;
    r9.xyz = r6.zzz * r2.xyz;
    r15.x = ps;
    ps = AmbientColorAndSkyFactor.y * r6.x;
    r9.w = dot(r12.xyy, float3(1.0, 1.0, 1.0));
    r15.y = ps;
    r6.z = (r9.w == 0.0) ? r19.x : r19.y;
    ps = rsqrt(abs(r0.w));
    r0.xyz = r9.zww + float3(0.1, -3.0, -2.0);
    r0.w = ps;
    ps = 5.0 * r0.x;
    r12.xyz = r0.www * r5.xyz;
    r7.y = saturate(ps);
    r6.z = (r0.z == 0.0) ? r19.z : r6.z;
    r8.z = (r0.y == 0.0) ? r19.w : r6.z;
    ps = 1.0 - r7.y;
    r2.yzw = r18.xyz * r17.yzw;
    r6.z = ps;
    ps = 1.0 - r7.z;
    r0.y = max(r8.z, 0.0);
    r0.x = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r7.x = min(r0.y, 0.3);
    r8.x = ps;
    ps = 3.3333333 * r7.x;
    r17.yz = r15.zw + r11.xw;
    r7.z = ps;
    ps = ModShadowGroupColor.y * r6.z;
    r7.xyz = r7.zzz * r17.xyz;
    r8.y = ps;
    ps = AmbientColorAndSkyFactor.z * r6.w;
    r8 = -r8.wxyz + 1.0;
    r15.z = ps;
    r0.zw = r4.yz * UniformVector_4.xy + r7.yz;
    ps = r8.w;
    r7.z = r7.x + 1.0;
    r6.z = ps;
    ps = 2.5 * r6.z;
    r0.y = abs(r7.z) * abs(r7.z);
    r2.x = ps;
    ps = r0.y * r0.y;
    r4.yz = r0.zw - r13.yz;
    r4.x = ps;
    r0.yzw = r2.yzw * r7.zzz - r16.xyz;
    r4.w = saturate(r2.x * r7.z - 0.5);
    r2.xyz = r0.yzw * r4.www + r16.xyz;
    r5.xyz = r15.xyz * r2.xyz + UniformVector_0.xyz;
    r7.xyz = r4.zxy * r4.wxw;
    ps = r13.y;
    r0.y = float((r8.x >= 0.004));
    ps = r7.z + ps;
    r14.yzw = r7.yyy * r14.yzw;
    r7.y = ps;
    ps = r13.z;
    r4 = r14.xywz * r4.wwww;
    ps = r7.x + ps;
    r6.z = r4.x - 2.0;
    r7.z = ps;
    r7.x = r7.w * 2.0 + r6.z;
    r7.yzw = r7.xyz * r0.xxx;
    ps = 1.0 + r7.y;
    r7.x = ps;
    r7.xyz = (r0.xxx > 0.0) ? r7.xzw : float3(1.0, 0.0, 0.0);
    r7.xyz = (r0.xxx >= 0.0) ? r7.xyz : float3(1.0, 0.0, 0.0);
    r6.z = dot(r7.xyz, r7.xyz);
    ps = rsqrt(abs(r6.z));
    r0.xzw = r2.xyz * r6.yxw;
    r6.x = ps;
    r6.yzw = r7.xyz * r6.xxx;
    r2.xyz = r6.yyy * r1.xyz;
    r11.x = dot(r12.zxy, r6.yzw);
    ps = -ModShadowColor.x;
    r6.x = dot(r6.yzw, r10.zxy);
    ps = 1.0 + ps;
    r6.xzw = r6.zwy * r6.xxx;
    r6.y = ps;
    ps = -ModShadowColor.y;
    r7 = r11.xyzx * float4(-0.5, 0.875, 0.875, 0.5);
    r6.xzw = r6.xzw * 2.0 - r10.xyz;
    ps = 1.0 + ps;
    r6.x = saturate(dot(r9.zxy, r6.wxz));
    r6.z = ps;
    r7.yz = r7.yz * r8.yz + 0.125;
    ps = -ModShadowColor.z;
    r7.xw = r7.xw + 0.5;
    ps = 1.0 + ps;
    r7.xw = abs(r7.wx) * abs(r7.wx);
    r6.w = ps;
    ps = log2(r6.x);
    r7.y = r7.y * r7.z;
    r6.x = ps;
    r6.yzw = r7.yyy * r6.yzw + ModShadowColor.xyz;
    ps = 15.0 * r6.x;
    r8.xyz = r0.xwz * r7.xxx;
    r6.x = ps;
    r7.xyz = r8.xyz * UpperSkyColor.xzy + r5.xzy;
    ps = pow(2.0, r6.x);
    r5.xyz = r0.xwz * r7.www;
    r6.x = ps;
    r1.xyz = r6.xxx * r1.xzy;
    r7.xyz = r5.xzy * LowerSkyColor.xyz + r7.xzy;
    r7.xyz = r1.xyz * r4.yzw + r7.xzy;
    r7.xyz = r2.xyz * r0.xzw + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xyz * r6.yzw;
    ps = OpacityOverride.x + ps;
    r7.xyz = r6.xyz - r6.xyz;
    r7.w = ps;
    oC0.w = r7.w * r0.y + r3.w;
    r6.xyz = r7.xyz * r0.yyy + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
