// ps_c04aeedb62a01444.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 216 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000360 10040E00 0000070A 00000000 000068E7 001F007F 00000001 0000F050 00007154 0000F255 00007356 0000F457 0000F5A0 0000F6A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r2 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> COLOR0 (flags 0xF)
//   interpolator: r6 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c10); // float4
float4 ModShadowAccumResolution : register(c13); // float2
float4 ModShadowColor : register(c11); // float3
float4 ModShadowGroupColor : register(c12); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c5); // float
float4 UniformScalar_21 : register(c6); // float
float4 UniformScalar_22 : register(c7); // float
float4 UniformScalar_23 : register(c8); // float
float4 UniformScalar_24 : register(c9); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_2 : register(c4); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D ModShadowAccumTexture : register(s6);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord4 : TEXCOORD4; // r1
    float4 texcoord5 : TEXCOORD5; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 color0 : COLOR0; // r5
    float4 color2 : COLOR2; // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord4;
    float4 r2 = In.texcoord5;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.color0;
    float4 r6 = In.color2;
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r5.x = ps;
    ps = 1.0 / r4.w;
    r5.y = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r5.zw = r5.yy * ScreenPositionScaleBias.xy;
    r5.y = ps;
    r5.xy = r5.xy * abs(r7.xy);
    r5.zw = r5.zw * r4.xy + ScreenPositionScaleBias.wz;
    r4.xyz = tex2D(LightAttenuationTexture, r5.zw).xyz;
    r7.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r13.xyz = tex2D(Texture2D_2, r0.xy).xyz;
    r9.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r6.xyz = tex2D(Texture2D_1, r0.xy).zxy;
    r11.xyz = tex2D(Texture2D_0, r0.xy).xyz;
    r5.xz = tex2D(ModShadowAccumTexture, r5.xy).xy;
    ps = UniformVector_2.x * UniformVector_2.x;
    r7.w = max(UniformScalar_24.x, 0.0001);
    r8.y = ps;
    ps = UniformVector_2.z * UniformVector_2.z;
    r12.z = UniformScalar_22.x - UniformScalar_21.x;
    r8.z = ps;
    ps = r1.z;
    r5.y = dot(r2.zxy, r2.zxy);
    ps = 0.1 + ps;
    r10.yz = r5.xz * 0.875;
    r5.x = ps;
    r2.xzw = r11.zxy * 2.0 - 1.0;
    ps = r6.x + r6.x;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r14.z = ps;
    ps = 5.0 * r5.x;
    r6.w = dot(r3.zxy, r3.zxy);
    r5.x = saturate(ps);
    ps = 4.0 * r6.y;
    r0.w = dot(r1.zxy, r1.zxy);
    r14.x = ps;
    ps = 4.0 * r6.z;
    r5.z = -r5.x + 1.0;
    r14.y = ps;
    ps = ModShadowGroupColor.x * r5.z;
    r9.xyz = r9.xyz + r9.xyz;
    r12.y = ps;
    ps = ModShadowGroupColor.y * r5.z;
    r6.xyz = r14.yxz + float3(-2.0, -2.0, -1.0);
    r12.w = ps;
    ps = r6.x;
    r12.x = dot(r13.zxy, float3(1.0, 1.0, 1.0));
    ps = -r2.w + ps;
    r5.xzw = -r12.yxw + 1.0;
    r9.w = ps;
    ps = UniformScalar_23.x * r5.z;
    r12.xy = r6.yz - r2.zx;
    r2.x = ps;
    ps = UniformVector_2.y * r5.z;
    r6.xyz = r12.xzy * UniformScalar_0.xxx;
    r2.y = ps;
    ps = UniformScalar_21.x + r6.y;
    r7.xyz = r5.zzz * r7.xyz;
    r1.w = ps;
    r9.xyz = r7.xyz * r9.xyz - r7.xyz;
    ps = r2.z;
    r3.w = r6.z - 2.0;
    r10.x = r11.z * 2.0 + r3.w;
    r7.xyz = r9.xyz * UniformScalar_0.xxx + r7.xyz;
    r8.x = r9.w * UniformScalar_0.x + r2.w;
    ps = r6.x + ps;
    r8.w = r2.y * r8.x;
    r10.w = ps;
    r12.xy = r10.xw * r5.zz + float2(1.0, 0.0);
    r11.yzw = r10.xyz * r5.zxw + float3(1.0, 0.125, 0.125);
    ps = r8.z;
    r6.xyz = -ModShadowColor.xyz + 1.0;
    ps = r11.y * ps;
    r12.zw = r8.xy * r5.zz;
    r11.x = ps;
    ps = r12.w;
    r9.xyz = r7.xyz * r1.www;
    ps = r10.w * ps;
    r8.xyz = r12.yxz * UniformVector_2.xzy;
    r1.w = ps;
    ps = r1.w;
    r2.yz = r11.xz * r11.yw;
    r6.xyz = r2.zzz * r6.xyz + ModShadowColor.xyz;
    ps = r5.z * ps;
    r1.w = r8.w * r8.z;
    r5.x = ps;
    r9.w = r5.x * r10.w + r1.w;
    ps = rsqrt(abs(r0.w));
    r2 = r9 + r2.xxxy;
    r5.x = ps;
    ps = rsqrt(abs(r2.w));
    r1.xyz = r5.xxx * r1.xyz;
    r5.x = ps;
    r9.xyz = r8.xyw * r5.xxx;
    ps = rsqrt(abs(r6.w));
    r5.x = dot(r9.yxz, r9.yxz);
    r5.z = ps;
    ps = rsqrt(abs(r5.x));
    r8.xyz = r5.zzz * r3.xyz;
    r5.x = ps;
    r3.xyz = r9.xyz * r5.xxx;
    r5.x = dot(r3.yxz, r8.zxy);
    ps = 1.0 - r5.y;
    r9.xyz = r3.xzy * r5.xxx;
    r5.x = saturate(ps);
    r5.yzw = r9.xyz * 2.0 - r8.xyz;
    ps = log2(r5.x);
    r5.y = saturate(dot(r1.zxy, r5.wyz));
    r5.x = ps;
    ps = log2(r5.y);
    r5.x = r5.x * LightColorAndFalloffExponent.w;
    r0.w = ps;
    r0 = r7 * r0;
    ps = pow(2.0, r0.w);
    r6.w = saturate(dot(r3.yxz, r1.zxy));
    r5.y = ps;
    ps = pow(2.0, r5.x);
    r5.yzw = r2.xyz * r5.yyy;
    r5.x = ps;
    r5.yz = r0.xy * r6.ww + r5.yz;
    r5.w = r0.z * r6.w + r5.w;
    r5.xyz = r5.yzw * r5.xxx;
    r5.xyz = r5.xyz * r4.xyz;
    r5.xyz = r5.xyz * LightColorAndFalloffExponent.xyz;
    r5.xyz = r5.xzy * r6.xzy;
    oC0.xyz = r5.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
