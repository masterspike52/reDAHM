// ps_58635f0241dfcd5c.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 168 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002A0 10040C00 0000090A 00000000 00008929 003F01FF 00000001 0000F050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c8); // float3
float4 ModShadowAccumResolution : register(c11); // float2
float4 ModShadowColor : register(c9); // float3
float4 ModShadowGroupColor : register(c10); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c6); // float
float4 UniformScalar_1 : register(c7); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D ModShadowAccumTexture : register(s5);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.y;
    r6.y = r0.w * UniformVector_2.x;
    r7.y = ps;
    ps = 1.0 / r5.w;
    r6.x = -r0.z + 1.0;
    r6.z = ps;
    ps = ScreenPositionScaleBias.x * r6.z;
    r6.w = dot(r2.zxy, r2.zxy);
    r8.x = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r1.xyz = -UniformVector_0.xyz + 1.0;
    r7.x = ps;
    ps = rsqrt(abs(r6.w));
    r7.xy = r7.xy * abs(r9.xy);
    r7.w = ps;
    ps = ScreenPositionScaleBias.y * r6.z;
    r7.z = dot(r4.zxy, r4.zxy);
    r8.y = ps;
    r6.zw = r8.xy * r5.xy + ScreenPositionScaleBias.wz;
    ps = rsqrt(abs(r7.z));
    r3.xyw = r7.www * r2.xyz;
    r7.z = ps;
    ps = UniformVector_2.y * r6.x;
    r4.xyz = r7.zzz * r4.zxy;
    r6.x = ps;
    ps = (-0.5) + r6.y;
    r8.xy = r3.xy * -r4.yz;
    r8.z = ps;
    ps = (-0.5) + r6.x;
    r11.xy = float2((r6.yx >= 0.5));
    r8.w = ps;
    r8.zw = r8.zw - r6.yx;
    r8.zw = r8.zw * r11.xy + r6.yx;
    r3.xyz = tex2D(Texture2D_3, r0.wz).xyz;
    r9.yzw = tex2D(Texture2D_4, r0.xy).xyz;
    ps = 0.1 - -r2.z;
    r8.xyw = r8.zwx + r8.zwy;
    r6.x = ps;
    r10 = tex2D(Texture2D_2, r8.xy);
    r12.xyz = tex2D(LightAttenuationTexture, r6.zw).xyz;
    r5 = tex2D(Texture2D_1, r0.xy);
    r6.yz = tex2D(ModShadowAccumTexture, r7.xy).xy;
    ps = 5.0 * r6.x;
    r0.xy = r6.yz * 0.875;
    r6.x = saturate(ps);
    ps = r5.w;
    r4.yzw = -ModShadowColor.xyz + 1.0;
    r6.y = ps;
    ps = 1.0 - r6.y;
    r6.z = r5.w * UniformScalar_0.x;
    r6.y = ps;
    r7.xyz = r6.yyy * UniformVector_1.xyz + r6.zzz;
    ps = 1.0 - r6.x;
    r8.xyz = r12.xyz * LightColor.xyz;
    r6.z = ps;
    r4.x = r4.x * 2.0 - r4.x;
    ps = ModShadowGroupColor.x * r6.z;
    r6.y = dot(r11.xyy, float3(1.0, 1.0, 1.0));
    r6.x = ps;
    r6.w = (r6.y == 0.0) ? r10.x : r10.y;
    ps = ModShadowGroupColor.y * r6.z;
    r0.zw = r6.yy + float2(-3.0, -2.0);
    r6.y = ps;
    r6.z = (r0.w == 0.0) ? r10.z : r6.w;
    r6.w = (r0.z == 0.0) ? r10.w : r6.z;
    r6.z = saturate(r3.w * r4.x + r8.w);
    ps = log2(r6.z);
    r6.xyw = -r6.xyw + 1.0;
    r9.x = ps;
    r6.w = saturate(r6.w * 2.5 - 0.5);
    r6.xy = r0.xy * r6.xy + 0.125;
    ps = r6.x * r6.y;
    r0 = r9.yzwx * float4(0.5, 0.5, 0.5, 15.0);
    r6.x = ps;
    r6.xyz = r6.xxx * r4.yzw + ModShadowColor.xyz;
    r4.xyz = r0.xyz * r5.www;
    r7.xyz = r4.xyz * UniformScalar_1.xxx + r7.xyz;
    r4.xyz = r5.xyz * 2e+01 + r4.xyz;
    r0.xyz = r4.xyz - r3.xyz;
    ps = pow(2.0, r0.w);
    r4.xyz = r7.xyz * r6.www;
    r7.x = ps;
    ps = r7.w;
    r7.xyz = r4.xyz * r7.xxx;
    r0.xyz = r0.xyz * r6.www + r3.xyz;
    ps = r2.z * ps;
    r0.xyz = r0.xyz * r1.xyz;
    r6.w = saturate(ps);
    r7.xy = r0.xy * r6.ww + r7.xy;
    r7.z = r0.z * r6.w + r7.z;
    r7.xyz = r8.xyz * r7.xyz;
    r6.xyz = r7.xzy * r6.xzy;
    oC0.xyz = r6.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
