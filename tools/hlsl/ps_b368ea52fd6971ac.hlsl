// ps_b368ea52fd6971ac.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 153 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 00000264 10040D00 0000080A 00000000 00008108 003F00FF 00000001 0000F050 0000F153 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0xF)
//   interpolator: r1 <-> TEXCOORD3 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c7); // float4
float4 LowerSkyColor : register(c6); // float3
float4 ModShadowAccumResolution : register(c10); // float2
float4 ModShadowColor : register(c8); // float3
float4 ModShadowGroupColor : register(c9); // float3
float4 OpacityOverride : register(c4); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformVector_0 : register(c3); // float4
float4 UpperSkyColor : register(c5); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D ModShadowAccumTexture : register(s2);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord3 : TEXCOORD3; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color2 : COLOR2; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord3;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color2;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    ps = 1.0 / ModShadowAccumResolution.x;
    r7.xyz = -UniformVector_0.zxy + 1.0;
    r6.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r6.y = dot(r4.zxy, r4.zxy);
    r6.z = ps;
    r6.xz = r6.xz * abs(r8.xy);
    r11.yw = tex2D(ModShadowAccumTexture, r6.xz).xy;
    r8.xyz = tex2D(Texture2D_1, r0.xy).xyz;
    r0.xyz = tex2D(Texture2D_0, r0.xy).xyz;
    r10.xyz = -ModShadowColor.xyz + 1.0;
    r7.w = dot(r2.zxy, r2.zxy);
    r6.x = dot(r5.zxy, r5.zxy);
    r13.xyz = r0.zxy * 2.0 - 1.0;
    ps = OpacityOverride.x;
    r6.z = dot(r8.zxy, float3(0.5, 0.5, 0.5));
    r11.z = saturate(ps);
    ps = rsqrt(abs(r6.y));
    r9.xyz = r6.zzz - r8.xyz;
    r6.y = ps;
    ps = rsqrt(abs(r6.x));
    r12.xyz = r6.yyy * r4.xyz;
    r6.x = ps;
    ps = AmbientColorAndSkyFactor.x * r7.y;
    r6.xyz = r6.xxx * r5.xyz;
    r5.x = ps;
    ps = rsqrt(abs(r7.w));
    r6.w = dot(r13.xyz, r13.xyz);
    r7.w = ps;
    ps = rsqrt(abs(r6.w));
    r0.xyz = r7.www * r2.xzy;
    r6.w = ps;
    ps = AmbientColorAndSkyFactor.y * r7.z;
    r2.xyz = r13.xyz * r6.www;
    r5.y = ps;
    ps = AmbientColorAndSkyFactor.z * r7.x;
    r4.xyz = r2.xxx * r1.xyz;
    r5.z = ps;
    ps = 0.1 - -r0.y;
    r11.x = dot(r6.zxy, r2.xyz);
    r6.x = ps;
    ps = 5.0 * r6.x;
    r6.y = dot(r2.xyz, r12.zxy);
    r6.x = saturate(ps);
    ps = 1.0 - r6.x;
    r13.xyz = r2.yzx * r6.yyy;
    r6.z = ps;
    ps = ModShadowGroupColor.x * r6.z;
    r2 = r11.xxyw * float4(0.5, -0.5, 0.875, 0.875);
    r11.x = ps;
    r6.xyw = r13.xyz * 2.0 - r12.xyz;
    ps = ModShadowGroupColor.y * r6.z;
    r6.y = saturate(dot(r0.yxz, r6.wxy));
    r11.y = ps;
    ps = r2.y;
    r0.xyz = -r11.xyz + 1.0;
    r6.x = ps;
    ps = 0.5 + r6.x;
    r6.w = float((r0.z >= 0.004));
    r7.w = ps;
    r6.xz = r2.zw * r0.xy + 0.125;
    ps = log2(r6.y);
    r6.x = r6.x * r6.z;
    r9.w = ps;
    r6.xyz = r6.xxx * r10.xyz + ModShadowColor.xyz;
    ps = 0.5 + r2.x;
    r9 = r9 * float4(0.5, 0.5, 0.5, 15.0);
    r0.w = ps;
    ps = pow(2.0, r9.w);
    r0.xyz = r9.xyz + r8.xyz;
    r2.x = ps;
    ps = abs(r7.w) * abs(r7.w);
    r1.xyz = r2.xxx * r1.xzy;
    r7.w = ps;
    r5.xyz = r5.xyz * r0.xyz + UniformVector_0.xyz;
    ps = abs(r0.w) * abs(r0.w);
    r0.xyz = r0.xyz * r7.yzx;
    r7.x = ps;
    r2.xyz = r0.xzy * r7.www;
    r7.xyz = r0.xzy * r7.xxx;
    r7.xyz = r7.xzy * UpperSkyColor.xyz + r5.xyz;
    r7.xyz = r2.xyz * LowerSkyColor.xzy + r7.xzy;
    r7.xyz = r1.xzy * r8.zzz + r7.xzy;
    r7.xyz = r4.xzy * r0.xzy + r7.xzy;
    ps = -r3.w;
    r6.xyz = r7.xzy * r6.xyz;
    ps = OpacityOverride.x + ps;
    r7.xyz = r6.xyz - r6.xyz;
    r7.w = ps;
    oC0.w = r7.w * r6.w + r3.w;
    r6.xyz = r7.xyz * r6.www + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
