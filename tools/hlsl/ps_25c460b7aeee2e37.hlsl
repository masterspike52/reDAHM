// ps_25c460b7aeee2e37.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 384 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 00000600 10041400 00000606 00000000 000048C6 003F003F 00000021 00003050 00003151 00007254 0000F355 00007456 0000F557
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColor : register(c17); // float3
float4 ModShadowAccumResolution : register(c20); // float2
float4 ModShadowColor : register(c18); // float3
float4 ModShadowGroupColor : register(c19); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_1 : register(c10); // float
float4 UniformScalar_10 : register(c16); // float
float4 UniformScalar_2 : register(c11); // float
float4 UniformScalar_4 : register(c12); // float
float4 UniformScalar_5 : register(c13); // float
float4 UniformScalar_8 : register(c14); // float
float4 UniformScalar_9 : register(c15); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
float4 UniformVector_4 : register(c6); // float4
float4 UniformVector_5 : register(c7); // float4
float4 UniformVector_6 : register(c8); // float4
float4 UniformVector_7 : register(c9); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_0 : register(s1);
sampler2D Texture2D_1 : register(s2);
sampler2D Texture2D_2 : register(s3);
sampler2D Texture2D_3 : register(s4);
sampler2D Texture2D_4 : register(s5);
sampler2D Texture2D_5 : register(s6);
sampler2D Texture2D_6 : register(s7);
sampler2D Texture2D_7 : register(s8);
sampler2D Texture2D_8 : register(s9);
sampler2D ModShadowAccumTexture : register(s10);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float2 vPos : VPOS;   // r6 (pixel parameters)
    float vFace : VFACE;  // r6
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = float4(In.vFace < 0.0 ? -In.vPos.x : In.vPos.x, In.vPos.y, 0.0, 0.0);
    float4 r7 = 0.0;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 r20 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r9 = tex2D(Texture2D_8, r0.xy);
    ps = (-0.5) + r9.w;
    r12.y = UniformScalar_2.x * UniformVector_6.y;
    r0.z = ps;
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r0.zzzz)) clip(-1.0);
    r7.xyz = tex2D(Texture2D_3, r0.xy).xyz;
    r11.x = r1.x * UniformVector_2.x;
    ps = UniformVector_6.y;
    r8.xy = r1.xy * 6.0;
    ps = UniformScalar_5.x * ps;
    r11.z = -r1.y + 1.0;
    r12.w = ps;
    ps = r5.x;
    r8.zw = r1.xy * UniformVector_7.xy;
    ps = ScreenPositionScaleBias.x * ps;
    r3.xy = r1.xy * UniformVector_1.xy;
    r3.z = ps;
    ps = r5.y;
    r2.w = dot(r4.zxy, r4.zxy);
    ps = ScreenPositionScaleBias.y * ps;
    r1.z = r2.z + 0.1;
    r3.w = ps;
    ps = 1.0 / UniformVector_1.x;
    r1.w = r5.w - 4e+02;
    r11.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r0.z = UniformScalar_1.x * UniformVector_4.x;
    r15.x = ps;
    ps = 1.0 / ModShadowAccumResolution.y;
    r0.w = UniformVector_4.x * UniformScalar_4.x;
    r15.y = ps;
    ps = UniformVector_6.x * r0.z;
    r14.xyz = r7.xyz + r7.xyz;
    r12.x = ps;
    ps = rsqrt(abs(r2.w));
    r1.zw = saturate(r1.zw * float2(5.0, 0.00022222222));
    r2.w = ps;
    ps = 1.0 / UniformVector_1.y;
    r7.xyz = r2.www * r4.xyz;
    r2.w = ps;
    ps = 1.0 / r5.w;
    r4.y = r1.w * (-0.025);
    r4.x = ps;
    r3.xy = r7.xy * r4.yy + r3.xy;
    r10.xy = r3.zw * r4.xx + ScreenPositionScaleBias.wz;
    r11.y = -r3.y * r2.w + 1.0;
    ps = UniformVector_6.x * r0.w;
    r11.yzw = r11.zyw * UniformVector_2.yyx;
    r12.z = ps;
    ps = r11.w;
    r0 = r12 + r0.xyxy;
    ps = r3.x * ps;
    r4.xyz = r11.zxy - 0.5;
    r14.w = ps;
    r18.x = float((r14.w >= 0.5));
    r18.yzw = float3((r11.xzy >= 0.5));
    ps = r15.x;
    r12.yzw = r4.xzy - r11.zyx;
    ps = abs(r6.x) * ps;
    r13 = r14 + float4(-1.0, -1.0, -1.0, -0.5);
    r10.z = ps;
    ps = r15.y;
    r12.x = r13.w - r14.w;
    r4.x = r12.x * r18.x + r14.w;
    r4.yzw = r12.ywz * r18.zyw + r11.zxy;
    ps = abs(r6.y) * ps;
    r4 = r4.zwxy + r4.zwxy;
    r10.w = ps;
    r6.xyz = tex2D(Texture2D_6, r3.xy).xyz;
    r16.xy = tex2D(ModShadowAccumTexture, r10.zw).xy;
    r17.xyw = tex2D(LightAttenuationTexture, r10.xy).xyz;
    r11 = tex2D(Texture2D_1, r4.xy);
    r15 = tex2D(Texture2D_1, r4.zw);
    r19 = tex2D(Texture2D_4, r0.xy);
    r20 = tex2D(Texture2D_5, r0.zw);
    r4.xyz = tex2D(Texture2D_7, r8.zw).xyz;
    r3.xyw = tex2D(Texture2D_0, r3.xy).xyz;
    r0.xyz = tex2D(Texture2D_2, r8.xy).xyw;
    r8.xyz = tex2D(Texture2D_2, r1.xy).xwy;
    r16.zw = r0.xy * 2.0 - 1.0;
    r0.x = r0.z * r8.y;
    r0.w = dot(r2.zxy, r2.zxy);
    r12.xyz = UniformVector_5.xyz * 2e+01;
    r0.y = float((UniformScalar_8.x >= 1.0));
    r0.z = float((UniformScalar_8.x > 1.0));
    r10.xyz = (-abs(r0.zzz) >= 0.0) ? r4.xyz : 1.0;
    r4.xyz = r9.xyz * UniformScalar_9.xxx;
    r5.xyz = r20.xyz * r20.www;
    r5.xyz = r19.xyz * r19.www + r5.xyz;
    r10.xyz = (-abs(r0.yyy) >= 0.0) ? 1.0 : r10.xyz;
    r4.xyz = r4.xyz * r10.xyz + UniformScalar_10.xxx;
    ps = 0.0001 * r5.w;
    r12.xyz = r12.xyz * r5.xyz;
    r14.w = saturate(ps);
    ps = r3.x + r3.x;
    r0.y = dot(r12.zxy, float3(0.11, 0.3, 0.59));
    r5.x = ps;
    ps = r3.y + r3.y;
    r0.y = r0.y - r12.x;
    r5.y = ps;
    ps = r0.y;
    r5.w = dot(r18.yww, float3(1.0, 1.0, 1.0));
    r5.z = r18.z * 2.0 + r18.x;
    r1.x = (r5.z == 0.0) ? r15.x : r15.y;
    r0.y = (r5.w == 0.0) ? r11.x : r11.y;
    ps = r12.x + ps;
    r14.xyz = r13.xyz * UniformVector_4.xyz;
    r0.z = saturate(ps);
    r11.xy = -r14.xy * r0.zz + r14.xy;
    ps = r3.w + r3.w;
    r13 = r5.zzww + float4(-3.0, -2.0, -2.0, -3.0);
    r5.z = ps;
    r0.y = (r13.z == 0.0) ? r11.z : r0.y;
    r1.x = (r13.y == 0.0) ? r15.z : r1.x;
    r5.w = (r13.x == 0.0) ? r15.w : r1.x;
    r1.x = (r13.w == 0.0) ? r11.w : r0.y;
    ps = 1.0 - r1.z;
    r0.y = max(r1.x, 0.0);
    r1.w = ps;
    ps = 1.0 - r1.x;
    r13 = r5 + float4(-1.0, -1.0, -1.0, 0.25);
    r1.x = ps;
    r8.yw = -r1.ww * ModShadowGroupColor.xy + 1.0;
    r1.y = r13.w + r5.w;
    ps = rsqrt(abs(r0.w));
    r1.y = saturate(r1.y - r0.x);
    r0.w = ps;
    ps = r8.x + r8.x;
    r15.xyz = -ModShadowColor.xyz + 1.0;
    r17.z = ps;
    ps = r8.z + r8.z;
    r3.xyz = r17.xyw * LightColor.xyz;
    r17.w = ps;
    ps = (-1.0) - -r0.x;
    r5.xyz = r0.www * r2.xyz;
    r0.x = ps;
    ps = 1.0 - r0.z;
    r16 = r16 * float4(0.875, 0.875, 0.5, 0.5);
    r0.w = ps;
    ps = r0.y;
    r4.xyz = r4.xyz * r0.www;
    r0.y = ps;
    ps = 0.3;
    r8.xz = -r14.zw + 1.0;
    r0.w = ps;
    ps = min(r0.y, r0.w);
    r17.xy = r16.xy * r8.yw;
    r0.y = ps;
    ps = 3.3333333 * r0.y;
    r2 = r17 + float4(0.125, 0.125, -1.0, -1.0);
    r8.y = ps;
    ps = r2.x * r2.y;
    r0.yw = r2.zw + r16.zw;
    r1.z = ps;
    r2.xyz = r1.zzz * r15.xyz + ModShadowColor.xyz;
    ps = 2.5 * r1.x;
    r0 = r8.yyyx * r0.ywxz;
    r1.z = ps;
    ps = r14.z;
    r6.xyz = r1.yyy * r6.xyz;
    ps = r0.w + ps;
    r1.y = r0.z + 1.0;
    r0.w = ps;
    ps = r1.y * r1.x;
    r0.xz = r11.xy + r0.xy;
    r0.y = ps;
    ps = r1.z * r1.y;
    r11.xyz = -r13.xyz + r0.xzw;
    r0.x = ps;
    ps = (-0.5) + r0.x;
    r0.z = float((r0.y > 0.9));
    r1.w = saturate(ps);
    r0.xy = r11.xy * r1.ww + r13.xy;
    r1.xyz = r12.xyz * r0.zzz - r6.xyz;
    r0.z = r11.z * r1.w - 2.0;
    r0.z = r3.w * 2.0 + r0.z;
    r1.xyz = r10.xyz * r9.xyz + r1.xyz;
    r6.xyz = r1.xyz * r1.www + r6.xyz;
    r0.xyz = r0.zxy * r8.zzz;
    ps = 1.0 + r0.x;
    r1.xyz = -UniformVector_0.xyz + 1.0;
    r0.w = ps;
    r0.xyz = (r8.zzz > 0.0) ? r0.yzw : float3(0.0, 0.0, 1.0);
    r0.yzw = (r8.zzz >= 0.0) ? r0.xyz : float3(0.0, 0.0, 1.0);
    r0.x = dot(r0.wyz, r0.wyz);
    ps = rsqrt(abs(r0.x));
    r4.xyz = r4.xyz * r1.www;
    r0.x = ps;
    r0.yzw = r0.yzw * r0.xxx;
    r0.x = dot(r0.wyz, r7.zxy);
    r8.xyz = r0.yzw * r0.xxx;
    r7.xyz = r8.xyz * 2.0 - r7.xyz;
    r0.x = saturate(dot(r5.zxy, r7.zxy));
    ps = log2(r0.x);
    r1.xyz = r6.xyz * r1.xyz;
    r0.x = ps;
    ps = 15.0 * r0.x;
    r0.x = ps;
    ps = pow(2.0, r0.x);
    r0.w = saturate(dot(r0.wzy, r5.zyx));
    r0.x = ps;
    r0.xyz = r4.xyz * r0.xxx;
    r0.xy = r1.xy * r0.ww + r0.xy;
    r0.z = r1.z * r0.w + r0.z;
    r0.xyz = r3.xyz * r0.xyz;
    r0.xyz = r0.xzy * r2.xzy;
    oC0.xyz = r0.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
