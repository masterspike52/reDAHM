// ps_4c1ace4c6bb5bbff.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 435 ucode dwords, 8 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000080 000006CC 10041300 0000080A 00000000 00007108 003F00FF 00000021 00003050 00003151 0000F254 0000F356 0000F457 0000F558 0000F6A0 0000F7A1
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0x3)
//   interpolator: r2 <-> TEXCOORD4 (flags 0xF)
//   interpolator: r3 <-> TEXCOORD6 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r5 <-> TEXCOORD8 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
// Definition header: 00000000 00000003 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 AmbientColorAndSkyFactor : register(c27); // float4
float4 ConstantLighting : register(c28); // float3
float4 LocalToWorldMatrix[3] : register(c3); // float3x3 (matrix_columns)
float4 LowerSkyColor : register(c26); // float3
float4 ModShadowAccumResolution : register(c31); // float2
float4 ModShadowColor : register(c29); // float3
float4 ModShadowGroupColor : register(c30); // float3
float4 OpacityOverride : register(c24); // float
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 UniformScalar_0 : register(c12); // float
float4 UniformScalar_1 : register(c13); // float
float4 UniformScalar_10 : register(c21); // float
float4 UniformScalar_11 : register(c22); // float
float4 UniformScalar_12 : register(c23); // float
float4 UniformScalar_3 : register(c14); // float
float4 UniformScalar_4 : register(c15); // float
float4 UniformScalar_5 : register(c16); // float
float4 UniformScalar_6 : register(c17); // float
float4 UniformScalar_7 : register(c18); // float
float4 UniformScalar_8 : register(c19); // float
float4 UniformScalar_9 : register(c20); // float
float4 UniformVector_0 : register(c6); // float4
float4 UniformVector_1 : register(c7); // float4
float4 UniformVector_2 : register(c8); // float4
float4 UniformVector_4 : register(c9); // float4
float4 UniformVector_5 : register(c10); // float4
float4 UniformVector_6 : register(c11); // float4
float4 UpperSkyColor : register(c25); // float3
sampler2D Texture2D_0 : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D Texture2D_5 : register(s5);
sampler2D Texture2D_6 : register(s6);
sampler2D Texture2D_7 : register(s7);
samplerCUBE TextureCube_0 : register(s8);
sampler2D ModShadowAccumTexture : register(s9);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord6 : TEXCOORD6; // r3
    float4 texcoord7 : TEXCOORD7; // r4
    float4 texcoord8 : TEXCOORD8; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord6;
    float4 r4 = In.texcoord7;
    float4 r5 = In.texcoord8;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = 0.0;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 r13 = 0.0;
    float4 r14 = 0.0;
    float4 r15 = 0.0;
    float4 r16 = 0.0;
    float4 r17 = 0.0;
    float4 r18 = 0.0;
    float4 r19 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r12 = tex2D(Texture2D_6, r0.xy);
    ps = 1.0 / ModShadowAccumResolution.y;
    r7.w = r12.w - 0.5;
    r0.w = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r14.xyz = UniformVector_6.xyz * UniformVector_6.www;
    r0.z = ps;
    r0.zw = r0.zw * abs(r8.xy);
    if (any(float4(0.0, 0.0, 0.0, 0.0) > r7.wwww)) clip(-1.0);
    r8.zw = tex2D(ModShadowAccumTexture, r0.zw).xy;
    ps = (-4e+02) - -r3.w;
    r8.x = r1.x * UniformVector_2.x;
    r0.z = ps;
    ps = r0.x;
    r1.zw = r1.xy * 6.0;
    ps = UniformScalar_0.x * ps;
    r7.w = dot(r4.zxy, r4.zxy);
    r3.x = ps;
    ps = rsqrt(abs(r7.w));
    r8.y = saturate(r0.z * 0.00022222222);
    r7.w = ps;
    ps = -r1.y;
    r9.xyz = r7.www * r4.xyz;
    ps = 1.0 + ps;
    r19.yzw = r8.zwy * float3(0.875, 0.875, -0.025);
    r7.w = ps;
    ps = UniformVector_2.y * r7.w;
    r8.zw = r9.xy * r19.ww;
    r8.y = ps;
    ps = r1.x;
    r3.yz = r8.xy - 0.5;
    ps = UniformVector_1.x * ps;
    r0.zw = float2((r8.xy >= 0.5));
    r4.x = ps;
    ps = r1.y;
    r3.yz = r3.yz - r8.xy;
    ps = UniformVector_1.y * ps;
    r4.zw = r3.yz * r0.zw;
    r4.y = ps;
    ps = r0.y;
    r11 = r8.zwxy + r4;
    ps = UniformScalar_0.x * ps;
    r4.xy = r11.zw + r11.zw;
    r3.y = ps;
    r13 = tex2D(Texture2D_1, r4.xy);
    r8.xyw = tex2D(Texture2D_0, r11.xy).xyz;
    r10.xyz = tex2D(Texture2D_4, r0.xy).xyz;
    r3.xyz = tex2D(Texture2D_3, r3.xy).xyw;
    r4.yzw = tex2D(Texture2D_2, r1.zw).xyw;
    r1.xyz = tex2D(Texture2D_2, r1.xy).xyw;
    r4.x = dot(r2.zxy, r2.zxy);
    r6 = r6.yxzw * 2.0 - 1.0;
    r18.xyz = r7.xyz * 2.0 - 1.0;
    r12.xyz = r14.xyz * r12.xyz;
    r14.w = r4.w * r1.z - 1.0;
    r4.yz = r4.yz * 2.0 - 1.0;
    r7.xy = r3.xy + r3.xy;
    ps = r1.x + r1.x;
    r7.w = dot(r18.zxy, r18.zxy);
    r7.z = ps;
    ps = rsqrt(abs(r4.x));
    r1.w = dot(r6.zyx, r6.zyx);
    r4.x = ps;
    r17.xyz = r10.xyz * 2.0 - 1.0;
    r16.xyz = r8.xyw * 2.0 - 1.0;
    r14.z = r17.z * UniformVector_4.z - r16.z;
    ps = rsqrt(abs(r1.w));
    r10.xyz = r4.xxx * r2.xyz;
    r1.w = ps;
    ps = rsqrt(abs(r7.w));
    r15.xyz = r6.xzy * r1.www;
    r7.w = ps;
    ps = r1.y + r1.y;
    r8.xyz = r18.xzy * r7.www;
    r7.w = ps;
    ps = OpacityOverride.x;
    r18 = r7 - 1.0;
    r7.x = saturate(ps);
    ps = r3.w;
    r7.yzw = r15.yzx * r8.zyx;
    r7.yzw = r15.xyz * r8.yxz - r7.yzw;
    ps = 0.0001 * ps;
    r1.xyz = r7.yzw * r6.www;
    r2.w = saturate(ps);
    ps = (-1.0) + r3.z;
    r7.w = dot(r1.zxy, r1.zxy);
    r19.x = ps;
    ps = r9.z;
    r10.w = dot(r0.zww, float3(1.0, 1.0, 1.0));
    r0.z = ps;
    r6.w = (r10.w == 0.0) ? r13.x : r13.y;
    ps = 0.0;
    r6.xyz = r10.zww + float3(0.1, -3.0, -2.0);
    r0.w = ps;
    r6.w = (r6.z == 0.0) ? r13.z : r6.w;
    r7.y = (r6.y == 0.0) ? r13.w : r6.w;
    ps = 5.0 * r6.x;
    r6.w = max(r7.y, 0.0);
    r7.z = saturate(ps);
    ps = max(r0.z, r0.w);
    r7.xyz = -r7.xyz + 1.0;
    r2.z = ps;
    ps = ModShadowGroupColor.x * r7.z;
    r4.x = min(r6.w, 0.3);
    r2.x = ps;
    ps = ModShadowGroupColor.y * r7.z;
    r6.xyz = r4.yzx * float3(0.5, 0.5, 3.3333333);
    r2.y = ps;
    ps = 2.5 * r7.y;
    r4 = -r2.xzyw + 1.0;
    r13.x = ps;
    ps = r4.w;
    r14.xy = r18.zw + r6.xy;
    r6.y = ps;
    r2.yzw = r19.xyz * r4.wxz + float3(1.0, 0.125, 0.125);
    r4.x = r6.z * r14.w + 1.0;
    ps = UniformScalar_1.x * r6.y;
    r13.yzw = r12.xyz * r2.yyy;
    r6.w = ps;
    r0.zw = r6.ww * r18.xy + r17.xy;
    r6.xy = r0.zw * UniformVector_4.xy - r16.xy;
    r0.w = saturate(r13.x * r4.x - 0.5);
    r14.xy = r6.zz * r14.xy + r6.xy;
    r6.xy = r14.xy * r0.ww + r16.xy;
    r6.z = r14.z * r0.w - 2.0;
    r6.z = r8.w * 2.0 + r6.z;
    r6.xyz = r6.xzy * r4.www;
    ps = 1.0 + r6.y;
    r6.w = ps;
    r6.xyz = (r4.www > 0.0) ? r6.xzw : float3(0.0, 0.0, 1.0);
    r6.yzw = (r4.www >= 0.0) ? r6.xyz : float3(0.0, 0.0, 1.0);
    ps = rsqrt(abs(r7.w));
    r6.x = dot(r6.wyz, r6.wyz);
    r7.w = ps;
    ps = rsqrt(abs(r6.x));
    r14.xyz = r1.xzy * r7.www;
    r6.x = ps;
    r1.xyz = r6.yzw * r6.xxx;
    r6.x = dot(r1.zxy, r9.zxy);
    r6.xyz = r1.xzy * r6.xxx;
    r9.xyz = r6.xyz * 2.0 - r9.xzy;
    r6.xyz = r15.zxy * r9.yyy;
    r6.xyz = r14.xzy * r9.zzz + r6.xyz;
    r6.xyw = r8.yzx * r9.xxx + r6.zyx;
    r8.xyz = r6.xxx * LocalToWorldMatrix[2].xyz;
    r6.xyz = r6.yyy * LocalToWorldMatrix[1].xyz + r8.xyz;
    r6.xyz = r6.www * LocalToWorldMatrix[0].xzy + r6.xzy;
    r6 = xe_cube(r6.xzy);
    ps = 1.0 / abs(r6.z);
    r8.z = r6.w;
    r7.w = ps;
    r8.xy = r6.yx * r7.ww + 1.5;
    r1.w = tex2D(Texture2D_5, r0.xy).x;
    r6.xzw = tex2D(Texture2D_7, r11.xy).xyz;
    r8.yzw = texCUBE(TextureCube_0, xe_cube_dir(r8.xyz)).xyz;
    ps = UniformScalar_9.x;
    r15.xyz = UniformVector_5.xyz * UniformVector_5.www;
    ps = (-XE_FLT_MAX) * ps;
    r7.w = dot(r5.zxy, r5.zxy);
    r6.y = ps;
    ps = rsqrt(abs(r7.w));
    r14.xyz = -ModShadowColor.xyz + 1.0;
    r4.w = ps;
    ps = pow(2.0, r6.y);
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r6.y = ps;
    ps = UniformScalar_10.x * r6.y;
    r11.xyz = r0.xyz * AmbientColorAndSkyFactor.xyz;
    r12.w = ps;
    ps = r4.w;
    r6.y = dot(r8.wyz, float3(0.11, 0.3, 0.59));
    r12.xyz = r12.xyz * UniformScalar_7.xxx + UniformScalar_8.xxx;
    ps = r5.x * ps;
    r16.xyz = r6.yyy - r8.yzw;
    r8.x = ps;
    r8.yzw = r16.xyz * UniformScalar_6.xxx + r8.yzw;
    ps = log2(abs(r4.y));
    r8.yzw = r15.xyz * r8.yzw;
    r6.y = ps;
    ps = UniformScalar_3.x * r6.y;
    r7.x = float((r7.x >= 0.004));
    r6.y = ps;
    r7.yzw = r13.yzw * UniformScalar_11.xxx + UniformScalar_12.xxx;
    ps = pow(2.0, r6.y);
    r2.x = abs(r4.x) * abs(r4.x);
    r6.y = ps;
    r4.xyz = r13.yzw * r4.xxx - r6.xzw;
    r6.y = r6.y * UniformScalar_5.x + UniformScalar_4.x;
    ps = r4.w;
    r13.xyz = r8.yzw * r6.yyy;
    r4.xyz = r4.xyz * r0.www + r6.xzw;
    ps = r5.y * ps;
    r6.xw = r2.zx * r2.wx;
    r8.y = ps;
    r6.xyz = r6.xxx * r14.xyz + ModShadowColor.xyz;
    ps = r4.w;
    r12.xyz = r13.xyz * r12.xyz;
    ps = r5.z * ps;
    r2 = r12 * r2.yyyy;
    r8.z = ps;
    ps = r6.w * r6.w;
    r5 = r2 * r1.wwww;
    r6.w = ps;
    ps = r4.x;
    r2.xyz = r7.yzw + r5.www;
    r7.yzw = r5.xyz * r0.www + UniformVector_0.xyz;
    r7.yzw = r11.xyz * r4.xyz + r7.yzw;
    ps = r0.x * ps;
    r2.xyz = r6.www * r2.xyz;
    r0.x = ps;
    ps = r4.y;
    r2.xyz = r2.xzy * r0.www;
    ps = r0.y * ps;
    r6.w = saturate(dot(r10.zxy, r9.yxz));
    r0.y = ps;
    ps = log2(r6.w);
    r5.x = dot(r8.zxy, r1.zxy);
    r5.y = ps;
    ps = r4.z;
    r1.xyw = r5.xxy * float3(-0.5, 0.5, 15.0);
    ps = r0.z * ps;
    r4.xy = r1.xy + 0.5;
    r0.z = ps;
    ps = pow(2.0, r1.w);
    r4.yz = abs(r4.xy) * abs(r4.xy);
    r6.w = ps;
    ps = ConstantLighting.x * r6.w;
    r1.xyz = r1.zzz * ConstantLighting.xyz;
    r4.x = ps;
    ps = ConstantLighting.z * r6.w;
    r5.xyz = r0.xzy * r4.yyy;
    r4.y = ps;
    ps = ConstantLighting.y * r6.w;
    r8.xyz = r0.xzy * r4.zzz;
    r4.z = ps;
    r7.yzw = r8.xyz * UpperSkyColor.xzy + r7.ywz;
    r7.yzw = r5.xzy * LowerSkyColor.xyz + r7.ywz;
    r7.yzw = r4.xyz * r2.xyz + r7.ywz;
    r7.yzw = r1.xyz * r0.xyz + r7.ywz;
    ps = -r3.w;
    r6.xyz = r7.yzw * r6.xyz;
    ps = OpacityOverride.x + ps;
    r0.xyz = r6.xyz - r6.xyz;
    r0.w = ps;
    oC0.w = r0.w * r7.x + r3.w;
    r6.xyz = r0.xyz * r7.xxx + r6.xyz;
    oC0.xyz = r6.xyz * SCENE_COLOR_BIAS_FACTOR.xxx;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
