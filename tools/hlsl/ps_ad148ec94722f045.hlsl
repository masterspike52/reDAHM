// ps_ad148ec94722f045.bin
// Xenos pixel shader decompiled to HLSL (D3D9 / SM3).
// Container flags 0x102A1100, 186 ucode dwords, 4 literal constant(s).
// Constant table creator: 2.0.6995.0 (ps_3_0)
// Shader header: 00000040 000002E8 10040C00 0000090A 00000000 00008129 003F01FF 00000001 00003050 0000F151 00007254 0000F355 00007456 0000F557 0000F6A0 0000F7A1 0000F8A2
//   interpolator: r0 <-> TEXCOORD0 (flags 0x3)
//   interpolator: r1 <-> TEXCOORD1 (flags 0xF)
//   interpolator: r2 <-> TEXCOORD4 (flags 0x7)
//   interpolator: r3 <-> TEXCOORD5 (flags 0xF)
//   interpolator: r4 <-> TEXCOORD6 (flags 0x7)
//   interpolator: r5 <-> TEXCOORD7 (flags 0xF)
//   interpolator: r6 <-> COLOR0 (flags 0xF)
//   interpolator: r7 <-> COLOR1 (flags 0xF)
//   interpolator: r8 <-> COLOR2 (flags 0xF)
// Definition header: 00000000 00000001 00000000 00000000
// Non-float definitions (raw): 00000000 00000000

#include "xenos_common.hlsli"

float4 LightColorAndFalloffExponent : register(c8); // float4
float4 ModShadowAccumResolution : register(c11); // float2
float4 ModShadowColor : register(c9); // float3
float4 ModShadowGroupColor : register(c10); // float3
float4 SCENE_COLOR_BIAS_FACTOR : register(c0); // float4
float4 ScreenPositionScaleBias : register(c1); // float4
float4 UniformScalar_0 : register(c6); // float
float4 UniformScalar_1 : register(c7); // float
float4 UniformVector_0 : register(c3); // float4
float4 UniformVector_1 : register(c4); // float4
float4 UniformVector_2 : register(c5); // float4
sampler2D LightAttenuationTexture : register(s0);
sampler2D Texture2D_1 : register(s1);
sampler2D Texture2D_2 : register(s2);
sampler2D Texture2D_3 : register(s3);
sampler2D Texture2D_4 : register(s4);
sampler2D ShadowTexture : register(s5);
sampler2D ModShadowAccumTexture : register(s6);

struct PS_INPUT
{
    float4 texcoord0 : TEXCOORD0; // r0
    float4 texcoord1 : TEXCOORD1; // r1
    float4 texcoord4 : TEXCOORD4; // r2
    float4 texcoord5 : TEXCOORD5; // r3
    float4 texcoord6 : TEXCOORD6; // r4
    float4 texcoord7 : TEXCOORD7; // r5
    float4 color0 : COLOR0; // r6
    float4 color1 : COLOR1; // r7
    float4 color2 : COLOR2; // r8
};

struct PS_OUTPUT
{
    float4 color0 : COLOR0;
};

PS_OUTPUT main(PS_INPUT In)
{
    float4 r0 = In.texcoord0;
    float4 r1 = In.texcoord1;
    float4 r2 = In.texcoord4;
    float4 r3 = In.texcoord5;
    float4 r4 = In.texcoord6;
    float4 r5 = In.texcoord7;
    float4 r6 = In.color0;
    float4 r7 = In.color1;
    float4 r8 = In.color2;
    float4 r9 = 0.0;
    float4 r10 = 0.0;
    float4 r11 = 0.0;
    float4 r12 = 0.0;
    float4 oC0 = 0.0;
    float ps = 0.0;

    r7.x = r1.w * UniformVector_2.x;
    r7.w = 1.0 / ModShadowAccumResolution.y;
    r6.x = -r1.z + 1.0;
    r6.z = 1.0 / r5.w;
    ps = ScreenPositionScaleBias.x * r6.z;
    r6.w = dot(r2.zxy, r2.zxy);
    r7.y = ps;
    ps = 1.0 / ModShadowAccumResolution.x;
    r6.y = dot(r3.zxy, r3.zxy);
    r7.z = ps;
    ps = rsqrt(abs(r6.w));
    r0.zw = r7.zw * abs(r9.xy);
    r7.z = ps;
    ps = ScreenPositionScaleBias.y * r6.z;
    r6.w = dot(r4.zxy, r4.zxy);
    r7.w = ps;
    r5.xy = r7.yw * r5.xy + ScreenPositionScaleBias.wz;
    ps = rsqrt(abs(r6.w));
    r8.xyw = r7.zzz * r2.xyz;
    r6.z = ps;
    ps = UniformVector_2.y * r6.x;
    r3.xyw = r6.zzz * r4.xyz;
    r7.y = ps;
    ps = (-0.5) + r7.x;
    r9.xy = r8.xy * -r3.xy;
    r6.x = ps;
    ps = (-0.5) + r7.y;
    r4.xy = float2((r7.xy >= 0.5));
    r6.z = ps;
    ps = 1.0 - r6.y;
    r6.xw = r6.xz - r7.xy;
    r6.z = saturate(ps);
    r9.zw = r6.xw * r4.xy + r7.xy;
    r6.xyw = r9.zwx + r9.zwy;
    r8.xyz = tex2D(LightAttenuationTexture, r5.xy).xyz;
    r3.xyz = tex2D(Texture2D_3, r1.wz).xyz;
    r4.z = tex2D(ShadowTexture, r0.xy).x;
    r10.xyz = tex2D(Texture2D_4, r1.xy).xyz;
    r11.xy = tex2D(ModShadowAccumTexture, r0.zw).xy;
    r12 = tex2D(Texture2D_2, r6.xy);
    r5 = tex2D(Texture2D_1, r1.xy);
    ps = r5.w;
    r0.xyz = -UniformVector_0.xyz + 1.0;
    r6.x = ps;
    ps = 1.0 - r6.x;
    r7.x = r5.w * UniformScalar_0.x;
    r7.y = ps;
    r9.xyz = r7.yyy * UniformVector_1.xyz + r7.xxx;
    ps = log2(r6.z);
    r7.w = saturate(r7.z * r2.z);
    r6.x = ps;
    r7.x = r3.w * 2.0 - r3.w;
    ps = 0.1 - -r2.z;
    r7.y = dot(r4.xyy, float3(1.0, 1.0, 1.0));
    r6.y = ps;
    r6.z = (r7.y == 0.0) ? r12.x : r12.y;
    ps = 5.0 * r6.y;
    r7.yz = r7.yy + float2(-3.0, -2.0);
    r6.y = saturate(ps);
    r6.z = (r7.z == 0.0) ? r12.z : r6.z;
    r6.z = (r7.y == 0.0) ? r12.w : r6.z;
    r7.z = saturate(r8.w * r7.x + r6.w);
    r11.zw = -r6.yz + 1.0;
    r7.xy = -r11.zz * ModShadowGroupColor.xy + 1.0;
    ps = log2(r7.z);
    r6.yzw = r11.wxy * float3(2.5, 0.875, 0.875);
    r10.w = ps;
    r1 = r10 * float4(0.5, 0.5, 0.5, 15.0);
    r4.xy = r6.zw * r7.xy + 0.125;
    ps = (-0.5) + r6.y;
    r7.xyz = r1.xyz * r5.www;
    r0.w = saturate(ps);
    r2.xyz = r7.xyz * UniformScalar_1.xxx + r9.xyz;
    r5.xyz = r5.xyz * 2e+01 + r7.xyz;
    ps = r4.x * r4.y;
    r6.yzw = -ModShadowColor.xyz + 1.0;
    r7.y = ps;
    ps = r4.z * r4.z;
    r1.xyz = r5.xyz - r3.xyz;
    r8.w = ps;
    ps = pow(2.0, r1.w);
    r2.xyz = r2.xyz * r0.www;
    r7.x = ps;
    r6.yzw = r7.yyy * r6.yzw + ModShadowColor.xyz;
    ps = LightColorAndFalloffExponent.w * r6.x;
    r7.xyz = r2.xyz * r7.xxx;
    r6.x = ps;
    r1.xyz = r1.xyz * r0.www + r3.xyz;
    ps = pow(2.0, r6.x);
    r8.xyz = r8.www * r8.xyz;
    r6.x = ps;
    r8.xyz = r8.xzy * r6.xxx;
    r0.xyz = r1.xyz * r0.xyz;
    r7.xy = r0.xy * r7.ww + r7.xy;
    r7.z = r0.z * r7.w + r7.z;
    r7.xyz = r8.xzy * r7.xyz;
    r7.xyz = r7.xyz * LightColorAndFalloffExponent.xyz;
    r6.xyz = r7.xzy * r6.ywz;
    oC0.xyz = r6.xzy * SCENE_COLOR_BIAS_FACTOR.xxx;
    oC0.w = 0.0;

    PS_OUTPUT Out;
    Out.color0 = oC0;
    return Out;
}
