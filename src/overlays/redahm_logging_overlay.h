...
struct REDAHMLogEntry {
    spdlog::level::level_enum level;
    std::string               message;
};

class REDAHMLogOverlaySink : public spdlog::sinks::base_sink<std::mutex> {
public:
    static constexpr std::size_t kMaxEntries = 200;

    std::deque<REDAHMLogEntry> entries;
...
class REDAHMLogOverlayDialog : public rex::ui::ImGuiDialog {
public:
    explicit REDAHMLogOverlayDialog(rex::ui::ImGuiDrawer* drawer)
        : rex::ui::ImGuiDialog(drawer)
        , sink_(std::make_shared<REDAHMLogOverlaySink>())
    {
        rex::ui::RegisterBind("bind_REDAHM_log_overlay", "F2",
            "Toggle REDAHM log overlay", [this] {
                visible_ = !visible_;
            });

        rex::AddSink(redahm::log::Input, sink_);
    }

    ~REDAHMLogOverlayDialog() {
        rex::ui::UnregisterBind("bind_REDAHM_log_overlay");
        rex::RemoveSink(redahm::log::Input, sink_);
    }
    ...
    void OnDraw(ImGuiIO& io) override {
        ...
        if (!ImGui::Begin("REDAHM log##REDAHM_log_overlay", &visible_, flags)) {
            ImGui::End();
            return;
        }
        ...
        sink_->with_entries([&](const std::deque<REDAHMLogEntry>& entries) {
...
private:
    ...
    std::shared_ptr<REDAHMLogOverlaySink> sink_;
};